...
#[test]
fn cmd_writer_stage_ex_option_pixel_encodes_legacy_pixel_stage() {
    let mut w = AerogpuCmdWriter::new();
    w.set_texture_stage_ex(
        AerogpuShaderStage::Vertex,
        Some(AerogpuShaderStageEx::Pixel),
        0,
        99,
    );
    let buf = w.finish();
    ...
    // stageEx=Pixel should use the legacy shader_stage field (Pixel) with reserved0=0.
