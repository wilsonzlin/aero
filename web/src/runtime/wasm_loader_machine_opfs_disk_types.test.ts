...
describe("runtime/wasm_loader (Machine OPFS disk typings)", () => {
  it("requires feature detection and uses bigint sizeBytes", () => {
    ...
    const machine = {
      set_disk_opfs: async (_path: string, _create: boolean, _sizeBytes: bigint, _baseFormat?: string) => {},
      ...
      set_disk_opfs_existing: async (_path: string, _baseFormat?: string, _expectedSizeBytes?: bigint) => {},
    ...
    function assertStrictNullChecksEnforced() {
      ...
      // @ts-expect-error baseFormat must be a string when provided
      machine.set_disk_opfs?.("disk.img", true, 1024n, 123);
      ...
      // @ts-expect-error expectedSizeBytes must be a bigint when provided
      machine.set_disk_opfs_existing?.("disk.img", "raw", 1024);
    }
    void assertStrictNullChecksEnforced;

    if (machine.set_disk_opfs) {
      void machine.set_disk_opfs("disk.img", true, 1024n, "raw");
    }
    ...
    if (machine.set_disk_opfs_existing) {
      void machine.set_disk_opfs_existing("disk.img", "aerospar", 1024n);
    }
