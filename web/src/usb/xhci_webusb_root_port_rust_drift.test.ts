...
function parseRustU8Const(source: string, name: string): number {
  ...
  if (!match) {
    throw new Error(`Failed to locate \`const ${name}: u8\` in crates/aero-wasm/src/xhci_controller_bridge.rs`);
  }
...
describe("xHCI WebUSB root port reservation matches web runtime topology", () => {
  it("reserves a different root port than the external hub so WebUSB + WebHID/synthetic HID can coexist", () => {
    ...
    const xhciWebusbRootPort = parseRustU8Const(rust, "WEBUSB_ROOT_PORT");
    expect(xhciWebusbRootPort).toBe(WEBUSB_GUEST_ROOT_PORT);
    expect(xhciWebusbRootPort).not.toBe(EXTERNAL_HUB_ROOT_PORT);
  });
});
