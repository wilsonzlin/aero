    ...
    try {
      ...
      for (const raw of drained) {
        if (!isUsbHostAction(raw)) {
          ...
          const id = (raw && typeof raw === "object" && Number.isFinite((raw as { id?: unknown }).id) ? (raw as { id: number }).id : null) as
            | number
            | null;
          if (id !== null) {
            try {
              this.#bridge.push_completion(usbErrorCompletion(id, "Invalid UsbHostAction received from WASM."));
        ...
        if (this.#pending.has(id)) {
          ...
          try {
            this.#bridge.push_completion(usbErrorCompletion(id, `Duplicate UsbHostAction id received from WASM: ${id}`));
            this.#completionsApplied++;
          } catch (err) {
            this.#lastError = formatError(err);
          }
