...
describe("workers/vm_snapshot_wasm", () => {
  ...
  it("maps snapshot device kinds/ids for WorkerVmSnapshot", () => {
    ...
    expect(VM_SNAPSHOT_DEVICE_ID_AUDIO_VIRTIO_SND).toBe(22);
    expect(VM_SNAPSHOT_DEVICE_AUDIO_VIRTIO_SND_KIND).toBe("audio.virtio_snd");
    expect(VM_SNAPSHOT_DEVICE_ID_AEROGPU).toBe(25);
    expect(VM_SNAPSHOT_DEVICE_GPU_AEROGPU_KIND).toBe("gpu.aerogpu");
    ...
    expect(vmSnapshotDeviceKindToId(VM_SNAPSHOT_DEVICE_AUDIO_VIRTIO_SND_KIND)).toBe(VM_SNAPSHOT_DEVICE_ID_AUDIO_VIRTIO_SND);
    expect(vmSnapshotDeviceIdToKind(VM_SNAPSHOT_DEVICE_ID_AUDIO_VIRTIO_SND)).toBe(VM_SNAPSHOT_DEVICE_AUDIO_VIRTIO_SND_KIND);

    expect(vmSnapshotDeviceKindToId(VM_SNAPSHOT_DEVICE_GPU_AEROGPU_KIND)).toBe(VM_SNAPSHOT_DEVICE_ID_AEROGPU);
    expect(vmSnapshotDeviceIdToKind(VM_SNAPSHOT_DEVICE_ID_AEROGPU)).toBe(VM_SNAPSHOT_DEVICE_GPU_AEROGPU_KIND);
