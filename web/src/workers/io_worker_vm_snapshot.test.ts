...
import { IO_WORKER_VRAM_SNAPSHOT_DEVICE_ID_BASE } from "./io_worker_vram_snapshot";
...
const VM_SNAPSHOT_DEVICE_VIRTIO_INPUT_ID_KIND = `${VM_SNAPSHOT_DEVICE_KIND_PREFIX_ID}${VM_SNAPSHOT_DEVICE_ID_VIRTIO_INPUT}`;
...
describe("snapshot usb: workers/io_worker_vm_snapshot", () => {
  ...
  it("includes gpu.vram device blobs on save when a VRAM buffer is provided", async () => {
    const calls: Array<{ path: string; devices: unknown }> = [];
    const api = {
      vm_snapshot_save_to_opfs: (path: string, _cpu: Uint8Array, _mmu: Uint8Array, devices: unknown) => {
        calls.push({ path, devices });
      },
    } as unknown as WasmApi;
    ...
    const payload = calls[0]!.devices as Array<{ kind: string; bytes: Uint8Array }>;
    expect(payload).toHaveLength(1);
    expect(payload[0]!.kind).toBe(`device.${VM_SNAPSHOT_DEVICE_ID_GPU_VRAM}`);
    const bytes = payload[0]!.bytes;
  ...
  it("includes gpu.vram device blobs on save when opts.vram is provided (legacy buffer)", async () => {
    const calls: Array<{ path: string; devices: unknown }> = [];
    const api = {
      vm_snapshot_save_to_opfs: (path: string, _cpu: Uint8Array, _mmu: Uint8Array, devices: unknown) => {
        calls.push({ path, devices });
  ...
  it("prefers vramU8 over opts.vram when both are provided (BAR1 wins)", async () => {
    const calls: Array<{ devices: unknown }> = [];
    const api = {
      vm_snapshot_save_to_opfs: (_path: string, _cpu: Uint8Array, _mmu: Uint8Array, devices: unknown) => {
        calls.push({ devices });
    ...
    expect(payload).toHaveLength(1);
    expect(payload[0]!.kind).toBe(`device.${VM_SNAPSHOT_DEVICE_ID_GPU_VRAM}`);
    expect(Array.from(payload[0]!.bytes.subarray(24, 24 + 16))).toEqual(Array.from(vramU8));
  });
  ...
  it("applies gpu.vram blobs into opts.vram (when vramU8 is absent) and does not forward/cache them", async () => {
    const makeVramChunk = (data: Uint8Array, chunkIndex = 0): Uint8Array => {
      const headerBytes = 24;
      const out = new Uint8Array(headerBytes + data.byteLength);
      // "AERO"
    ...
    expect(vram.subarray(0, data.byteLength)).toEqual(data);
    expect(vram.subarray(data.byteLength)).toEqual(new Uint8Array(vram.byteLength - data.byteLength));
    expect(res.devices).toBeUndefined();
    expect(res.restoredDevices).toEqual([]);
  });
  ...
  it("chunks gpu.vram into multiple device blobs on save when vramChunkBytes override is provided (test-only)", async () => {
    ...
    await saveIoWorkerVmSnapshotToOpfs({
      ...
      guestSize: 0x1000,
      vramU8,
      // Use a small chunk size so we can exercise chunking without allocating a >64MiB array.
      vramChunkBytes: 64,
      runtimes: {
        usbXhciControllerBridge: null,
        usbUhciRuntime: null,
  ...
  it("applies gpu.vram blobs into opts.vram via WorkerVmSnapshot builder restore (when vramU8 is absent) and does not forward/cache them", async () => {
    const makeVramChunk = (data: Uint8Array): Uint8Array => {
      const headerBytes = 24;
      const out = new Uint8Array(headerBytes + data.byteLength);
      // "AERO"
    ...
    class FakeBuilder {
      ...
      async restore_snapshot_from_opfs(_path: string): Promise<unknown> {
        return {
          ...
          devices: [
            { id: VM_SNAPSHOT_DEVICE_ID_GPU_VRAM, version: 1, flags: 0, data: vramBlob },
            { id: 123, version: 1, flags: 0, data: unknown },
          ],
        };
      }
      free(): void {
    ...
    expect(vram.subarray(0, snapshotData.length)).toEqual(snapshotData);
    expect(vram.subarray(snapshotData.length)).toEqual(new Uint8Array(vram.byteLength - snapshotData.length));
    expect(res.devices?.map((d) => d.kind)).toEqual(["device.123"]);
    expect(res.devices?.[0]?.bytes).toBeInstanceOf(ArrayBuffer);
    expect(res.restoredDevices.map((d) => d.kind)).toEqual(["device.123"]);
  });
  ...
  it("restores BAR1 VRAM blobs into vramU8 and does not forward them to the coordinator", async () => {
    ...
    const restore = vi.fn(() => ({
      ...
      mmu: new Uint8Array([0xbb]),
      devices: [
        { kind: `device.${IO_WORKER_VRAM_SNAPSHOT_DEVICE_ID_BASE}`, bytes: snapshotVram },
        { kind: "device.123", bytes: unknown },
      ],
    }));
    ...
    // VRAM should be restored into the IO worker's local BAR1 view.
    expect(vramU8.subarray(0, snapshotVram.length)).toEqual(snapshotVram);
    expect(vramU8.subarray(snapshotVram.length)).toEqual(new Uint8Array(vramU8.length - snapshotVram.length));

    // VRAM blobs must not be forwarded back to the coordinator (or cached as restoredDevices).
    expect(res.devices?.map((d) => d.kind)).toEqual(["device.123"]);
    expect(res.restoredDevices.map((d) => d.kind)).toEqual(["device.123"]);
  });

  it("prefers reserved BAR1 VRAM blobs over gpu.vram when both are present (compat priority)", async () => {
    const makeVramChunk = (data: Uint8Array, chunkIndex = 0): Uint8Array => {
      const headerBytes = 24;
      const out = new Uint8Array(headerBytes + data.byteLength);
      // "AERO"
    ...
    const snapshotReserved = new Uint8Array([0x01, 0x02, 0x03, 0x04]);
    const snapshotGpuVram = new Uint8Array([0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f]);
    const vramBlob = makeVramChunk(snapshotGpuVram);
    const unknown = new Uint8Array([0xde, 0xad]);

    // Note: order intentionally includes gpu.vram first to ensure reserved-id VRAM still wins.
    const restore = vi.fn(() => ({
      cpu: new Uint8Array([0xaa]),
      mmu: new Uint8Array([0xbb]),
      devices: [
        { kind: `device.${VM_SNAPSHOT_DEVICE_ID_GPU_VRAM}`, bytes: vramBlob },
        { kind: `device.${IO_WORKER_VRAM_SNAPSHOT_DEVICE_ID_BASE}`, bytes: snapshotReserved },
        { kind: "device.123", bytes: unknown },
      ],
    }));
    const api = { vm_snapshot_restore_from_opfs: restore } as unknown as WasmApi;
    ...
    // Reserved-id VRAM should win and clear the remainder to zero.
    expect(vramU8.subarray(0, snapshotReserved.length)).toEqual(snapshotReserved);
    expect(vramU8.subarray(snapshotReserved.length)).toEqual(new Uint8Array(vramU8.length - snapshotReserved.length));
    expect(res.devices?.map((d) => d.kind)).toEqual(["device.123"]);
    expect(res.restoredDevices.map((d) => d.kind)).toEqual(["device.123"]);
  });

  it("clears vramU8 on restore when the snapshot contains no VRAM blobs", async () => {
    const restore = vi.fn(() => ({
      cpu: new Uint8Array([0xaa]),
      mmu: new Uint8Array([0xbb]),
      devices: [{ kind: "device.123", bytes: new Uint8Array([0x01]) }],
    }));
    const api = { vm_snapshot_restore_from_opfs: restore } as unknown as WasmApi;

    const vramU8 = new Uint8Array(32);
    vramU8.fill(0xcc);
    ...
    expect(vramU8).toEqual(new Uint8Array(vramU8.length));
  });
  ...
  it("preserves legacy raw EHCI USB blobs across restore → save by tagging them as EHCI in the merged container", async () => {
    const cachedEhciRaw = makeAeroIoSnapshotHeader("EHCI");
    const uhciFresh = makeAeroIoSnapshotHeader("UHRT");
    const saveCalls: Array<{ devices: unknown }> = [];
    const api = {
      vm_snapshot_save_to_opfs: (_path: string, _cpu: Uint8Array, _mmu: Uint8Array, devices: unknown) => {
    ...
    await saveIoWorkerVmSnapshotToOpfs({
      ...
      restoredDevices: [{ kind: "usb", bytes: cachedEhciRaw }],
    });
    ...
    const decoded = decodeUsbSnapshotContainer(payload[0]!.bytes);
    expect(decoded).not.toBeNull();
    expect(decoded!.entries.find((e) => e.tag === USB_SNAPSHOT_TAG_EHCI)?.bytes).toEqual(cachedEhciRaw);
    expect(decoded!.entries.find((e) => e.tag === USB_SNAPSHOT_TAG_UHCI)?.bytes).toEqual(uhciFresh);
  });
