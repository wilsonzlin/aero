...
describe("net/connectL2Tunnel", () => {
  ...
  it("resolves relative gateway base URLs against http:// location (ws:// WebSocket + http:// session)", async () => {
    const originalFetch = globalThis.fetch;
    const originalWs = (globalThis as unknown as Record<string, unknown>).WebSocket;
    const originalLocation = (globalThis as unknown as Record<string, unknown>).location;
    ...
    resetFakeWebSocket();
    (globalThis as unknown as Record<string, unknown>).WebSocket = FakeWebSocket as unknown as WebSocketConstructor;
    (globalThis as unknown as Record<string, unknown>).location = { href: "http://gateway.example.com/app/index.html" };
    ...
    try {
      expect(fetchMock).toHaveBeenCalledTimes(1);
      const [url] = (fetchMock as unknown as { mock: { calls: any[][] } }).mock.calls[0]!;
      expect(url).toBe("http://gateway.example.com/base/session");

      expect(FakeWebSocket.last?.url).toBe("ws://gateway.example.com/base/l2");
    } finally {
      tunnel.close();
      globalThis.fetch = originalFetch;
      if (originalWs === undefined) delete (globalThis as { WebSocket?: unknown }).WebSocket;
  ...
  it("supports same-origin relative gateway URLs that already include /l2 (session endpoint is a sibling)", async () => {
    const originalFetch = globalThis.fetch;
    const originalWs = (globalThis as unknown as Record<string, unknown>).WebSocket;
    const originalLocation = (globalThis as unknown as Record<string, unknown>).location;
    ...
    const events: L2TunnelEvent[] = [];
    const tunnel = await connectL2Tunnel("/base/l2", {
      mode: "ws",
      sink: (ev) => events.push(ev),
    });
