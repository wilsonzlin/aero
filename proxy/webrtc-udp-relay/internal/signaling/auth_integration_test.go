...
func TestAuth_JWT_RejectsConcurrentSessionsWithSameSID_WebSocketSignal_HeaderAlias(t *testing.T) {
	cfg := config.Config{
		AuthMode:  config.AuthModeJWT,
		JWTSecret: "supersecret",
	}
	...
	// Use a different header carrier/scheme for the second connection (forward
	// compat for mode-agnostic clients). This must still be rejected based on
	// JWT sid.
	h2 := http.Header{}
...
func TestAuth_JWT_RejectsConcurrentSessionsWithSameSID_HTTPOfferEndpoints(t *testing.T) {
	...
	t.Run("POST /webrtc/offer header X-API-Key alias", func(t *testing.T) {
		body, err := json.Marshal(map[string]any{
			"sdp": sdp{Type: "offer", SDP: "v=0"},
		})
		if err != nil {
	...
	t.Run("POST /webrtc/offer header Authorization ApiKey alias", func(t *testing.T) {
		body, err := json.Marshal(map[string]any{
			"sdp": sdp{Type: "offer", SDP: "v=0"},
		})
		if err != nil {
		...
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Authorization", "ApiKey "+tokenB)
	...
	t.Run("POST /offer header X-API-Key alias", func(t *testing.T) {
		body, err := json.Marshal(legacyOfferRequest{Version: 1, Offer: legacySessionDescription{Type: "offer", SDP: "v=0"}})
		if err != nil {
			t.Fatalf("marshal offer: %v", err)
		}
	...
	t.Run("POST /offer header Authorization ApiKey alias", func(t *testing.T) {
		body, err := json.Marshal(legacyOfferRequest{Version: 1, Offer: legacySessionDescription{Type: "offer", SDP: "v=0"}})
		if err != nil {
			t.Fatalf("marshal offer: %v", err)
		}
...
func TestAuth_APIKey_WebSocketSignal_HeaderAuth(t *testing.T) {
	cfg := config.Config{
		AuthMode:                      config.AuthModeAPIKey,
		APIKey:                        "secret",
		SignalingAuthTimeout:          2 * time.Second,
