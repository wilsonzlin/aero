...
test.describe("runtime disk IO worker", () => {
    ...
    test(`can read/write sectors (${backend})`, async ({ page }) => {
      ...
      const result = await page.evaluate(
        async ({ backend }) => {
          ...
          // RuntimeDiskClient.write may transfer (detach) standalone ArrayBuffer-backed views.
          // Capture expected bytes/length before calling write().
          const expected = Array.from(data);
          const expectedLength = data.length;
          await client.write(opened.handle, lba, data);
          await client.flush(opened.handle);

          const roundTrip = await client.read(opened.handle, lba, expectedLength);
          ...
          const roundTrip2 = await client.read(reopened.handle, lba, expectedLength);
  ...
  test("can write across IndexedDB chunk boundary", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      return { roundTrip: Array.from(roundTrip), expected };
    });
