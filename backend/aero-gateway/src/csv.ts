 ...
 *
 * Intended for parsing configuration strings defensively (e.g. env vars) while bounding work for
 * accidental huge values.
 */
export function splitCommaSeparatedList(
  ...
  while (i < raw.length) {
    ...
    let end = i;
    if (i < raw.length && raw.charCodeAt(i) === 0x2c /* ',' */) i += 1;
