...
// -----------------------------------------------------------------------------
// Minimal EHCI async schedule seed layout (all within MEM_SIZE).
// -----------------------------------------------------------------------------
...
fn biased_offset(u: &mut Unstructured<'_>, num_ports: usize) -> u64 {
    ...
    if sel & 0b11 != 0 {
        match sel % 12 {
            ...
            8 => REG_USBLEGCTLSTS,
            9 => reg_portsc(0),
            10 => reg_portsc(num_ports.saturating_sub(1)),
...
fn seed_async_schedule(bus: &mut FuzzBus) {
    // QH chain:
    ...
    // Keep QH overlays empty so `process_async_schedule` loads the first qTD from QH_NEXT_QTD.
...
fn rearm_interrupt_qtd(bus: &mut FuzzBus) {
    // After a successful IN transfer, `process_async_schedule` clears ACTIVE and sets TotalBytes=0.
    // Restore the qTD/QH state to keep ticks exercising the interrupt IN path.
    bus.write_u32(QTD_INTR_IN + QTD_TOKEN, make_token(QTD_PID_IN, 8, true));
    bus.write_u32(QTD_INTR_IN + QTD_BUF0, INTR_BUF_ADDR);
    bus.write_u32(QTD_INTR_IN + QTD_NEXT, LINK_TERMINATE);
    bus.write_u32(QTD_INTR_IN + QTD_ALT_NEXT, LINK_TERMINATE);

    // Reset the QH overlay so the qTD is reloaded from memory next tick.
    bus.write_u32(QH2_ADDR + QH_CUR_QTD, 0);
    bus.write_u32(QH2_ADDR + QH_NEXT_QTD, qtd_ptr(QTD_INTR_IN));
    bus.write_u32(QH2_ADDR + QH_ALT_NEXT_QTD, LINK_TERMINATE);
}
...
fuzz_target!(|data: &[u8]| {
    ...
    // Seed a tiny, valid async schedule so we exercise EHCI qTD processing even when `data` does
    // not contain a well-formed schedule.
    seed_async_schedule(&mut bus);
    ...
    // Point the controller at our async schedule head and run.
    ctl.mmio_write(REG_ASYNCLISTADDR, 4, QH0_ADDR);
    ctl.mmio_write(REG_USBCMD, 4, USBCMD_RS | USBCMD_ASE);
