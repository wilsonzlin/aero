...
| `AERO_SAFE_RUN_RUSTC_RETRIES` | Positive integer (attempt count, including the first run). Controls how many times `scripts/safe-run.sh` will retry Rust build/test commands (Cargo and common wrappers like `npm`/`wasm-pack`) when it detects transient OS resource-limit failures during thread/process spawn (EAGAIN/WouldBlock), e.g. `failed to spawn helper thread (WouldBlock)` or `fork: retry: Resource temporarily unavailable`. Set to `1` to disable retries. | `3` | `scripts/safe-run.sh` | `AERO_SAFE_RUN_RUSTC_RETRIES=1 bash ./scripts/safe-run.sh cargo test --locked` |
