...
5. Verify the virtio-input PCI device matches the Aero Win7 virtio contract v1 identity:
    ...
    - Note: the canonical in-tree virtio-input INF (`aero_virtio_input.inf`) is intentionally **SUBSYS-only**:
      - It binds only the Aero keyboard/mouse subsystem-qualified contract v1 HWIDs (`SUBSYS_0010`/`SUBSYS_0011`) for distinct Device Manager names, and
      - It does **not** contain the strict revision-gated generic fallback model line:
        - `PCI\VEN_1AF4&DEV_1052&REV_01` (no `SUBSYS`)
