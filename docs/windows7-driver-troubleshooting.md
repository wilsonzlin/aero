...
5. Verify the virtio-input PCI device matches the Aero Win7 virtio contract v1 identity:
   - In Device Manager → the virtio-input PCI device → Properties → Details → **Hardware Ids**
    - The list should include a **SUBSYS-qualified** contract v1 ID (keyboard or mouse), for example:
      - `PCI\VEN_1AF4&DEV_1052&SUBSYS_00101AF4&REV_01` *(keyboard)*, or
      - `PCI\VEN_1AF4&DEV_1052&SUBSYS_00111AF4&REV_01` *(mouse)*
    - Windows will also show less-specific variants such as `PCI\VEN_1AF4&DEV_1052&REV_01`.
      - Note: the canonical in-tree keyboard/mouse INF (`aero_virtio_input.inf`) is intentionally **SUBSYS-gated** (no
        generic fallback). If your device does not expose the Aero subsystem IDs, Windows will not bind it.
      - In that case, enable/install the optional legacy filename alias INF (`virtio-input.inf.disabled` → `virtio-input.inf`),
        which adds an opt-in strict fallback match (`PCI\VEN_1AF4&DEV_1052&REV_01`). When binding via the fallback entry,
        Device Manager will show **Aero VirtIO Input Device**.
        The alias INF is allowed to differ in the models sections but should otherwise stay in sync with `aero_virtio_input.inf`
