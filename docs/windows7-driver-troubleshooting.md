...
5. Verify the virtio-input PCI device matches the Aero Win7 virtio contract v1 identity:
    ...
    - Note: the canonical in-tree keyboard/mouse INF (`aero_virtio_input.inf`) is intentionally **SUBSYS-gated**
      (`SUBSYS_0010`/`SUBSYS_0011`) and does **not** include a generic (no `SUBSYS`) fallback HWID.
    - If your device does not expose/recognize the Aero subsystem IDs, enable the optional legacy filename alias INF
      (`virtio-input.inf.disabled` → `virtio-input.inf`) to provide an opt-in strict revision-gated generic fallback match:
      - `PCI\VEN_1AF4&DEV_1052&REV_01`
      When binding via the fallback entry, Device Manager will show **Aero VirtIO Input Device**.
      - The legacy alias INF is allowed to differ from `aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` /
        `[Aero.NTamd64]`) to provide the fallback entry above; outside those models sections it is expected to stay in sync
