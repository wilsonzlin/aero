...
5. Verify the virtio-input PCI device matches the Aero Win7 virtio contract v1 identity:
    ...
    - Note: the canonical in-tree virtio-input INF (`aero_virtio_input.inf`) includes both:
      ...
      If your virtio-input PCI device does **not** expose Aero subsystem IDs, Windows can still bind via the generic fallback
      entry (Device Manager name: **Aero VirtIO Input Device**). If you expect distinct keyboard/mouse names, ensure the
    ...
    - Optional legacy filename alias (if you need the old basename): `virtio-input.inf.disabled` → rename to `virtio-input.inf`
      ...
      - Does **not** change HWID matching behavior, and enabling the alias is **not** required for generic fallback binding (the
