...
- **AeroGPU (canonical / long-term):** `MachineConfig::enable_aerogpu=true` exposes the canonical
  ...
  integration in `crates/emulator` (see: [`21-emulator-crate-migration.md`](./21-emulator-crate-migration.md)).
...
VRAM offset    Purpose
0x00000..0x3FFFF  Legacy VGA planes backing text/planar/mode13h (4 × 64KiB = 256KiB)
...
Note: legacy VGA hardware exposes a 128KiB CPU-visible window, but uses a 4 × 64KiB (256KiB)
*planar* backing store internally. `aero_machine` reserves the full `VRAM[0x00000..0x3FFFF]` region
so future plane-accurate behavior can be implemented without moving the VBE LFB base.
