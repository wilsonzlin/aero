...
`PCI\VEN_1AF4&DEV_1059&REV_01`, so if Windows shows `DEV_1018` you must configure the hypervisor to expose a
modern-only device (for example QEMU `disable-legacy=on`).
