...
- **AeroGPU MVP behavior:**
  ...
  - The UMD generates `alloc_id` (u32) at shared-resource creation time and
    ...
    identical `alloc_id`.
  - For D3D9Ex shared surfaces, the KMD generates a collision-resistant
    `share_token` (ShareToken) and returns it to the UMD via allocation private
    driver data (`drivers/aerogpu/protocol/aerogpu_alloc_privdata.h`). The UMD
    uses this token as the protocol `share_token` for
