...
- `win7-shared-surfaces-share-token.md` — D3D9Ex/DWM shared surface ShareToken strategy (KMD-owned token vs process-local user-mode `HANDLE` values).
...
- `win7-d3d10-11-umd-allocations.md` — Win7 WDK 7.1 exact UMD allocation callback contract (`pfnAllocateCb`/`pfnDeallocateCb`) and CreateResource allocation flow for D3D10/D3D11.
