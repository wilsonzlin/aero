...
## AeroGPU contract: `share_token` is a stable token persisted in allocation private driver data
...
2. The UMD generates `share_token` and writes it into the preserved allocation private
   driver data blob (`aerogpu_wddm_alloc_priv.share_token`).
...
2. Consumer opens the resource; dxgkrnl returns the preserved allocation private driver
   data bytes so the UMD can recover the same `share_token`.
