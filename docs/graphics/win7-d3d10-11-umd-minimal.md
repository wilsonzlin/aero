...
* Geometry shader object creation:
  ...
  * AeroGPU note: WebGPU has no geometry shader stage. The command stream can encode a GS handle (via `aerogpu_cmd_bind_shaders::reserved0`), but the current AeroGPU/WebGPU backend ignores it. For Win7 bring-up we currently accept GS creation/binding but treat it as a **no-op** in the translation path (sufficient for pass-through GS usage such as `d3d11_geometry_shader_smoke`). Longer-term, Aero’s plan is to emulate GS via compute expansion; see [`geometry-shader-emulation.md`](./geometry-shader-emulation.md) and [`docs/16-d3d10-11-translation.md`](../16-d3d10-11-translation.md).
...
* If you advertise **FL10_0** (or higher) from `pfnGetCaps`, implement `pfnCreateGeometryShader` / `D3D11DDIARG_CREATEGEOMETRYSHADER` (and the corresponding bind/state entrypoints) even if the first implementation is “limited but functional”.
  * AeroGPU bring-up: accept GS creation/binding but treat it as a no-op (GS is representable in the command stream, but ignored by the WebGPU backend). Full support requires emulation; see [`geometry-shader-emulation.md`](./geometry-shader-emulation.md).
