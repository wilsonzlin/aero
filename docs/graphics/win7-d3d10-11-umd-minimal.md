...
* Geometry shader object creation:
  ...
  * AeroGPU note: WebGPU has no geometry shader stage. The command stream can encode a GS handle (via `aerogpu_cmd_bind_shaders::reserved0`).
    The current AeroGPU/WebGPU backend accepts the plumbing (`stage_ex` + compute-prepass), but GS DXBC is still treated as a **no-op** today (the compute prepass is placeholder and does not execute guest GS bytecode yet).
...
* If you advertise **FL10_0** (or higher) from `pfnGetCaps`, implement `pfnCreateGeometryShader` / `D3D11DDIARG_CREATEGEOMETRYSHADER` (and the corresponding bind/state entrypoints) even if the first implementation is “limited but functional”.
  * AeroGPU bring-up: accept GS creation/binding, but treat it as a no-op in terms of guest DXBC execution (the WebGPU backend has `stage_ex` + compute-prepass scaffolding, but does not execute GS bytecode yet). See [`geometry-shader-emulation.md`](./geometry-shader-emulation.md).
