...
- Legacy stacks exist with different IDs/ABIs, notably:
  ...
  - Retired experimental cmd/completion-ring ABI (prototype), used for early host-side experiments and must not be used for the WDDM AeroGPU device.
...
- **Legacy IDs/ABIs are deprecated and must be explicitly labeled + gated.**
  ...
  - Retired experimental cmd/completion-ring ABI (prototype): treated as an internal/experimental ABI; must not be presented as the AeroGPU WDDM device.
