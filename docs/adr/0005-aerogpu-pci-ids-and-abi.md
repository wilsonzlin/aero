...
- Legacy stacks exist with different IDs/ABIs, notably:
  ...
  - Retired `aero-gpu-device` cmd/completion-ring ABI (prototype), used for early host-side experiments and must not be used for the WDDM AeroGPU device.
...
1. **Keep the retired `aero-gpu-device` cmd/completion-ring ABI as “the” AeroGPU ABI**
...
- **Legacy IDs/ABIs are deprecated and must be explicitly labeled + gated.**
  ...
  - Retired `aero-gpu-device` cmd/completion-ring ABI (prototype): treated as an internal/experimental ABI; must not be presented as the AeroGPU WDDM device.
...
- **Migration / removal timeline (project policy):**
  ...
  2. After **one release cycle** with A3A0 as default: remove the retired `aero-gpu-device` cmd/completion-ring prototype ABI and 1AE0 from any default configs and docs; keep only in clearly archived prototype locations.
