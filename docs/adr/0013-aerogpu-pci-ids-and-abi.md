...
- `docs/windows-device-contract.{md,json}` currently documents AeroGPU as **A0E0**, historically tied to the `crates/aero-gpu-device` command/completion ring ABI.
- Legacy stacks exist with different IDs/ABIs, notably:
  ...
  - **A0E0**: obsolete `aero-gpu-device` ABI used for early experiments.
...
1. **Keep the A0E0 cmd/completion ring ABI (`crates/aero-gpu-device`) as “the” AeroGPU ABI**
...
- **Legacy IDs/ABIs are deprecated and must be explicitly labeled + gated.**
  ...
  - **A0E0** (`aero-gpu-device` ABI): treated as an internal/experimental ABI; must not be presented as the AeroGPU WDDM device.
