...
1. **Keep the retired `aero-gpu-device` cmd/completion-ring ABI (formerly `crates/aero-gpu-device`) as “the” AeroGPU ABI**
...
- **Legacy IDs/ABIs are deprecated and must be explicitly labeled + gated.**
  ...
  - Retired experimental cmd/completion-ring ABI (`aero-gpu-device`, formerly `crates/aero-gpu-device`): treated as an internal/experimental ABI; must not be presented as the AeroGPU WDDM device.
