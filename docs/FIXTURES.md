...
- `assets/bios.bin` (generated fixture; canonical ROM comes from `firmware::bios::build_bios_rom()`; regenerate via `cargo xtask fixtures` (or `cargo run -p firmware --bin gen_bios_rom --locked`))
