...
- `assets/bios.bin` (generated fixture; canonical ROM comes from `firmware::bios::build_bios_rom()`; see [`assets/README.md`](../assets/README.md); regenerate via `cargo xtask fixtures` (or `cargo run -p firmware --bin gen_bios_rom --locked`))
- `crates/firmware/acpi/dsdt.aml` (ACPI DSDT AML built from this repo; regenerate via `cargo xtask fixtures` (or `cargo run -p firmware --bin gen_dsdt --locked`))
- `tests/fixtures/boot/*.bin` and `tests/fixtures/boot/*.img` (boot sector + tiny disk images generated by `cargo xtask fixtures`)
- `tests/fixtures/bootsector.bin` and `tests/fixtures/realmode_vbe_test.bin` (boot/test programs; sources live alongside)
- `test_images/boot_sectors/int_sanity.bin` (boot sector built from in-repo `.asm` source)
- `tools/qemu_diff/boot/boot.bin` (boot sector built from `boot.S`; used by the `qemu-diff` Cargo package)
