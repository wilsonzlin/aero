...
- `config/devices.cmd` is generated by `aero_packager` from a Windows device contract JSON
  ...
  For Aero builds, ensure the contract’s `driver_service_name` for `virtio-blk` matches the packaged driver’s INF
  `AddService` name so `setup.cmd` boot-critical pre-seeding aligns with the driver packages that are shipped.
  For virtio-win builds, use `docs/windows-device-contract-virtio-win.json` so the packaged `devices.cmd` matches
  virtio-win’s `AddService` names (e.g. `viostor`).
