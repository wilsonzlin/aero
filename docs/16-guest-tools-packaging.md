...
|---|---|---|---|---|
| `tools/packaging/specs/win7-signed.json` | CI/release workflows (packaging from `out/packages` + `out/certs`) | `aerogpu`, `virtio-blk`, `virtio-net`, `virtio-input` | `virtio-snd` | `expected_hardware_ids` intentionally empty |
...
- The wrapper drives inclusion/validation via a packager spec (`-SpecPath`):
  ...
  - CI/release workflows: pass `tools/packaging/specs/win7-signed.json` (HWID regex lists intentionally empty).
- `config/devices.cmd` is generated by `aero_packager` from `docs/windows-device-contract.json`.
  For Aero builds, ensure the contract’s `driver_service_name` for `virtio-blk` matches the packaged
  driver’s INF `AddService` name so `setup.cmd` boot-critical pre-seeding aligns with the driver
  packages that are shipped. For virtio-win builds, `aero_packager` overrides service names in the
  packaged `devices.cmd` to match virtio-win’s `AddService` names (e.g. `viostor`).
