...
- `MachineConfig::enable_aerogpu=true`: expose **AeroGPU** at `00:07.0` (`A3A0:0001`). This is the
  canonical Windows driver binding target (`PCI\VEN_A3A0&DEV_0001`) and also owns the VGA/VBE boot
  display compatibility path.
...
- With `MachineConfig::enable_aerogpu=true`, the boot display is provided by **AeroGPU itself**
  (`00:07.0`) via its VGA/VBE compatibility path (see
  [`16-aerogpu-vga-vesa-compat.md`](./16-aerogpu-vga-vesa-compat.md)).
  - In this mode, firmware derives the VBE mode-info `PhysBasePtr` from AeroGPU BAR1:
