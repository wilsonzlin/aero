...
> - It does **not** include the strict, REV-gated generic fallback model line (no `SUBSYS`): `PCI\VEN_1AF4&DEV_1052&REV_01`.
>
> Tablet devices bind via the separate `inf/aero_virtio_tablet.inf` (`SUBSYS_00121AF4`); that INF is more specific and wins
> over the generic fallback match (if enabled) when both packages are present. If the tablet INF is not installed, the generic
> fallback entry (if enabled) can also bind to tablet devices (but will use the generic device name).
>
...
>
> Policy: enabling the alias is an **opt-in** way to enable strict generic fallback binding:
> - The alias adds the strict, REV-gated generic fallback HWID (no `SUBSYS`): `PCI\VEN_1AF4&DEV_1052&REV_01`
>   (shown as **Aero VirtIO Input Device**).
> - From `[Version]` onward, the alias INF is expected to match `aero_virtio_input.inf` **excluding** the models sections
>   (`[Aero.NTx86]` / `[Aero.NTamd64]`); banner/comments above `[Version]` may differ (see
>   `drivers/windows7/virtio-input/scripts/check-inf-alias.py`).
> - Because it adds the fallback entry, enabling the alias **does** change HWID matching behavior.
>
