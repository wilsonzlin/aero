...
|---|---|---|---|---|
| `crates/aero-wasm` | `Machine` | `aero_machine::Machine` (`crates/aero-machine`) | `web/src/workers/machine_cpu.worker.ts` (`vmRuntime=machine`)<br>`web/src/main.ts` (demo) | **Canonical full-system machine**. Owns PCI/IO/MMIO/device wiring in Rust/WASM and supports attaching `NET_TX`/`NET_RX` rings for networking. This is the canonical web runtime when `vmRuntime=machine`. |
