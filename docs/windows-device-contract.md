...
- Boot display is provided by `aero_gpu_vga` (VGA + Bochs VBE) when `MachineConfig::enable_vga=true`.
  ...
  - When `enable_pc_platform=true`, the machine exposes a minimal Bochs/QEMU-compatible “Standard VGA”
    PCI function (currently `00:0c.0`, `1234:1111`) so the LFB can be reached via PCI BAR0 inside the PCI MMIO
...
; Canonical keyboard/mouse INF includes:
; - subsystem-qualified keyboard/mouse HWIDs for distinct Device Manager naming, and
...
; - Enabling the alias does not change HWID matching behavior.
...
; Note: this SUBSYS-qualified HWID is more specific, so it wins over the generic fallback (`PCI\VEN_1AF4&DEV_1052&REV_01`)
; when both driver packages are installed.
