...
- `wasm-pack`
- Binaryen (`wasm-opt`)
- QEMU + boot-test deps: `qemu-system-x86`, `mtools`, `unzip`, `nasm` (nasm is optional; boot fixtures are generated by `cargo xtask fixtures`, so CI does not require an assembler)
