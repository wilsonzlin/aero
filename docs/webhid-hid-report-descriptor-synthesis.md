...
(A legacy native emulator harness also has an older copy in `crates/emulator` (`emulator::io::usb`),
but `crates/aero-usb` is the primary reference for the browser/WASM path.)
