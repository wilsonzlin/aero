...
CI runs the `--check` form and fails if any fixture generated by `cargo xtask fixtures` is missing or out-of-date.

`cargo xtask test-all` runs the same fixture checks automatically (unless you pass `--skip-rust`).
