...
fn input_invokes_expected_web_unit_test_subset() {
    ...
    let _node_modules_lock =
        acquire_node_modules_lock(&repo_root).expect("acquire node_modules lock");
    let _node_modules_guard =
        ensure_node_modules_dir(&repo_root).expect("ensure node_modules dir");
    ...
    let npm_unit = invocations
        .iter()
        .find(|argv| argv.first().map(|s| s.as_str()) == Some("npm") && argv.contains(&"test:unit".to_string()))
