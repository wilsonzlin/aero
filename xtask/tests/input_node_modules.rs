...
const EXPECTED_WEB_UNIT_TEST_PATHS: &[&str] = &[
    ...
    "src/io/devices/virtio_input_compat.test.ts",
    "src/io/devices/virtio_input_keymap.test.ts",
...
/// Verify `cargo xtask input --node-dir web` fails with a helpful hint when `node_modules` is
/// missing, and preserves the node-dir flag in the suggested `--rust-only` escape hatch.
#[test]
#[cfg(unix)]
fn input_reports_missing_node_modules_with_node_dir_rust_only_hint() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    Command::new(env!("CARGO_BIN_EXE_xtask"))
        ...
        .stderr(predicate::str::contains(
            "cargo xtask input --node-dir web --rust-only",
        ));
...
/// Verify the missing-Node hint preserves `--node-dir` so developers can keep their preferred Node
/// workspace layout even when falling back to rust-only mode.
#[test]
#[cfg(unix)]
fn input_reports_missing_node_with_node_dir_rust_only_hint() {
    let tmp = tempfile::tempdir().expect("create tempdir");
    let bin_dir = tmp.path().join("bin");
    fs::create_dir(&bin_dir).expect("create bin dir");
    let log_path = tmp.path().join("argv.log");
...
/// Verify `cargo xtask input --node-dir web` suggests `--rust-only` when npm is missing (but Node is
/// present), and preserves the node-dir flag in the suggested escape hatch.
#[test]
#[cfg(unix)]
fn input_reports_missing_npm_with_node_dir_rust_only_hint() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    assert!(
        invocations
            .iter()
            .any(|argv| argv.first().map(|s| s.as_str()) == Some("node")),
        "expected node version check to run before reporting missing npm; invocations={invocations:?}"
    );
    assert!(
        !invocations
            .iter()
            .any(|argv| argv.first().map(|s| s.as_str()) == Some("npm")),
        "expected npm not to be invoked when it is missing; invocations={invocations:?}"
    );
}

/// Verify `cargo xtask input --node-dir <path>` fails with a helpful message when the override does
/// not contain a `package.json`.
#[test]
#[cfg(unix)]
fn input_reports_invalid_node_dir_override() {
    let tmp = tempfile::tempdir().expect("create tempdir");
    let bin_dir = tmp.path().join("bin");
    fs::create_dir(&bin_dir).expect("create bin dir");
    let log_path = tmp.path().join("argv.log");

    let invalid_node_dir = tmp.path().join("invalid-node-dir");
    fs::create_dir(&invalid_node_dir).expect("create invalid node dir");
    ...
    // Avoid inheriting the real PATH: all invoked tools should be stubbed.
    let path = bin_dir.display().to_string();
    let invalid_node_dir_str = invalid_node_dir.to_string_lossy().to_string();

    Command::new(env!("CARGO_BIN_EXE_xtask"))
        .args(["input", "--node-dir", &invalid_node_dir_str])
        .env_remove("AERO_NODE_DIR")
        .env_remove("AERO_WEB_DIR")
        .env_remove("WEB_DIR")
        .env("AERO_XTASK_TEST_LOG", &log_path)
        ...
        .code(1)
        .stderr(predicate::str::contains(
            "package.json not found in node dir override",
        ))
        .stderr(predicate::str::contains(&invalid_node_dir_str));
    ...
    assert!(
        !invocations
            .iter()
            .any(|argv| argv.first().map(|s| s.as_str()) == Some("npm")),
        "expected npm not to be invoked when node-dir override is invalid; invocations={invocations:?}"
    );
}
...
/// Verify `AERO_NODE_DIR=.` behaves like the default repo-root mode (i.e. uses `npm -w web ...` from
/// the repo root).
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_aero_node_dir_is_dot() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
...
/// Verify `AERO_WEB_DIR=web` switches the npm unit test step to run directly inside the `web/`
/// package (rather than using `npm -w web` from the repo root).
///
/// `AERO_WEB_DIR` is a deprecated alias for `AERO_NODE_DIR`, but `cargo xtask input` still supports
/// it for compatibility.
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_aero_web_dir_is_web() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    assert!(
        !npm_unit.iter().any(|arg| arg == "-w"),
        "expected AERO_WEB_DIR=web to avoid `npm -w web`; argv={npm_unit:?}"
    );
    assert!(
        !npm_unit.iter().any(|arg| arg == "web"),
        "expected AERO_WEB_DIR=web to avoid a literal `web` workspace arg; argv={npm_unit:?}"
    );
...
/// Verify `WEB_DIR=web` (deprecated alias for `AERO_NODE_DIR`) switches the npm unit test step to
/// run directly inside the `web/` package.
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_web_dir_is_web() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    assert!(
        !npm_unit.iter().any(|arg| arg == "-w"),
        "expected WEB_DIR=web to avoid `npm -w web`; argv={npm_unit:?}"
    );
    assert!(
        !npm_unit.iter().any(|arg| arg == "web"),
        "expected WEB_DIR=web to avoid a literal `web` workspace arg; argv={npm_unit:?}"
    );
...
/// Verify `WEB_DIR=.` behaves like the default repo-root mode (i.e. uses `npm -w web ...` from the
/// repo root).
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_web_dir_is_dot() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
...
/// Verify `--node-dir web` switches the npm unit test step to run directly inside the `web/`
/// package (rather than using `npm -w web` from the repo root).
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_node_dir_flag_is_web() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    assert!(
        !npm_unit.iter().any(|arg| arg == "-w"),
        "expected --node-dir web to avoid `npm -w web`; argv={npm_unit:?}"
    );
    assert!(
        !npm_unit.iter().any(|arg| arg == "web"),
        "expected --node-dir web to avoid a literal `web` workspace arg; argv={npm_unit:?}"
    );
...
/// Verify `--node-dir .` behaves like the default repo-root mode (i.e. uses `npm -w web ...` from
/// the repo root rather than attempting to run `npm run test:unit` from `./`).
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_node_dir_flag_is_dot() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
...
/// Verify `--web-dir web` switches the npm unit test step to run directly inside the `web/`
/// package (rather than using `npm -w web` from the repo root).
#[test]
#[cfg(unix)]
fn input_invokes_expected_web_unit_test_subset_when_web_dir_flag_is_web() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask/CARGO_MANIFEST_DIR should have a parent")
        .to_path_buf();
    ...
    assert!(
        !npm_unit.iter().any(|arg| arg == "-w"),
        "expected --web-dir web to avoid `npm -w web`; argv={npm_unit:?}"
    );
    assert!(
        !npm_unit.iter().any(|arg| arg == "web"),
        "expected --web-dir web to avoid a literal `web` workspace arg; argv={npm_unit:?}"
    );
