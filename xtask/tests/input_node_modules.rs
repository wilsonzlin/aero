...
fn input_reports_invalid_node_dir_override() {
    ...
    assert!(
        ...
        "expected cargo to run before validating node-dir override; invocations={invocations:?}"
    );
    assert!(
        invocations
            .iter()
            .any(|argv| argv.first().map(|s| s.as_str()) == Some("node")),
        "expected node version check to run before validating node-dir override; invocations={invocations:?}"
    );
    assert!(
        !invocations
            .iter()
