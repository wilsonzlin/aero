...
fn pin_power_state_round_trips() {
    ...
    // AFG (nid=1): set to D1 and read back (packed current+target).
    assert_eq!(hda.codec_mut().execute_verb(1, verb_12(0x705, 0x01)), 0);
    assert_eq!(hda.codec_mut().execute_verb(1, verb_12(0xF05, 0)), 0x11);

    // Output pin (nid=3): set to D3 and read back.
    assert_eq!(hda.codec_mut().execute_verb(3, verb_12(0x705, 0x03)), 0);
    assert_eq!(hda.codec_mut().execute_verb(3, verb_12(0xF05, 0)), 0x33);

    // Mic pin (nid=5): set to D2 and read back.
    assert_eq!(hda.codec_mut().execute_verb(5, verb_12(0x705, 0x02)), 0);
    assert_eq!(hda.codec_mut().execute_verb(5, verb_12(0xF05, 0)), 0x22);
}
