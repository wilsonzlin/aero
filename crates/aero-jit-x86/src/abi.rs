...
//!
//! Exposes byte offsets into the canonical CPU state struct stored in linear memory.
//! Offsets are `u32` because WebAssembly encodes memory immediates as 32-bit.
//!
//! Tier-1 blocks may additionally use a JIT-only context region stored separately from the CPU
//! state. See [`crate::jit_ctx::JitContext`] for its layout.
//!
//! If the `aero_cpu_core` CPU state layout ever changes, the unit tests in
//! `crates/aero-jit-x86/tests/abi.rs` are expected to fail loudly so that JIT codegen can be
//! updated in lockstep.
