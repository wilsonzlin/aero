...
#[test]
fn tier2_cfg_builder_masks_16bit_entry_rip_for_block_keys() {
    // jmp -2 (to self)
    let code = [
        0xeb, 0xfe, // jmp 0x0000
    ];

    let mut bus = SimpleBus::new(0x1000);
    bus.load(0, &code);

    // 16-bit guests architecturally mask IP to 16 bits; an entry RIP with high bits set should
    // still build a CFG rooted at address 0 without creating duplicate blocks.
    let func = build_function_from_x86(&bus, 0x1_0000, 16, CfgBuildConfig::default());
    ...
    assert_eq!(
        func.blocks.iter().filter(|b| b.start_rip == 0).count(),
        1,
        "unexpected duplicate blocks for masked 16-bit RIP"
    );
}
