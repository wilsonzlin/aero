...
#[test]
fn tier2_shr8_count_eq_operand_width_sets_cf_from_old_msb() {
    // mov al, 0x80
    // shr al, 8         ; result=0, CF=old MSB (1)
    // jc taken          ; must take
    // mov al, 0
    // <invalid>
    // taken: mov al, 1
...
#[test]
fn tier2_shr16_count_eq_operand_width_sets_cf_from_old_msb() {
    // 66 mov ax, 0x8000
    // 66 shr ax, 16     ; result=0, CF=old MSB (1)
    // jc taken          ; must take
    // mov al, 0
    // <invalid>
    // taken: mov al, 1
...
#[test]
fn tier2_shift_count_gt_operand_width_leaves_cf_unchanged_16bit() {
    // 66 mov ax, 1
    // 66 shl ax, 17
    // jc +3
    // mov al, 0
    ...
    // <invalid>
    //
    // For 16-bit shifts, x86 masks the count to 5 bits, so 17 is *not* reduced to 1.
...
fn tier2_sar_count_1_sets_of_to_0() {
    ...
    // Branch should not be taken; we should stop at the first <invalid>.
