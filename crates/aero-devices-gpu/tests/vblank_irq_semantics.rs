...
fn vblank_irq_status_not_latched_while_masked_or_on_reenable() {
    ...
    let t0 = Instant::now();
    dev.mmio_write_dword(&mut mem, t0, false, mmio::SCANOUT0_ENABLE, 1);
    ...
    dev.tick(&mut mem, t0, false);

    let enable_time = t0 + period * 3;
