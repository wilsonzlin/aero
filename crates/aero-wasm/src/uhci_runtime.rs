...
const DEFAULT_EXTERNAL_HUB_PORT_COUNT: u8 = aero_machine::Machine::UHCI_EXTERNAL_HUB_PORT_COUNT;
...
impl UhciRuntime {
    ...
    pub fn load_state(&mut self, bytes: &[u8]) -> Result<(), JsValue> {
        ...
        let passthrough_snapshot_states: HashMap<u8, Vec<u8>> = if let Some(buf) =
            r.bytes(TAG_USB_HID_PASSTHROUGH_DEVICES)
        {
            ...
            let max_ports = hub_port_count.expect("checked Some above") as usize;
            ...
            // Snapshot is untrusted; cap work to the number of hub ports so a corrupted snapshot
            // cannot encode millions of empty records and force pathological decode loops.
            if count > max_ports {
                return Err(js_error(&format!(
                    "UHCI runtime snapshot has too many passthrough HID devices ({count}, max {max_ports})"
                )));
            }
            for idx in 0..count {
                ...
                if hub_port as usize > max_ports {
                    return Err(js_error(&format!(
                        "Invalid UHCI runtime snapshot passthrough HID record #{idx}: hub port {hub_port} exceeds hub port count {max_ports}",
                    )));
                }
        ...
        // nested device-model snapshots for every attached device, including WebUSB passthrough
        // models. Those nested snapshots may reintroduce host-side bookkeeping (queued actions +
        // in-flight maps) that cannot be resumed after a VM snapshot restore.
        //
        // Clear any restored host-side asynchronous state *after* the controller snapshot has been
        // applied so guest TD retries will re-emit fresh host actions instead of deadlocking on a
