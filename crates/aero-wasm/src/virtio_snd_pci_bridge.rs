...
impl OptionalWorkletSink {
    fn attach(&mut self, ring: SharedArrayBuffer, capacity_frames: u32, channel_count: u32) -> Result<(), JsValue> {
        if capacity_frames == 0 {
            return Err(js_error("capacity_frames must be non-zero"));
        }
        if channel_count != 2 {
            return Err(js_error("channel_count must be 2 for virtio-snd playback (stereo)"));
        }
...
impl VirtioSndPciBridge {
    ...
    pub fn set_mic_ring_buffer(&mut self, ring_sab: Option<SharedArrayBuffer>) -> Result<(), JsValue> {
        match ring_sab {
