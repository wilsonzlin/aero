...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_next_descriptor_is_unreadable() {
    ...
    assert!(qsz >= 3, "expected event queue size >= 3");

    // Place the descriptor table at the end of guest RAM so descriptor index 2 is within range,
    // but unreadable (GuestMemory out-of-bounds) when the queue parser follows the NEXT pointer.
    let desc_table = guest_size - 32;
    let avail = 0x2000u32;
    let used = 0x3000u32;
    ...
    // Descriptor 0: NEXT points to descriptor index 2, which is within qsz but whose descriptor
    // table entry is out-of-bounds due to `desc_table` placement.
    write_desc(
        &guest,
        desc_table,
        0,
