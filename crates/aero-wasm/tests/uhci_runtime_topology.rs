...
fn uhci_runtime_supports_multiple_webhid_devices_behind_hub() {
    ...
    let path1 = serde_wasm_bindgen::to_value(&vec![0u32, 1u32]).expect("path1");
    rt.webhid_attach_at_path(
        1,
        0x1234,
        0x0001,
    ...
    let path2 = serde_wasm_bindgen::to_value(&vec![0u32, 2u32]).expect("path2");
    rt.webhid_attach_at_path(
        2,
        0x1234,
        0x0002,
...
fn uhci_runtime_resizes_external_hub_and_reattaches_devices() {
    ...
    let p1 = serde_wasm_bindgen::to_value(&vec![0u32, 1u32]).expect("p1");
    rt.webhid_attach_at_path(
        10,
        0x1234,
        0x0001,
    ...
    let p2 = serde_wasm_bindgen::to_value(&vec![0u32, 2u32]).expect("p2");
    rt.webhid_attach_at_path(
        11,
        0x1234,
        0x0002,
