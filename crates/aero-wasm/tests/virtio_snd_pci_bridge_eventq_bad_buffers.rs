...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_chain_buffer_is_too_small() {
    ...
    // First descriptor is too small for an 8-byte event; second is valid.
    let buf_small = 0x4000u32;
    let buf = 0x4100u32;
    guest.fill(buf_small, 4, 0xAA);
    ...
    guest.write_u16(avail + 4, 0); // ring[0] = desc 0 (too small)
    guest.write_u16(avail + 6, 1); // ring[1] = desc 1 (valid)
    ...
    // The short buffer chain should be completed with len=0 and the event should still be
    ...
    // The short buffer should not be modified.
    let mut got_small = [0u8; 4];
    guest.read_into(buf_small, &mut got_small);
    assert_eq!(&got_small, &[0xAAu8; 4]);
}

#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_chain_buffer_is_out_of_bounds() {
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
        size: guest_size,
    ...
    // Descriptor 0 points just past the end of guest RAM: out-of-bounds for len=8.
    write_desc(
        &guest,
        desc_table,
        0,
    ...
    guest.write_u16(avail, 0);
    guest.write_u16(avail + 2, 2); // avail.idx = 2
    guest.write_u16(avail + 4, 0); // ring[0] = desc 0 (OOB)
    guest.write_u16(avail + 6, 1); // ring[1] = desc 1 (valid)
    guest.write_u16(used, 0);
    guest.write_u16(used + 2, 0);
