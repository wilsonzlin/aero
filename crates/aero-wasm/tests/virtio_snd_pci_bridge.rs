...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_is_gated_on_pci_bus_master_enable() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Post a single 8-byte writable event buffer.
    ...
    // Queue a speaker JACK_CONNECTED event while BME is still disabled.
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_legacy_only_delivers_speaker_jack_event_into_cached_buffer_on_poll() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Post a single event buffer and kick eventq while no events are queued. This should cache the
    ...
    // Queue a JACK_CONNECTED event and poll: should deliver into the cached buffer.
    let ring = WorkletBridge::new(8, 2).unwrap();
    let sab = ring.shared_buffer();
    bridge
        .set_audio_ring_buffer(Some(sab), 8, 2)
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_transitional_legacy_io_eventq_is_gated_on_pci_bus_master_enable() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_transitional_allows_legacy_reset_to_unlock_modern_transport_mode() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    ...
    assert!(
        bridge.driver_ok(),
        "expected legacy STATUS write to take effect after reset"
    );
}

#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_transitional_allows_modern_reset_to_unlock_legacy_transport_mode() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    ...
    assert!(
        bridge.driver_ok(),
        "expected modern STATUS write to take effect after reset"
    );
}
...
fn virtio_snd_pci_bridge_delivers_speaker_jack_events_one_buffer_at_a_time_after_events_are_queued()
{
    ...
    // Write two descriptors, but only publish one in the avail ring initially.
    guest.fill(buf0, 8, 0xAA);
    guest.fill(buf1, 8, 0xBB);
    write_desc(&guest, desc_table, 0, buf0 as u64, 8, VIRTQ_DESC_F_WRITE, 0);
    write_desc(&guest, desc_table, 1, buf1 as u64, 8, VIRTQ_DESC_F_WRITE, 0);
    guest.write_u16(avail, 0);
    guest.write_u16(avail + 2, 1); // avail.idx = 1
    guest.write_u16(avail + 4, 0); // avail.ring[0] = desc 0
    guest.write_u16(used, 0);
    guest.write_u16(used + 2, 0);
    ...
    assert_eq!(
        &got1, &[0xBBu8; 8],
        "second buffer should remain untouched until the second event buffer is posted"
    );

    // Publish the second buffer and notify again; the second JACK event should be delivered.
    guest.write_u16(avail + 6, 1); // avail.ring[1] = desc 1
    guest.write_u16(avail + 2, 2); // avail.idx = 2
    bridge.mmio_write(
        NOTIFY + notify_off * 4,
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_delivers_speaker_jack_event_into_indirect_eventq_descriptor_chain() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Expose the event buffer via an indirect descriptor table.
    guest.fill(buf, 8, 0xAA);
    write_desc(
        &guest,
        indirect_table,
    ...
    // Queue a speaker JACK_CONNECTED event.
    let ring = WorkletBridge::new(8, 2).unwrap();
    let sab = ring.shared_buffer();
    bridge
        .set_audio_ring_buffer(Some(sab), 8, 2)
...
fn virtio_snd_pci_bridge_eventq_writes_event_across_multiple_descriptors_without_overwriting_extra_bytes(
    ...
    // Publish a writable event buffer chain split across 2 descriptors:
    // - first: 4 bytes (event type)
    // - second: 12 bytes (event data + extra bytes that must remain untouched)
    guest.fill(buf1, 4, 0xAA);
    guest.fill(buf2, 12, 0xBB);
    write_desc(
        &guest,
        desc_table,
        0,
    ...
    // Queue a microphone JACK_CONNECTED event.
    let capacity_samples = 16u32;
    let mic_sab = SharedArrayBuffer::new(
    ...
    let mut got_type = [0u8; 4];
    guest.read_into(buf1, &mut got_type);
    assert_eq!(&got_type, &VIRTIO_SND_EVT_JACK_CONNECTED.to_le_bytes());

    let mut got_buf2 = [0u8; 12];
    guest.read_into(buf2, &mut got_buf2);
    assert_eq!(&got_buf2[0..4], &JACK_ID_MICROPHONE.to_le_bytes());
    assert_eq!(&got_buf2[4..12], &[0xBBu8; 8]);
}
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_buffer_is_out_of_bounds() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    let avail = 0x2000u32;
    let used = 0x3000u32;
    let ok_buf = 0x4000u32;
    let oob_buf = guest_size + 0x100;
    ...
    // The first (out-of-bounds) buffer should be completed with len=0, and the event should be
    // retained and delivered into the second buffer.
    assert_eq!(guest.read_u16(used + 2), 2);
    assert_eq!(guest.read_u32(used + 4), 0);
    assert_eq!(guest.read_u32(used + 8), 0);
    assert_eq!(guest.read_u32(used + 12), 1);
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_buffer_is_not_write_only() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Post one malformed buffer (not write-only) and then a valid writable buffer.
    ...
    // The malformed chain should be completed with len=0, and the event should be retained and
    // delivered into the valid chain.
    assert_eq!(guest.read_u16(used + 2), 2);
    assert_eq!(guest.read_u32(used + 4), 0);
    assert_eq!(guest.read_u32(used + 8), 0);
    assert_eq!(guest.read_u32(used + 12), 1);
    ...
    let mut bad_before = [0u8; 8];
    guest.read_into(bad_buf, &mut bad_before);
    assert_eq!(
        &bad_before, &[0xAAu8; 8],
        "malformed buffer should not be modified"
    );
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_delivers_cached_microphone_jack_events_across_multiple_polls() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Queue the first event (mic attach) and poll: should consume only the first cached buffer.
    let capacity_samples = 16u32;
    let byte_len =
        (mic_ring::HEADER_BYTES + capacity_samples as usize * core::mem::size_of::<f32>()) as u32;
    ...
    // Queue the second event (mic detach) and poll: should now consume the second cached buffer.
    bridge
...
fn virtio_snd_pci_bridge_delivers_multiple_microphone_jack_events_into_cached_eventq_buffers_on_poll()
 {
    ...
    // Queue two mic JACK events: CONNECTED then DISCONNECTED.
    let capacity_samples = 16u32;
    let byte_len =
        (mic_ring::HEADER_BYTES + capacity_samples as usize * core::mem::size_of::<f32>()) as u32;
    let mic_sab = SharedArrayBuffer::new(byte_len);
