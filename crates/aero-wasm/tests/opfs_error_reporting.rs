...
#[wasm_bindgen_test(async)]
async fn install_media_opfs_iso_alias_unavailable_error_includes_operation_and_hint() {
    if aero_opfs::opfs_sync_access_supported() {
        return;
    }

    let path = unique_path("opfs-error-install-media-alias", "iso");
    let mut m = aero_wasm::Machine::new(2 * 1024 * 1024).expect("Machine::new");
    let err = m
        .attach_install_media_opfs_iso(path.clone())
        .await
        .expect_err("expected OPFS-backed ISO attach to fail when OPFS sync access unavailable");
    ...
    assert!(
        msg.contains("Machine.attach_install_media_opfs_iso"),
        "expected operation name in message, got: {msg}"
    );
    assert!(
        msg.contains(&path),
