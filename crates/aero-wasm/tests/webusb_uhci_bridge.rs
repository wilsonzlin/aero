...
fn install_control_in_schedule(guest_base: u32) -> u32 {
...
fn uhci_controller_bridge_does_not_reuse_action_ids_across_disconnect_reconnect() {
    ...
    bridge.io_write(REG_FRBASEADD as u16, 4, fl_base);
    ...
    // Stop the controller so the schedule doesn't run while the port is in reset.
    ...
    assert!(
        !actions.is_empty(),
        "expected at least one queued UsbHostAction after reconnect"
    );
...
fn webusb_uhci_bridge_does_not_reuse_action_ids_across_disconnect_reconnect() {
    ...
    // Stop the controller so the schedule doesn't run while the port is in reset.
    bridge.io_write(REG_USBCMD, 2, 0);
    bridge.set_connected(false);
    bridge.set_connected(true);
