...
fn uhci_controller_bridge_snapshot_roundtrip_preserves_irq_and_registers() {
    ...
    let mut ctrl = UhciControllerBridge::new(guest_base, guest.len() as u32).unwrap();
    ...
    let qh_element = read_u32(&guest, 0x2004);
    let td_ctrl = read_u32(&guest, 0x3004);

    let snapshot = ctrl.save_state();

    let mut ctrl2 = UhciControllerBridge::new(guest_base, guest.len() as u32).unwrap();
    ...
    assert_eq!(read_u32(&guest, 0x2004), qh_element);
    assert_eq!(read_u32(&guest, 0x3004), td_ctrl);
