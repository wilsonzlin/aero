...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_chain_has_invalid_indirect_len() {
    ...
    // Post an invalid indirect descriptor (len not a multiple of 16) followed by a valid 8-byte
    // writable event buffer.
    write_desc(
        &guest,
        desc_table,
        0,
    ...
    guest.write_u16(avail + 4, 0); // avail.ring[0] = desc 0 (invalid)
    guest.write_u16(avail + 6, 1); // avail.ring[1] = desc 1 (valid)
    ...
    // The invalid chain should be completed with len=0, and the pending event should still be
    // delivered into the valid buffer chain.
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_chain_head_index_is_out_of_range() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Post one invalid avail entry whose head index is out of range, followed by a valid event
    // buffer at desc 0.
    guest.fill(buf, 8, 0xAA);
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_skips_zero_length_descriptor_with_out_of_bounds_addr() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // Buffer chain: first descriptor is zero-length but has an out-of-bounds address, followed by a
    // valid 8-byte buffer. The device should skip the zero-length descriptor without attempting to
    // DMA, so the event is still delivered.
    let oob_addr = u64::from(guest_size) + 0x1000;
    write_desc(
        &guest,
        desc_table,
        0,
...
#[wasm_bindgen_test]
fn virtio_snd_pci_bridge_eventq_retains_event_when_first_chain_is_descriptor_loop() {
    // Synthetic guest RAM region outside the wasm heap.
    let (guest_base, guest_size) = common::alloc_guest_region_bytes(0x20000);
    let guest = common::GuestRegion {
        base: guest_base,
    ...
    // First chain is a descriptor loop (desc 0 points to itself), second chain is a valid buffer.
    guest.fill(loop_buf, 8, 0xAA);
    ...
    guest.write_u16(avail, 0);
    guest.write_u16(avail + 2, 2); // avail.idx = 2
    guest.write_u16(avail + 4, 0); // avail.ring[0] = desc 0 (loop)
    guest.write_u16(avail + 6, 1); // avail.ring[1] = desc 1 (valid)
    guest.write_u16(used, 0);
    guest.write_u16(used + 2, 0);
