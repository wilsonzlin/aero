...
#[test]
fn d3d9_runtime_create_texture_rejects_exceeding_device_limits() {
    pollster::block_on(async {
        let require_webgpu = require_webgpu();
        let mut rt = match D3D9Runtime::new(RuntimeConfig::default()).await {
            Ok(rt) => rt,
        ...
        let err = rt
            ...
            .expect_err("expected create_texture to reject out-of-range dimensions");

        match err {
            RuntimeError::Validation(msg) => assert!(
                msg.contains("device limit"),
                "unexpected validation message: {msg}"
            ),
            other => panic!("unexpected error type: {other:?}"),
        }
