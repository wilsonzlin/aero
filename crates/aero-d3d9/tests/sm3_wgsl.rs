...
#[test]
fn wgsl_nonuniform_if_loop_with_dsx_avoids_invalid_control_flow() {
    // ps_3_0:
    //   dcl_texcoord0 v0
    //   def c0, 0,0,0,0
    //   defi i0, 0, 0, 1, 0
    ...
    //   end
    //
    // Regression test: when a non-uniform `if` contains a `loop` whose body uses derivatives, the
    // lowering must avoid emitting the loop itself under `if (cond) { ... }`, since that places
    // `dpdx` in non-uniform control flow.
    let tokens = vec![
        version_token(ShaderStage::Pixel, 3, 0),
        // dcl_texcoord0 v0  (usage 5 = texcoord)
        opcode_token(31, 1) | (5u32 << 16),
        ...
        0u32,
        0u32,
        // defi i0, 0, 0, 1, 0  (single-iteration loop)
        opcode_token(82, 5),
        dst_token(7, 0, 0xF),
        0u32,
        0u32,
    ...
    assert!(wgsl.contains("loop {"), "{wgsl}");
    assert!(
        !wgsl.contains("if ((v0.xxxx.x != 0.0))"),
        "expected loop body to be predicated rather than guarded by an outer if; got:\n{wgsl}"
    );
...
#[test]
fn wgsl_nonuniform_if_rep_with_dsx_avoids_invalid_control_flow() {
    // ps_3_0:
    //   dcl_texcoord0 v0
    //   defi i0, 1, 0, 0, 0
    //   if v0.x
    ...
    //   end
    //
    // Like `loop`, `rep` can contain derivatives. Ensure we don't emit `rep` under a potentially
    // non-uniform `if`.
    let tokens = vec![
        version_token(ShaderStage::Pixel, 3, 0),
        // dcl_texcoord0 v0  (usage 5 = texcoord)
        opcode_token(31, 1) | (5u32 << 16),
        dst_token(1, 0, 0xF),
        // defi i0, 1, 0, 0, 0  (repeat count = 1)
        opcode_token(82, 5),
        dst_token(7, 0, 0xF),
        1u32,
        0u32,
    ...
    assert!(
        !wgsl.contains("if ((v0.xxxx.x != 0.0))"),
        "expected rep body to be predicated rather than guarded by an outer if; got:\n{wgsl}"
    );
...
#[test]
fn sm3_translate_to_wgsl_with_options_does_not_emit_half_pixel_uniform_in_pixel_shader() {
    // ps_2_0:
    //   mov oC0, v0
    //   end
    let tokens = vec![
    ...
    assert_eq!(out.entry_point, "fs_main");
    assert!(
        !out.wgsl.contains("var<uniform> half_pixel"),
        "{}",
        out.wgsl
    );
