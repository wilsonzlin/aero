...
use aero_protocol::aerogpu::{
    aerogpu_cmd::{
        AerogpuCmdHdr as ProtocolCmdHdr, AerogpuCmdOpcode, AerogpuPrimitiveTopology,
...
use aero_protocol::aerogpu::aerogpu_cmd::AerogpuCmdStreamHeader as ProtocolCmdStreamHeader;
...
fn enc_decl_token(usage_raw: u8, usage_index: u8) -> u32 {
    (usage_raw as u32) | ((usage_index as u32) << 16)
}
...
fn assemble_vs_dcl_positiont_v0_color_v7() -> Vec<u8> {
    // vs_2_0 with explicit DCL semantics:
    //   dcl_positiont v0
    //   dcl_color v7
    //   mov oPos, v0
    //   mov oD0, v7
    ...
    words.extend(enc_inst(
        0x001F,
        &[enc_decl_token(9, 0), enc_dst(1, 0, 0xF)],
    ));
    words.extend(enc_inst(
        0x001F,
...
#[test]
fn d3d9_cmd_stream_dcl_semantics_map_to_canonical_locations() {
    ...
    for (x, y, z, w) in verts {
        ...
        push_f32(&mut vb_data, color.0);
        push_f32(&mut vb_data, color.1);
        push_f32(&mut vb_data, color.2);
        push_f32(&mut vb_data, color.3);
    }
    assert_eq!(vb_data.len(), 3 * 32);

    let vs_bytes = assemble_vs_dcl_positiont_v0_color_v7();
    let ps_bytes = assemble_ps_passthrough_color_v0();

    // D3DVERTEXELEMENT9 stream (little-endian), intentionally *swapped* order:
    // Element 0: COLOR0 float4 at stream 0 offset 16.
    // Element 1: POSITIONT0 float4 at stream 0 offset 0.
    ...
    // Verify background clear and triangle draw.
