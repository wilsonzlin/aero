...
#[test]
fn d3d9_cmd_stream_constants_are_isolated_per_shader_stage() {
    let mut exec = match pollster::block_on(AerogpuD3d9Executor::new_headless()) {
        Ok(exec) => exec,
        Err(AerogpuD3d9Error::AdapterNotFound) => {
            eprintln!("skipping d3d9 stage constants test: wgpu adapter not found");
            return;
        }
    ...
    let stream = build_stream(|out| {
        ...
        // Pixel shader constant update that *must not* clobber the vertex constant register file.
        emit_packet(out, OPC_SET_SHADER_CONSTANTS_F, |out| {
            push_u32(out, 1); // AEROGPU_SHADER_STAGE_PIXEL
            push_u32(out, 0); // start_register
            push_u32(out, 1); // vec4_count
            push_u32(out, 0); // reserved0
                              // c0 = vec4(0.75, 0.75, 0.0, 1.0) => bottom-right texel.
            push_f32(out, 0.75);
            push_f32(out, 0.75);
            push_f32(out, 0.0);
            push_f32(out, 1.0);
    ...
    assert_eq!(
        px(32, 16),
        [255, 0, 0, 255],
        "inside probe should sample the top-left texel (red)"
    );
}
