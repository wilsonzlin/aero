...
#[test]
fn d3d9_cmd_stream_triangle_fan_nonindexed_expands_to_triangle_list() {
    ...
    const AEROGPU_TOPOLOGY_TRIANGLEFAN: u32 = AerogpuPrimitiveTopology::TriangleFan as u32;
    ...
    // Use a clockwise-fanned quad so both triangles render without needing state changes.
    let verts = [
        // v0: top-left (fan center)
        (-0.8f32, 0.8f32, 0.0f32, 1.0f32),
        // v1: top-right
        (0.8f32, 0.8f32, 0.0f32, 1.0f32),
        // v2: bottom-right
        (0.8f32, -0.8f32, 0.0f32, 1.0f32),
    ...
    let stream = build_stream(|out| {
        ...
        emit_packet(out, OPC_SET_PRIMITIVE_TOPOLOGY, |out| {
            push_u32(out, AEROGPU_TOPOLOGY_TRIANGLEFAN);
            push_u32(out, 0); // reserved0
        });
    ...
    // Pixel outside the quad should remain background.
    assert_eq!(px(32, 2), [0, 0, 0, 255]);
    // Probe inside the first triangle (0,1,2).
    assert_eq!(px(48, 16), [0, 255, 0, 255]);
    // Probe inside the second triangle (0,2,3). This is the regression check: previous
