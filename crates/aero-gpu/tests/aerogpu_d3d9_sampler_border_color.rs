...
#[test]
fn d3d9_cmd_stream_sampler_border_color_is_honored() {
        ...
        .contains(wgpu::Features::ADDRESS_MODE_CLAMP_TO_BORDER)
    {
        common::skip_or_panic(
            concat!(
                module_path!(),
                "::d3d9_cmd_stream_sampler_border_color_is_honored"
            ),
            "wgpu device missing ADDRESS_MODE_CLAMP_TO_BORDER",
        );
        return;
    }
    ...
    // Vertex buffer: a fullscreen triangle, clockwise winding (D3D9 default culling).
    // Each vertex is:
    //   float4 position, float4 texcoord
    let uv = [2.0f32, 2.0f32, 0.0f32, 0.0f32];
    let verts = [
        // x, y, z, w
    ...
    let tex_data = [0xFFu8, 0x00, 0x00, 0xFF]; // 1x1 red
    ...
    let stream = build_stream(|out| {
        ...
        for (state, value) in [
            ...
            (D3DSAMP_BORDERCOLOR, 0xFFFF_FFFF),
