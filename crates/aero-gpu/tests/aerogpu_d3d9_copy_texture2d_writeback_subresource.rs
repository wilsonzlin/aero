...
fn d3d9_copy_texture2d_writeback_dst_respects_padded_row_pitch_for_mip0() {
    ...
    // Use a padded row pitch for mip0 (larger than width*bpp). mip1 remains tightly packed.
    let mip0_row_pitch_bytes = 32u32;

    let mip0_size_bytes = (mip0_row_pitch_bytes * dst_h) as usize; // 4 rows with padding.
    let mip1_size_bytes = (2u32 * 2u32 * bpp) as usize; // 2x2 RGBA8
    let backing_size_bytes = mip0_size_bytes + mip1_size_bytes;
    ...
    // Use an exact-size alloc entry to validate backing-size accounting with padded mip0 rows.
    let alloc_table = AllocTable::new([(
        TEX_ALLOC_ID,
        AllocEntry {
            flags: 0,
    ...
    // mip0 should be unchanged (zeros, including padded bytes).
    let mut mip0 = vec![0u8; mip0_size_bytes];
    guest_memory.read(TEX_GPA, &mut mip0).unwrap();
    assert_eq!(mip0, vec![0u8; mip0_size_bytes]);

    // mip1 should contain the written back pattern at the padded offset.
    let mut mip1 = vec![0u8; mip1_size_bytes];
    guest_memory
        .read(TEX_GPA + mip1_offset_bytes, &mut mip1)
        .unwrap();
