...
#[test]
fn d3d9_cmd_stream_fixedfunc_poscolor_vertex_buffer_offset_renders_triangle() {
    ...
    // Vertex decl: POSITIONT float4 at offset 0, COLOR0 d3dcolor at offset 16.
    ...
    push_u16(&mut vertex_decl, 0x00FF); // end marker
    ...
    // Vertex buffer: pad 3 vertices, then triangle vertices. We'll bind the VB with offset_bytes
    // to point at the triangle.
    let red_argb: u32 = 0xFFFF_0000u32;
    let pad_vertex = (0.0f32, 0.0f32, 0.0f32, 1.0f32, 0u32);
    // Clockwise winding.
    let tri_verts = [
    ...
    for (x, y, z, w, color) in tri_verts {
        push_f32(&mut vb_data, x);
        push_f32(&mut vb_data, y);
        push_f32(&mut vb_data, z);
        push_f32(&mut vb_data, w);
    ...
    assert_eq!(vb_data.len(), 6 * 20);
    let vb_offset_bytes = 3u32 * 20u32;
    ...
    let stream = build_stream(|out| {
        ...
        emit_packet(out, OPC_SET_VERTEX_BUFFERS, |out| {
            ...
            push_u32(out, vb_offset_bytes); // offset_bytes
            push_u32(out, 0); // reserved0
        });
