...
fn assemble_ps_sample_s0_add_v0() -> Vec<u8> {
    // ps_2_0:
    //   texld r0, t0, s0
    //   add r0, r0, v0
    //   mov oC0, r0
...
#[test]
fn d3d9_vs_and_ps_sampler_namespaces_are_independent() {
    ...
    const VS_TEX_HANDLE: u32 = 3;
    const PS_TEX_HANDLE: u32 = 4;
    ...
    let vs_bytes = assemble_vs_sample_s0_to_od0_with_t0();
    let ps_bytes = assemble_ps_sample_s0_add_v0();

    let red_tex_data = [0xFFu8, 0x00, 0x00, 0xFF]; // 1x1 red
    let green_tex_data = [0x00u8, 0xFF, 0x00, 0xFF]; // 1x1 green
    ...
    stream.upload_resource(VS_TEX_HANDLE, 0, &red_tex_data);
    stream.upload_resource(PS_TEX_HANDLE, 0, &green_tex_data);
    ...
    // Bind s0 independently for VS and PS. These must not conflict.
    stream.set_texture(AerogpuShaderStage::Vertex, 0, VS_TEX_HANDLE);
    stream.set_texture(AerogpuShaderStage::Pixel, 0, PS_TEX_HANDLE);
