...
#[test]
fn d3d9_cmd_stream_fixedfunc_poscolor_draw_indexed_negative_base_vertex_multi_stream_renders_triangle(
) {
    ...
    const OPC_SET_INDEX_BUFFER: u32 = AerogpuCmdOpcode::SetIndexBuffer as u32;
    ...
    const OPC_DRAW_INDEXED: u32 = AerogpuCmdOpcode::DrawIndexed as u32;
    ...
    // We bind vertex buffers with an offset (vertex 3) and use base_vertex=-3 so that indices 0..2
    // read vertices 0..2 (addresses before the bound offset). This is valid in D3D9 and must be
    // emulated in the executor.
    let base_vertex = -3i32;

    // Stream 0: positions, stride 16. Offset must be >= |base_vertex|*stride to be valid.
    let vb_pos_offset_bytes = 3u32 * 16u32;
    // Stream 1: colors, stride 4.
    let vb_color_offset_bytes = 3u32 * 4u32;

    // Provide 6 vertices so the offset points into valid memory.
    let positions = [
        // Triangle (clockwise) in vertices 0..2.
        (-0.5f32, -0.5f32, 0.0f32, 1.0f32),
        (0.0f32, 0.5f32, 0.0f32, 1.0f32),
        (0.5f32, -0.5f32, 0.0f32, 1.0f32),
        // Padding vertices (unused).
    ...
    assert_eq!(vb_pos_data.len(), 6 * 16);

    // Stream 1: colors (D3DCOLOR).
    let red_argb: u32 = 0xFFFF_0000u32;
    let mut vb_color_data = Vec::new();
    // First 3 vertices: red.
    for _ in 0..3 {
        push_u32(&mut vb_color_data, red_argb);
    }
    // Padding vertices: zero.
    for _ in 0..3 {
        push_u32(&mut vb_color_data, 0);
    }
    assert_eq!(vb_color_data.len(), 6 * 4);

    // Index buffer: three u16 indices, padded to 8 bytes to satisfy executor alignment.
    ...
    let stream = build_stream(|out| {
        ...
        // Bind both streams with offsets so negative base_vertex must be emulated.
        ...
        emit_packet(out, OPC_SET_INDEX_BUFFER, |out| {
            push_u32(out, IB_HANDLE);
            push_u32(out, 0); // reserved0
