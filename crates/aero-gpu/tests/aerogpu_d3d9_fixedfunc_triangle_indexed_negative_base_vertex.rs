...
use aero_protocol::aerogpu::{
    aerogpu_cmd::{
        ...
        AerogpuPrimitiveTopology, AEROGPU_CLEAR_COLOR, AEROGPU_CMD_STREAM_MAGIC, AEROGPU_RESOURCE_USAGE_INDEX_BUFFER,
        AEROGPU_RESOURCE_USAGE_RENDER_TARGET, AEROGPU_RESOURCE_USAGE_TEXTURE, AEROGPU_RESOURCE_USAGE_VERTEX_BUFFER,
    },
...
#[test]
fn d3d9_cmd_stream_fixedfunc_poscolor_draw_indexed_negative_base_vertex_renders_triangle() {
    ...
    // Vertex buffer: 5 vertices, where vertices 0..3 define a clockwise triangle. We will bind
    // the vertex buffer with an offset (60 bytes = 3 vertices) and use a negative base vertex so
    // that indices 0..2 fetch vertices 0..2 (addresses before the bound offset).
    //
    ...
    let verts = [
        ...
        (0.5f32, -0.5f32, 0.0f32, 1.0f32, red_argb),
        // Unused padding vertices (still need to exist because we bind offset_bytes=60).
        (0.0f32, 0.0f32, 0.0f32, 1.0f32, 0),
        (0.0f32, 0.0f32, 0.0f32, 1.0f32, 0),
    ];
    ...
    // Bind the vertex buffer at vertex 3 and use base_vertex=-3 so indices 0..2 fetch vertices 0..2.
    ...
    let center: [u8; 4] = rgba[idx..idx + 4].try_into().unwrap();
    assert_eq!(
        center,
        [255, 0, 0, 255],
        "center pixel should be red (D3DCOLOR should be RGBA in shader even with negative base vertex)"
    );
}
