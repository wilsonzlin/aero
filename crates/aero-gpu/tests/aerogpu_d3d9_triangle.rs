...
fn d3d9_cmd_stream_default_cullmode_culls_ccw_triangles() {
    ...
    let mut vb_data = Vec::new();
    // D3D9 defaults to back-face culling with clockwise front faces, so:
    // - the CCW left-hand triangle should be culled
    // - the CW right-hand triangle should render
    let verts = [
        // Left triangle (CCW) - should be culled.
        (-0.9f32, -0.2f32, 0.0f32, 1.0f32),
        (-0.1f32, -0.2f32, 0.0f32, 1.0f32),
        (-0.5f32, 0.6f32, 0.0f32, 1.0f32),
        // Right triangle (CW) - should render.
        (0.1f32, -0.2f32, 0.0f32, 1.0f32),
        (0.5f32, 0.6f32, 0.0f32, 1.0f32),
        (0.9f32, -0.2f32, 0.0f32, 1.0f32),
    ];
    ...
    let stream = build_stream(|out| {
        ...
        // Draw the CCW triangle (should be culled).
        emit_packet(out, OPC_DRAW, |out| {
            push_u32(out, 3); // vertex_count
            push_u32(out, 1); // instance_count
            push_u32(out, 0); // first_vertex
        ...
        // Draw the CW triangle (should render).
        emit_packet(out, OPC_DRAW, |out| {
            push_u32(out, 3); // vertex_count
            push_u32(out, 1); // instance_count
    ...
    assert_eq!(
        px(16, 32),
        [0, 0, 0, 255],
        "CCW triangle should be culled by default"
    );
    assert_eq!(
        px(48, 32),
        [0, 255, 0, 255],
        "CW triangle should be front-facing by default"
    );
}
