...
#[test]
fn d3d9_cmd_stream_fixedfunc_poscolor_dual_d3dcolor_swizzles_all_color_elements() {
    ...
    // Custom passthrough VS: mov oPos, v0; mov oD0, v2; end.
    //
    ...
    const VS_WORDS: [u32; 8] = [
        ...
        0x0300_0001u32, // mov (2 operands) oD0, v2
    ...
    // Vertex decl: POSITIONT float4 at offset 0, COLOR0 d3dcolor at offset 16, COLOR1 d3dcolor at
    // offset 20.
    ...
    push_u8(&mut vertex_decl, 0); // usage_index (COLOR0)

    push_u16(&mut vertex_decl, 0); // stream
    push_u16(&mut vertex_decl, 20); // offset
    push_u8(&mut vertex_decl, D3DDECLTYPE_D3DCOLOR);
    push_u8(&mut vertex_decl, 0); // method
    push_u8(&mut vertex_decl, D3DDECLUSAGE_COLOR);
    push_u8(&mut vertex_decl, 1); // usage_index (COLOR1)
    ...
    push_u8(&mut vertex_decl, 0);
    push_u8(&mut vertex_decl, 0);
    assert_eq!(vertex_decl.len(), 32);

    // Vertex buffer: float4 position + 2x D3DCOLOR.
    //
    // We output COLOR1 (v2) from the vertex shader. Set it to blue (0xFF0000FF) so we detect
    // BGRA->RGBA conversion errors: without conversion it would be interpreted as red.
    let color0_red_argb: u32 = 0xFFFF_0000u32;
    let color1_blue_argb: u32 = 0xFF00_00FFu32;
    ...
    for (x, y, z, w, c0, c1) in verts {
        ...
        push_u32(&mut vb_data, c0);
        push_u32(&mut vb_data, c1);
    }
    assert_eq!(vb_data.len(), 3 * 24);
