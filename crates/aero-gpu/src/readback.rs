...
/// Read back the stencil plane as an 8-bit-per-pixel array.
///
/// This is intended for tests and debugging; it uses a staging buffer and
/// `map_async`, and works on both native and WASM targets.
///
/// The returned data is tightly packed: `width * height` bytes in row-major
/// order.
///
/// # Panics
/// Panics if mapping fails. (This is test-facing API.)
pub async fn readback_stencil8(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    texture: &wgpu::Texture,
    region: TextureRegion,
    ...
    let buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("aero-gpu.readback_stencil8"),
        size: buffer_size,
        usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
        mapped_at_creation: false,
    });
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("aero-gpu.readback_stencil8.encoder"),
    });
...
/// Read back a Depth32Float region as a tightly packed `Vec<f32>`.
///
/// This is intended for tests and debugging; it uses a staging buffer and
/// `map_async`, and works on both native and WASM targets.
///
/// # Panics
/// Panics if mapping fails. (This is test-facing API.)
pub async fn readback_depth32f(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    texture: &wgpu::Texture,
    region: TextureRegion,
    ...
    let buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("aero-gpu.readback_depth32f"),
        size: buffer_size,
        usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
        mapped_at_creation: false,
    });
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("aero-gpu.readback_depth32f.encoder"),
    });
    ...
    for row in 0..height as usize {
        ...
        for (i, chunk) in mapped[src_start..src_end].chunks_exact(4).enumerate() {
            out[dst_start + i] = f32::from_le_bytes(chunk.try_into().unwrap());
        }
    }
    drop(mapped);
    buffer.unmap();
