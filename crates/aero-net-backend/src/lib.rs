...
//! and provides a ring-buffer-backed L2 tunnel backend suitable for the browser NET_TX/NET_RX
...
///
/// This backend bridges raw Ethernet frames to/from Aero IPC ring buffers so WASM code can
/// exchange frames with a JS forwarder without `postMessage` copies.
///
