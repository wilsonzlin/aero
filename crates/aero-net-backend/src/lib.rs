...
pub trait NetworkBackend {
    ...
    /// Optional stats for the ring-buffer-backed L2 tunnel backend (`NET_TX`/`NET_RX` rings).
    ///
    /// Most backends do not expose ring stats (and return `None`). The [`L2TunnelRingBackend`]
    /// implementation returns a snapshot of its internal counters.
    fn l2_ring_stats(&self) -> Option<L2TunnelRingBackendStats> {
        None
    }
}
