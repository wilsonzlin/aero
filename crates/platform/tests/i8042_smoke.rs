...
fn i8042_port_bus_delivers_translated_scancodes() {
    ...
    let status = platform.io.read_u8(0x64);
    ...
    assert_eq!(platform.io.read_u8(0x60), 0x1E);
...
fn i8042_command_byte_read_write_roundtrip() {
    ...
    platform.io.write_u8(0x64, 0x20);
    let cmd = platform.io.read_u8(0x60);
    assert_eq!(cmd, 0x45);

    // Update it and read it back.
    platform.io.write_u8(0x64, 0x60);
    platform.io.write_u8(0x60, 0x47);

    platform.io.write_u8(0x64, 0x20);
    assert_eq!(platform.io.read_u8(0x60), 0x47);
}
