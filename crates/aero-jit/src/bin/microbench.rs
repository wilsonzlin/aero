...
fn main() {
    ...
    let mut tier1 = Engine::new(
        &program,
        JitConfig { tier1_threshold: 1, tier2_threshold: u64::MAX, ..JitConfig::default() },
    );
    // Warm-up compile.
    ...
    // Tier-2 enabled.
    let mut tier2 = Engine::new(&program, JitConfig { tier1_threshold: 1, tier2_threshold: 5, ..JitConfig::default() });
    ...
    println!("tier1-only: ret={ret1} time={}us stats={:?}", us1, tier1.stats());
    println!("tier2:      ret={ret2} time={}us stats={:?}", us2, tier2.stats());
    if us2 > 0 {
