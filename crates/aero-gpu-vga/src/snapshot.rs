...
// Cap VRAM allocations when decoding snapshots to avoid pathological OOM from corrupt inputs while
// still allowing reasonably-sized VRAM apertures (e.g. BAR1 at 128MiB).
const VGA_SNAPSHOT_MAX_VRAM_LEN: u32 = 512 * 1024 * 1024;
...
impl VgaSnapshotV1 {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        if vram_len > VGA_SNAPSHOT_MAX_VRAM_LEN {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        let vram_len: u32 = self
            ...
            .len()
            .try_into()
            .unwrap_or(VGA_SNAPSHOT_MAX_VRAM_LEN);
...
/// Snapshot payload for [`crate::VgaDevice`] (version 2).
///
/// V2 extends [`VgaSnapshotV1`] with the device's LFB base and explicit VRAM size so snapshot
/// consumers can restore the device configuration deterministically.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VgaSnapshotV2 {
    pub lfb_base: u32,
    pub vram_len: u32,
...
impl VgaSnapshotV2 {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let lfb_base = read_u32_le(&mut r)?;
        let vram_len = read_u32_le(&mut r)?;
        if vram_len > VGA_SNAPSHOT_MAX_VRAM_LEN {
            return Err(VgaSnapshotError::Corrupt("vram too large"));
        }
        ...
        let vram_len_usize = vram_len as usize;
        let mut vram = Vec::new();
        vram.try_reserve_exact(vram_len_usize)
            .map_err(|_| VgaSnapshotError::Corrupt("out of memory"))?;
        vram.resize(vram_len_usize, 0);
        r.read_exact(&mut vram)?;
    ...
    pub fn encode(&self) -> Vec<u8> {
        let mut out = Vec::with_capacity(self.vram.len() + 1024);
        out.extend_from_slice(&self.lfb_base.to_le_bytes());
        out.extend_from_slice(&self.vram_len.to_le_bytes());
...
impl VgaDevice {
    pub fn snapshot_v2(&self) -> VgaSnapshotV2 {
        let mut dac = [[0u8; 3]; 256];
        for (dst, src) in dac.iter_mut().zip(self.dac.iter()) {
            *dst = [src.r, src.g, src.b];
        }
        ...
        VgaSnapshotV2 {
            lfb_base: self.lfb_base(),
            vram_len: self.vram.len() as u32,
            misc_output: self.misc_output,
            sequencer_index: self.sequencer_index,
            sequencer: self.sequencer,
            graphics_index: self.graphics_index,
    ...
    pub fn encode_snapshot_v2(&self) -> Vec<u8> {
        let vram_len: u32 = self
            .vram
            .len()
            .try_into()
    ...
    pub fn restore_snapshot_v2(&mut self, snap: &VgaSnapshotV2) {
        self.set_svga_lfb_base(snap.lfb_base);
