...
async fn dns_fail_metric_increments_on_invalid_hostname() {
    ...
    let baseline = reqwest::get(format!("http://{addr}/metrics"))
        ...
        .await
        .unwrap();
    let start = parse_metric(&baseline, "l2_dns_fail_total").unwrap_or(0);
    ...
    let id = 0x3333;
    // `trust-dns-resolver` rejects hostnames with spaces without making a network request. This
    // gives us a deterministic proxy-side DNS failure for the metrics path.
    let query = build_dns_query(id, "bad name.local", DnsType::A as u16);
    ...
    assert_dns_rcode_and_ancount(&resp, id, 3, 0);
    ...
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        loop {
            let body = reqwest::get(format!("http://{addr}/metrics"))
                ...
                .await
                .unwrap();
            let val = parse_metric(&body, "l2_dns_fail_total").unwrap_or(0);
