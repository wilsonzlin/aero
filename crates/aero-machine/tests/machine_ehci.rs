...
struct LegacyEhciControllerUsbSnapshotSource {
    machine: Machine,
}

impl snapshot::SnapshotSource for LegacyEhciControllerUsbSnapshotSource {
    fn snapshot_meta(&mut self) -> snapshot::SnapshotMeta {
        snapshot::SnapshotSource::snapshot_meta(&mut self.machine)
    }
    ...
    fn device_states(&self) -> Vec<snapshot::DeviceState> {
        ...
        let version = <aero_usb::ehci::EhciController as IoSnapshot>::DEVICE_VERSION;
        devices[pos] = snapshot::DeviceState {
            id: snapshot::DeviceId::USB,
            version: version.major,
            flags: version.minor,
            data: ehci.borrow().controller().save_state(),
        };
        devices
    }
...
#[test]
fn ehci_restore_accepts_legacy_deviceid_usb_controller_payload_without_machine_remainder() {
    let cfg = MachineConfig {
        ram_size_bytes: 2 * 1024 * 1024,
        enable_pc_platform: true,
        enable_ehci: true,
    ...
    let mut src = LegacyEhciControllerUsbSnapshotSource {
        machine: Machine::new(cfg.clone()).unwrap(),
    };
    // Ensure high MMIO addresses decode correctly (avoid A20 aliasing).
    src.machine.io_write(A20_GATE_PORT, 1, 0x02);
    ...
    // Ensure our snapshot source really emitted the legacy payload (`EHCI` controller), not the
    ...
    assert_eq!(
        usb_state.data.get(8..12),
        Some(b"EHCI".as_slice()),
        "legacy snapshot should store EHCI controller snapshot directly under DeviceId::USB"
    );
