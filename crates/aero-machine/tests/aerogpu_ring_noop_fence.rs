...
#[test]
fn aerogpu_duplicate_fence_merges_no_irq_semantics() {
    let cfg = MachineConfig {
        ram_size_bytes: 16 * 1024 * 1024,
        enable_pc_platform: true,
        enable_aerogpu: true,
    ...
    // Two submissions that share the same fence value: the first suppresses IRQ, the second does
    // not. The device must still raise an IRQ when the fence completes.
    let signal_fence = 0x1111_2222_3333_4444u64;
    ...
    m.write_physical_u64(desc0_gpa + 16, 0); // cmd_gpa
    m.write_physical_u32(desc0_gpa + 24, 0); // cmd_size_bytes
    ...
    m.write_physical_u32(desc1_gpa + 8, 0); // context_id
    m.write_physical_u32(desc1_gpa + 12, ring::AEROGPU_ENGINE_0); // engine_id
    m.write_physical_u64(desc1_gpa + 16, 0); // cmd_gpa
    m.write_physical_u32(desc1_gpa + 24, 0); // cmd_size_bytes
    m.write_physical_u32(desc1_gpa + 28, 0);
    m.write_physical_u64(desc1_gpa + 32, 0); // alloc_table_gpa
    m.write_physical_u32(desc1_gpa + 40, 0); // alloc_table_size_bytes
    m.write_physical_u32(desc1_gpa + 44, 0);
    ...
    // Duplicate fence entries must merge NO_IRQ semantics: IRQ should assert because the second
    // submission requested it.
    let irq_status = m.read_physical_u32(bar0 + u64::from(pci::AEROGPU_MMIO_REG_IRQ_STATUS));
    assert_eq!(irq_status & pci::AEROGPU_IRQ_FENCE, pci::AEROGPU_IRQ_FENCE);
...
#[test]
fn aerogpu_zero_signal_fence_submission_is_drained() {
    let cfg = MachineConfig {
        ram_size_bytes: 16 * 1024 * 1024,
        enable_pc_platform: true,
        enable_aerogpu: true,
    ...
    // Submit desc in slot 0 with `signal_fence=0` but a non-empty command stream.
    let desc_gpa = ring_gpa + ring::AerogpuRingHeader::SIZE_BYTES as u64;
    let signal_fence = 0u64;
    ...
    // The command stream should still be captured for out-of-process execution even though
    // `signal_fence == 0`.
    let subs = m.aerogpu_drain_submissions();
