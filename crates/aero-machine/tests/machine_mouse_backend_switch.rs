...
#[test]
fn inject_input_batch_mouse_button_release_stays_on_usb_when_virtio_becomes_ready() {
    ...
    // Enable periodic reports so we can observe the mouse's *current* pressed state even if no new
    // reports are queued.
    ...
    // Batch: press left button (DOM bit0).
    ...
    // Flip virtio mouse to DRIVER_OK between press and release.
