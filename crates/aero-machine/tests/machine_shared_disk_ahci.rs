...
#[test]
fn machine_shared_bios_disk_is_visible_to_ahci_dma() {
    ...
    // READ DMA EXT into guest memory.
    write_cmd_header(&mut m, clb, 0, ctba, 1, false);
    write_cfis(&mut m, ctba, 0x25, lba, 1);
    write_prdt(&mut m, ctba, 0, data_buf, SECTOR_SIZE as u32);
    m.write_physical_u32(bar5_base + PORT_BASE + PORT_REG_CI, 1);

    // Process the command list once; the emulation is synchronous so this is sufficient.
    m.process_ahci();
