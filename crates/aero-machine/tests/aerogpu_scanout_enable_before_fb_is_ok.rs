...
fn aerogpu_scanout_enable_before_fb_is_ok() {
    ...
    // Explicit scanout disable blanks the display but keeps WDDM ownership sticky until reset
    // (Win7 uses this as a visibility toggle).
    m.write_physical_u32(
        bar0_base + aerogpu_pci::AEROGPU_MMIO_REG_SCANOUT0_ENABLE as u64,
        0,
    );
...
fn aerogpu_scanout_disable_blanks_wddm_even_with_legacy_vbe_panning_stride() {
    ...
    // configured (including non-default stride/panning), WDDM ownership remains sticky and the
    // disabled state is represented as a blank WDDM scanout.
    let w = 64u32;
    let h = 64u32;
    let pitch = w * 4;
