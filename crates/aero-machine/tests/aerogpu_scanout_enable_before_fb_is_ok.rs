...
#[test]
fn aerogpu_scanout_disable_reverts_to_legacy_vbe_even_with_panning_stride() {
    ...
    // Claim WDDM scanout, then explicitly disable it. Explicit disable releases WDDM ownership and
    // reverts the shared scanout descriptor back to the legacy VBE source (including the
    // BIOS-programmed stride/panning).
    let w = 64u32;
    let h = 64u32;
    let pitch = w * 4;
