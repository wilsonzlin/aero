...
#[test]
fn pc_machine_network_backend_l2_ring_stats_work_for_boxed_ring_backend() {
    // Host rings (NET_TX is guest->host).
    let tx_ring = Arc::new(RingBuffer::new(16 * 1024));
    let rx_ring = Arc::new(RingBuffer::new(16 * 1024));
...
fn pc_machine_e1000_tx_completes_descriptor_without_network_backend() {
    ...
    desc[10] = 0; // CSO
    desc[11] = (1 << 0) | (1 << 3); // EOP|RS
    desc[12] = 0; // status (device will set DD)
    desc[13] = 0; // CSS
    desc[14..16].copy_from_slice(&0u16.to_le_bytes());
    m.platform_mut().memory.write_physical(tx_ring_base, &desc);
    ...
    // Poll the machine networking bridge once. Even without a backend, DMA should run so the TX
    // descriptor is completed (DD set).
    m.poll_network();

    let mut desc_after = [0u8; 16];
    m.platform_mut()
        .memory
        .read_physical(tx_ring_base, &mut desc_after);
    assert_ne!(
        desc_after[12] & 0x1,
        0,
        "TX descriptor DD should be set even without a network backend"
    );
}
