...
#[test]
fn aerogpu_mmio_scanout_and_cursor_present_from_bar1_vram() {
    // Keep the topology minimal; this test only needs PCI + AeroGPU.
    let cfg = MachineConfig {
        ram_size_bytes: 16 * 1024 * 1024,
        enable_pc_platform: true,
    ...
    // Locate BAR0/BAR1 and enable bus mastering (DMA) so scanout/cursor reads behave like a real PCI
    // device.
    let (bar0_base, bar1_base, command) = {
        ...
        let bus = pci_cfg.bus_mut();
        let bar0 = bus.read_config(bdf, 0x10, 4) & 0xffff_fff0;
        let bar1 = bus.read_config(bdf, 0x14, 4) & 0xffff_fff0;
        assert_ne!(bar0, 0, "expected BIOS to assign a non-zero BAR0");
        assert_ne!(bar1, 0, "expected BIOS to assign a non-zero BAR1");
        let cmd = bus.read_config(bdf, 0x04, 2) as u16;
        (u64::from(bar0), u64::from(bar1), cmd)
    };
    ...
    // 2x2 scanout in BGRA, stored in BAR1 VRAM.
    let fb_gpa = bar1_base + 0x0040_0000;
    let scanout_bytes: [u8; 16] = [
        // Row 0: red, green
        0x00, 0x00, 0xFF, 0xFF, // BGRA red
        0x00, 0xFF, 0x00, 0xFF, // BGRA green
    ...
    // Writes go through the PCI MMIO router into the AeroGPU BAR1 handler, populating VRAM.
    m.write_physical(fb_gpa, &scanout_bytes);
    ...
    // Program a 1x1 opaque magenta cursor at (1, 0), replacing the green pixel.
    let cursor_gpa = bar1_base + 0x0040_1000;
    let cursor_bytes: [u8; 4] = [0xFF, 0x00, 0xFF, 0xFF]; // RGBA magenta
    m.write_physical(cursor_gpa, &cursor_bytes);
...
#[test]
fn aerogpu_mmio_cursor_bgra_present() {
    // Keep the topology minimal; this test only needs PCI + AeroGPU.
    let cfg = MachineConfig {
        ram_size_bytes: 16 * 1024 * 1024,
        enable_pc_platform: true,
    ...
    // 2x1 scanout in BGRA.
    let fb_gpa = 0x200000u64;
    let scanout_bytes: [u8; 8] = [
        // Row 0: green, green (BGRA)
    ...
    // Cursor in BGRA: 1x1 opaque red at (1, 0), overwriting the second pixel.
    let cursor_gpa = 0x201000u64;
    let cursor_bytes: [u8; 4] = [0x00, 0x00, 0xFF, 0xFF]; // BGRA red
    m.write_physical(cursor_gpa, &cursor_bytes);
    ...
    m.display_present();
    assert_eq!(m.display_resolution(), (2, 1));
    assert_eq!(m.display_framebuffer(), &[0xFF00_FF00, 0xFF00_00FF]);
}
