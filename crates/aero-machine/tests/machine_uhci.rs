...
struct LegacyUhciControllerUsbSnapshotSource {
    machine: Machine,
}

impl snapshot::SnapshotSource for LegacyUhciControllerUsbSnapshotSource {
    fn snapshot_meta(&mut self) -> snapshot::SnapshotMeta {
        snapshot::SnapshotSource::snapshot_meta(&mut self.machine)
    }
    ...
    fn device_states(&self) -> Vec<snapshot::DeviceState> {
        ...
        let version = <aero_usb::uhci::UhciController as IoSnapshot>::DEVICE_VERSION;
        devices[pos] = snapshot::DeviceState {
            id: snapshot::DeviceId::USB,
            version: version.major,
            flags: version.minor,
            data: uhci.borrow().controller().save_state(),
        };
        devices
    }
...
#[test]
fn uhci_restore_accepts_legacy_deviceid_usb_controller_payload_without_machine_remainder() {
    let cfg = MachineConfig {
        ram_size_bytes: 2 * 1024 * 1024,
        enable_pc_platform: true,
        enable_uhci: true,
    ...
    let mut src = LegacyUhciControllerUsbSnapshotSource {
        machine: Machine::new(cfg.clone()).unwrap(),
    };
    ...
    // Ensure our snapshot source really emitted the legacy payload (`UHCI` controller), not the
    ...
    assert_eq!(
        usb_state.data.get(8..12),
        Some(b"UHCI".as_slice()),
        "legacy snapshot should store UHCI controller snapshot directly under DeviceId::USB"
    );
