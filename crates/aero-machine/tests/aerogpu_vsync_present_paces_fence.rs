...
fn vsynced_present_does_not_complete_on_elapsed_vblank_before_submission() {
    ...
    // Enable fence IRQ delivery (not required for this test, but matches real driver config).
    m.write_physical_u32(
        bar0 + u64::from(pci::AEROGPU_MMIO_REG_IRQ_ENABLE),
        pci::AEROGPU_IRQ_FENCE,
    );
    ...
    // Establish a vblank schedule at time t0. Then advance the platform clock past the first vblank
    // edge *without* calling `tick_platform`/`process_aerogpu`, so the device hasn't observed that
    // edge yet. The vsynced present submission must not be allowed to complete on that already-
    // elapsed vblank when it is finally processed.
    m.process_aerogpu();
    ...
    // The submission should be consumed, but the fence must remain pending until the *next* vblank
    // edge after submission time.
    assert_eq!(m.read_physical_u32(ring_gpa + 24), 1);
    let completed_fence = mmio_read_u64_pair(
        &mut m,
        bar0 + u64::from(pci::AEROGPU_MMIO_REG_COMPLETED_FENCE_LO),
    ...
    // Advance to the next vblank edge; the vsynced present should now complete.
    clock.advance_ns(u64::from(vblank_period_ns) - 1);
    m.process_aerogpu();
