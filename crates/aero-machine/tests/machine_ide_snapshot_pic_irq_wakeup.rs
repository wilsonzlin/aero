...
fn program_pic_for_ide_irq(ints: &mut PlatformInterrupts, irq: u8) {
    ints.set_mode(PlatformInterruptMode::LegacyPic);
    ints.pic_mut().set_offsets(0x20, 0x28);
    for line in 0..16 {
        ints.pic_mut().set_masked(line, true);
    }
    // Unmask cascade + requested IRQ.
...
#[test]
fn machine_snapshot_roundtrip_preserves_inflight_ide_dma_read_and_wakes_hlt_via_pic_irq14() {
    ...
    let cfg = MachineConfig {
        ...
        enable_ide: true,
        // Keep this test focused on IDE + snapshot/restore + PIC delivery.
        enable_vga: false,
        enable_serial: false,
        enable_i8042: false,
    ...
    sector0[0..8].copy_from_slice(b"PICRDMA!");
    disk.write_sectors(0, &sector0).unwrap();
    src.attach_ide_primary_master_disk(Box::new(disk)).unwrap();

    // Route IRQ14 vector into a real-mode handler that writes a flag byte.
    ...
    // Configure the legacy PIC and unmask cascade + IRQ14.
    {
        let interrupts = src
            .platform_interrupts()
            .expect("pc platform should provide interrupts");
        let mut ints = interrupts.borrow_mut();
        program_pic_for_ide_irq(&mut ints, 14);
    }
    ...
    assert_eq!(
        restored
            ...
            .offsets(),
        (0x20, 0x28),
        "PIC vector offsets should survive snapshot restore"
    );
    ...
    for _ in 0..50 {
        ...
        if restored.read_physical_u8(u64::from(flag_addr)) == flag_value {
            assert_eq!(restored.read_physical_bytes(data_buf, 8), b"PICRDMA!");
            assert_ne!(
...
#[test]
fn machine_snapshot_roundtrip_preserves_inflight_ide_dma_write_and_wakes_hlt_via_pic_irq14() {
    const RAM_SIZE: u64 = 2 * 1024 * 1024;
    const VECTOR: u8 = 0x2E; // PIC slave base 0x28 + (IRQ14-8)
    ...
    let cfg = MachineConfig {
        ...
        enable_pc_platform: true,
        enable_ide: true,
        // Keep this test focused on IDE DMA write + snapshot/restore + PIC delivery.
        enable_vga: false,
        enable_serial: false,
        enable_i8042: false,
        enable_a20_gate: false,
...
#[test]
fn machine_snapshot_roundtrip_preserves_inflight_ide_secondary_atapi_dma_and_wakes_hlt_via_pic_irq15(
) {
    const RAM_SIZE: u64 = 2 * 1024 * 1024;
    const VECTOR: u8 = 0x2F; // PIC slave base 0x28 + (IRQ15-8)

    let cfg = MachineConfig {
        ram_size_bytes: RAM_SIZE,
        enable_pc_platform: true,
        enable_ide: true,
        // Keep this test focused on IDE secondary ATAPI DMA + snapshot/restore + PIC delivery.
        enable_vga: false,
        enable_serial: false,
        enable_i8042: false,
        enable_a20_gate: false,
    ...
    // Route IRQ15 vector into a real-mode handler that writes a flag byte.
    let handler_addr = 0x8000u64;
    let code_base = 0x9000u64;
    ...
    // Configure the legacy PIC and unmask cascade + IRQ15.
    {
        let interrupts = src
            .platform_interrupts()
            .expect("pc platform should provide interrupts");
        let mut ints = interrupts.borrow_mut();
        program_pic_for_ide_irq(&mut ints, 15);
    }
