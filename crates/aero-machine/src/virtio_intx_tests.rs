...
fn new_virtio_net_machine() -> Machine {
...
fn configure_virtio_net_pic(m: &mut Machine) -> (std::rc::Rc<std::cell::RefCell<aero_platform::interrupts::PlatformInterrupts>>, u8) {
    ...
    (interrupts, expected_vector)
}

fn enable_virtio_net_pci_bme_and_mem_decode(m: &mut Machine) {
    let bdf = aero_devices::pci::profile::VIRTIO_NET.bdf;

    // Enable PCI memory decoding + bus mastering for virtio-net so the device is allowed to DMA and
    // complete the TX descriptor chain.
...
fn init_virtio_net_and_post_tx_chain(
    m: &mut Machine,
    virtio: &std::rc::Rc<std::cell::RefCell<aero_virtio::pci::VirtioPciDevice>>,
) -> u64 {
    ...
    write_desc(m, TX_DESC, 1, PAYLOAD_ADDR, payload.len() as u32, 0, 0);
...
#[test]
fn pc_virtio_net_intx_is_synced_even_when_external_interrupt_queue_is_full() {
    let mut m = new_virtio_net_machine();
    ...
    // Fill the external interrupt FIFO to its capacity (1) and disable IF so the CPU cannot drain
    // it. Even though no additional vectors can be enqueued, the machine must still sync PCI INTx
    // sources so the PIC sees the asserted line.
    const ENTRY_IP: u16 = 0x1000;
    m.write_physical(u64::from(ENTRY_IP), &[0x90; 32]);
    init_real_mode_cpu(&mut m, ENTRY_IP, 0);
    m.cpu.state.halted = true;
...
#[test]
fn pc_virtio_net_intx_is_synced_but_not_acknowledged_when_pending_event() {
    let mut m = new_virtio_net_machine();
    ...
    // Process the TX chain so the virtio-net device asserts legacy INTx internally, but do not sync
    // it into the platform interrupts yet.
    m.poll_network();
    assert_eq!(m.read_physical_u16(tx_used + 2), 1);
    assert_eq!(
        PlatformInterruptController::get_pending(&*interrupts.borrow()),
    ...
    // Even though a pending event blocks delivery/acknowledge, the machine must still sync PCI INTx
    // sources so the PIC sees the asserted line.
