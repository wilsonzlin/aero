...
impl fmt::Display for DescriptorFixupError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DescriptorFixupError::TooShort { expected, actual } => write!(
                f,
                "descriptor blob too short: expected at least {expected} bytes, got {actual}"
            ),
            DescriptorFixupError::UnexpectedDescriptorType { expected, actual } => write!(
                f,
            ...
            DescriptorFixupError::TotalLengthExceedsBuffer {
                ...
                f,
                "wTotalLength ({w_total_length}) exceeds available bytes ({available})"
            ),
        }
    }
}

impl core::error::Error for DescriptorFixupError {}
