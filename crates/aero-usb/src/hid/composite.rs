...
mod tests {
    ...
    #[test]
    fn boot_protocol_mouse_wheel_still_triggers_remote_wakeup() {
        let mut dev = UsbCompositeHidInputHandle::new();
        configure(&mut dev);
        ...
        // Even though boot protocol omits the wheel/hwheel fields, scroll should still be treated
        // as user activity for remote wakeup.
        dev.mouse_wheel2(5, 7);
        assert!(dev.poll_remote_wakeup());
        assert!(
            !dev.poll_remote_wakeup(),
