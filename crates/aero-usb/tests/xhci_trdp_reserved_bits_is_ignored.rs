...
fn write_interrupt_in_endpoint_context_with_reserved_trdp(
    mem: &mut TestMemory,
    dev_ctx_base: u64,
    endpoint_id: u8,
    ring_base: u64,
    ...
    // Deliberately set reserved TR Dequeue Pointer bits 1..=3. The controller should treat this
    // as invalid and must not mask them away to execute transfers anyway.
...
#[test]
fn xhci_does_not_execute_transfers_when_trdp_reserved_bits_set() {
    ...
    write_interrupt_in_endpoint_context_with_reserved_trdp(&mut mem, dev_ctx, EP_ID, ring_base);
    ...
    // reserved bits and falls back to shadow ring state, it would still attempt to execute the TD.
    ...
    assert_eq!(
        buf, [0u8; 8],
        "controller must not execute transfers when TRDP reserved bits are set"
    );
    assert_eq!(
