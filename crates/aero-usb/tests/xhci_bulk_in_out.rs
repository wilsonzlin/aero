...
fn xhci_bulk_in_success_without_ioc_advances_ring_without_event() {
    ...
    write_trb(
        &mut mem,
        normal_trb_addr,
        make_normal_trb(buf, 4, true, false, false),
    );
    write_trb(
        &mut mem,
        link_trb_addr,
...
fn xhci_bulk_out_chained_normal_trbs_concatenate_payload() {
    ...
    write_trb(
        &mut mem,
        trb0_addr,
        make_normal_trb(buf0, 2, true, true, false),
    );
    write_trb(
        &mut mem,
        trb1_addr,
        make_normal_trb(buf1, 2, true, false, true),
    );
...
fn xhci_bulk_in_timeout_does_not_halt_and_allows_subsequent_trbs() {
    ...
    write_trb(
        &mut mem,
        trb0_addr,
        make_normal_trb(buf0, 4, true, false, false),
    );
    write_trb(
        &mut mem,
        trb1_addr,
        make_normal_trb(buf1, 4, true, false, true),
    );
...
fn xhci_transfer_executor_noop_trb_advances_ring_and_does_not_touch_device() {
    ...
    assert_eq!(
        in_queue.borrow().len(),
        1,
        "NoOp TRB must not consume IN queue"
    );
