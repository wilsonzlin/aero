...
fn xhci_caplength_hciversion_plausible() {
    ...
    let dboff = ctrl.mmio_read(regs::cap::DBOFF as u64, 4);
    assert_ne!(dboff, 0, "DBOFF should be non-zero");
    assert_eq!(dboff & 0x3, 0, "DBOFF must be dword-aligned");

    let rtsoff = ctrl.mmio_read(regs::cap::RTSOFF as u64, 4);
...
fn xhci_run_stop_toggles_halted_bit() {
    ...
    let op = op_base(&mut ctrl);
    let usbcmd = op + regs::op::USBCMD as u64;
    let usbsts = op + regs::op::USBSTS as u64;

    assert_ne!(
        (ctrl.mmio_read(usbsts, 4) as u32) & regs::op::USBSTS_HCH,
        0,
        "controller should start halted"
    );

    ctrl.mmio_write(usbcmd, 4, u64::from(regs::op::USBCMD_RUN_STOP));
    assert_eq!(
        (ctrl.mmio_read(usbsts, 4) as u32) & regs::op::USBSTS_HCH,
        0,
...
fn xhci_reset_clears_operational_state() {
    ...
    // Dirty some state.
    ctrl.mmio_write(crcr, 4, 0x100);
    ctrl.mmio_write(crcr + 4, 4, 0);
    ctrl.mmio_write(dcbaap, 4, 0x200);
    ctrl.mmio_write(dcbaap + 4, 4, 0);
    ctrl.mmio_write(config, 4, 5);

    ctrl.mmio_write(usbcmd, 4, u64::from(regs::op::USBCMD_RUN_STOP));
    ctrl.tick_1ms_no_dma();
    assert_ne!(
        (ctrl.mmio_read(mfindex, 4) as u32) & regs::runtime::MFINDEX_MASK,
        0,
        "MFINDEX should advance while running"
    );

    // Host controller reset should clear state and return to halted.
    ctrl.mmio_write(usbcmd, 4, u64::from(regs::op::USBCMD_HCRST));
    ...
    assert_eq!(ctrl.mmio_read(crcr, 4), 0);
    assert_eq!(ctrl.mmio_read(crcr + 4, 4), 0);
    assert_eq!(ctrl.mmio_read(dcbaap, 4), 0);
    assert_eq!(ctrl.mmio_read(dcbaap + 4, 4), 0);
    assert_eq!(ctrl.mmio_read(config, 4), 0);
    assert_eq!(ctrl.mmio_read(mfindex, 4), 0);
}
