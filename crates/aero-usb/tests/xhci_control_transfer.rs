...
#[test]
fn xhci_control_data_stage_does_not_mask_buffer_pointer_low_bits() {
    let mut mem = TestMemory::new(0x20_000);
    let mut alloc = Alloc::new(0x1000);
    ...
    // Deliberately use an unaligned buffer pointer. Buffer pointers are not ring pointers and do
    // not carry flags in the low bits; the controller must not mask them.
    let unaligned_buf = data_buf + 1;
    let mut data_trb = Trb {
        parameter: unaligned_buf,
        status: 64,                // TRB Transfer Length
        control: Trb::CONTROL_DIR, // IN
    };
    data_trb.set_cycle(true);
    ...
    // Verify the controller wrote to the *unaligned* buffer address, not the masked base address.
    let mut first = [0u8; 1];
    mem.read_physical(data_buf, &mut first);
    assert_eq!(
        first,
        [0],
        "controller must not mask low bits of DataStage buffer pointers"
    );

    let mut got = vec![0u8; expected.len()];
    mem.read_physical(unaligned_buf, &mut got);
    assert_eq!(got, expected);
