...
#[derive(Clone, Debug)]
struct InterruptInQueueDevice {
    reports: Rc<RefCell<VecDeque<Vec<u8>>>>,
}

impl InterruptInQueueDevice {
    fn new(reports: Rc<RefCell<VecDeque<Vec<u8>>>>) -> Self {
        Self { reports }
    }
}

impl UsbDeviceModel for InterruptInQueueDevice {
    fn handle_control_request(
        &mut self,
        _setup: SetupPacket,
        _data_stage: Option<&[u8]>,
    ...
    fn poll_interrupt_in(&mut self, ep_addr: u8) -> Option<Vec<u8>> {
        // Ensure the controller passed an IN endpoint address.
        assert_eq!(ep_addr & 0x80, 0x80);
        self.reports.borrow_mut().pop_front()
    }
}
...
#[test]
fn ehci_periodic_interrupt_in_nak_then_completes() {
    let mut mem = TestMemory::new(0x20000);
    let mut alloc = Alloc::new(0x1000);
    ...
    mem.write_u32(qh_addr + 0x10, qtd_addr);
    mem.write_u32(qh_addr + 0x14, LP_TERMINATE);

    let reports = Rc::new(RefCell::new(VecDeque::<Vec<u8>>::new()));
    let mut ehci = EhciController::new();
    ehci.hub_mut()
        .attach(0, Box::new(InterruptInQueueDevice::new(reports.clone())));
    ...
    // First tick: NAK leaves the qTD active and guest memory unchanged.
    ehci.tick_1ms(&mut mem);

    let token_after = mem.read_u32(qtd_addr + 0x08);
    assert_ne!(token_after & QTD_STATUS_ACTIVE, 0);
    assert_eq!((token_after >> QTD_TOKEN_BYTES_SHIFT) & 0x7fff, 8);

    let mut got = vec![0u8; 8];
    mem.read(buf_addr, &mut got);
    assert_eq!(got, sentinel);

    assert_eq!(mem.read_u32(qh_addr + 0x10), qtd_addr);
    assert_eq!(ehci.mmio_read(regs::REG_USBSTS, 4) & regs::USBSTS_USBINT, 0);

    // Second tick: provide data and complete the transfer.
    let expected = vec![1, 2, 3, 4, 5, 6, 7, 8];
    reports.borrow_mut().push_back(expected.clone());
    ehci.tick_1ms(&mut mem);
    ...
    assert_ne!(mem.read_u32(qh_addr + 0x10) & LP_TERMINATE, 0);
    assert_ne!(ehci.mmio_read(regs::REG_USBSTS, 4) & regs::USBSTS_USBINT, 0);
}

#[test]
fn ehci_periodic_smask_skips_non_matching_microframe() {
    let mut mem = TestMemory::new(0x20000);
    let mut alloc = Alloc::new(0x1000);
    ...
    ehci.mmio_write(regs::REG_PERIODICLISTBASE, 4, periodic_base);
    // Start at microframe 1. Since SMASK only enables microframe 0, the controller should skip the
    // QH and leave the qTD active.
    ehci.mmio_write(regs::REG_FRINDEX, 4, 1);
    ehci.mmio_write(regs::REG_USBINTR, 4, regs::USBINTR_USBINT);
    ehci.mmio_write(regs::REG_USBCMD, 4, regs::USBCMD_RS | regs::USBCMD_PSE);
    ...
    // Now switch to microframe 0 and tick again; the QH should run and complete.
    ehci.mmio_write(regs::REG_FRINDEX, 4, 0);
    ehci.tick_1ms(&mut mem);
