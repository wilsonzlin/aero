...
#[test]
fn address_device_missing_output_context_does_not_set_address() {
    let mut mem = TestMemory::new(0x20_000);
    let mut alloc = Alloc::new(0x1000);
    ...
    // Intentionally omit DCBAA[slot_id] (Output Device Context pointer). Address Device should fail
    // with ContextStateError, and must not issue SET_ADDRESS to the bound device.
    ...
    let dev = xhci
        ...
        .expect("device must remain reachable");
