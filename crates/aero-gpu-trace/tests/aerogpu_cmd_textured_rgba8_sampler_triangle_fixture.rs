...
use aero_protocol::aerogpu::aerogpu_cmd::{
    AerogpuPrimitiveTopology, AerogpuSamplerAddressMode, AerogpuSamplerFilter, AerogpuShaderStage,
...
fn make_cmd_stream() -> Vec<u8> {
    ...
    // 1x1 RGBA8 texture: solid orange.
    let texel_orange: [u8; 4] = [0xFF, 0x80, 0x00, 0xFF];
    ...
    w.upload_resource(TEX, 0, &texel_orange);
    ...
    w.set_sampler(AerogpuShaderStage::Pixel, 0, SAMPLER);
    w.draw(3, 1, 0, 0);
    w.present(0, 0);
...
#[test]
fn aerogpu_cmd_textured_rgba8_sampler_triangle_trace_fixture_is_stable() {
