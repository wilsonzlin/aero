...
fn fixture_path() -> PathBuf {
    ...
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/aerogpu_cmd_clear_b8g8r8x8.aerogputrace")
}
...
fn make_cmd_stream() -> Vec<u8> {
    ...
    // Use an X8 format (alpha is unused/forced opaque). This exercises:
    // - BGRA render-target mapping, and
    // - alpha forcing behavior (CLEAR must ignore the provided alpha and clear to opaque).
    w.create_texture2d(
        RT,
    ...
    // Clear red with *alpha=0*. For X8 formats the executor should force alpha to 1.0.
...
#[test]
fn aerogpu_cmd_clear_b8g8r8x8_trace_fixture_is_stable() {
