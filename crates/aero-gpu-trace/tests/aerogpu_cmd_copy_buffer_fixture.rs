...
use aero_protocol::aerogpu::aerogpu_cmd::{
    ...
    AEROGPU_COPY_FLAG_NONE, AEROGPU_RESOURCE_USAGE_RENDER_TARGET, AEROGPU_RESOURCE_USAGE_VERTEX_BUFFER,
};
...
fn fixture_path() -> PathBuf {
    ...
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/aerogpu_cmd_copy_buffer.aerogputrace")
}
...
fn make_cmd_stream() -> Vec<u8> {
    ...
    // Fullscreen triangle in solid yellow.
    //
    // Vertex format: float3 position + float4 color.
    let vertices: [f32; 21] = [
        -1.0, -1.0, 0.0, 1.0, 1.0, 0.0, 1.0, // v0
        -1.0, 3.0, 0.0, 1.0, 1.0, 0.0, 1.0, // v1
        3.0, -1.0, 0.0, 1.0, 1.0, 0.0, 1.0, // v2
    ];
    ...
    w.upload_resource(VB_SRC, 0, &vb_bytes);

    // Copy the vertex data into a second buffer and draw from that buffer.
    w.create_buffer(
        VB_DST,
        AEROGPU_RESOURCE_USAGE_VERTEX_BUFFER,
        vb_bytes.len() as u64,
...
#[test]
fn aerogpu_cmd_copy_buffer_trace_fixture_is_stable() {
