...
use aero_storage_server::{
    store::{BoxedAsyncRead, ChunkedObject, ImageCatalogEntry, ImageMeta, ImageStore, StoreError},
    AppState,
};
...
impl ImageStore for BlockingChunkedStore {
    ...
    async fn open_chunked_manifest(&self, image_id: &str) -> Result<ChunkedObject, StoreError> {
        ...
        Ok(ChunkedObject {
            meta: ImageMeta {
                ...
                etag: None,
                last_modified: None,
                content_type: aero_storage_server::store::CONTENT_TYPE_JSON,
            },
            reader: Box::pin(BlockingReader {
                release,
                sent: false,
            }),
        })
...
async fn chunked_concurrency_limit_rejects_second_in_flight_request() {
    let (open_tx, open_rx) = oneshot::channel::<()>();
    let (release_tx, release_rx) = oneshot::channel::<()>();

    let store = Arc::new(BlockingChunkedStore {
        entry: ImageCatalogEntry {
            id: "disk".to_string(),
            name: "disk".to_string(),
            description: None,
    ...
    let url = format!("http://{addr}/v1/images/disk/chunked/manifest.json");

    let resp1 = client.get(&url).send().await.unwrap();
    assert_eq!(resp1.status(), StatusCode::OK);

    // Ensure the first request has reached `open_chunked_manifest` and is now holding its permit.
