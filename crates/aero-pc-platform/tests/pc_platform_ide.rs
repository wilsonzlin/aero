...
fn pc_platform_ide_io_decode_bit_gates_legacy_ports_and_bus_master_bar4() {
    ...
    // present, the IDE model intentionally floats the command block reads high (0xFF) to match
    // common PATA probing logic.
    ...
    // Attach a minimal disk so decoded status reads are distinguishable from the IDE model's
    // "no device present" behavior (which also returns 0xFF for Status/AltStatus).
