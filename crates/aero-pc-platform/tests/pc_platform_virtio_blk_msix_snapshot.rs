...
#[test]
fn pc_platform_virtio_blk_msix_snapshot_restore_preserves_function_mask_pending_and_delivers_after_unmask(
) {
    const RAM_SIZE: usize = 2 * 1024 * 1024;
    let config = PcPlatformConfig {
        enable_ahci: false,
    ...
    // Program MSI-X table entry 0, leaving the entry unmasked. Interrupt delivery should be
    // blocked only by Function Mask.
    let vector: u16 = 0x58;
    let entry0 = bar0_base + table_offset;
    pc.memory.write_u32(entry0, 0xfee0_0000);
    pc.memory.write_u32(entry0 + 0x4, 0);
    ...
    assert_ne!(
        pba_bits & 1,
        0,
        "expected pending bit 0 to be set while Function Mask is set"
    );
    ...
    // Ensure Function Mask + PBA pending bit were restored.
    let msix_cap2 = find_capability(
        &mut restored,
        bdf.bus,
        bdf.device,
    ...
    assert_ne!(
        ctrl_restored & (1 << 14),
        0,
        "expected MSI-X Function Mask bit to be restored"
    );
    ...
    // Clear Function Mask after restore, then process once to ensure the virtio transport observes
    // the canonical PCI config state. This should immediately deliver the pending vector and clear
    // the PBA.
    write_cfg_u16(
        &mut restored,
        bdf.bus,
