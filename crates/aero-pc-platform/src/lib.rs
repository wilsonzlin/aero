...
///
/// The canonical PCI config space used for guest enumeration is owned by `PciConfigPorts` and is
/// distinct from the `AhciPciDevice` model instance. Keep the device model's PCI command register
/// in sync for every MMIO access so the controller's own gating behaviour (MEMORY_ENABLE/BUS_MASTER,
/// INTx disable) matches what the guest programmed in config space.
#[derive(Clone)]
