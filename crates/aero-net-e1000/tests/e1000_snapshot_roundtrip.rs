...
fn snapshot_roundtrip_preserves_state() {
    ...
    dev.mmio_write_u32(&mut mem, REG_OTHER, 0xDEAD_BEEF);

    // Enable interrupt masks and set a pending cause.
    dev.mmio_write_u32(&mut mem, REG_IMS, ICR_RXT0 | ICR_TXDW);
    dev.mmio_write_u32(&mut mem, REG_ICS, ICR_RXT0);

    // Enable RX/TX, but do not configure RX ring yet so frames remain in rx_pending.
    dev.mmio_write_u32(&mut mem, REG_RCTL, RCTL_EN);
    ...
    restored.mmio_write_u32(&mut mem, REG_TDT, 3);
    ...
    restored.mmio_write_u32(&mut mem, REG_RDBAL, 0x3000);
    restored.mmio_write_u32(&mut mem, REG_RDLEN, (RxDesc::LEN as u32) * 4);
    restored.mmio_write_u32(&mut mem, REG_RDH, 0);
    restored.mmio_write_u32(&mut mem, REG_RDT, 3);
