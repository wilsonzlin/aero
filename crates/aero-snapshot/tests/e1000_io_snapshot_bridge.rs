...
fn e1000_io_snapshot_roundtrips_through_bridge() {
    ...
    dev.mmio_write_u32_reg(0x00D0, 0xFFFF_FFFF); // IMS
    dev.mmio_write_u32_reg(0x00C8, 0x0000_0080); // ICS: RXT0
    assert!(dev.irq_level());

    dev.mmio_write_u32_reg(0x1234, 0xCAFE_BABE);

    // Program the MAC address via RAL0/RAH0.
    dev.mmio_write_u32_reg(0x5400, 0x4433_2211);
    dev.mmio_write_u32_reg(0x5404, 0x8000_6655);
