...
pub fn bios_post(pci: &mut PciBus, allocator: &mut PciResourceAllocator) -> Result<(), PciResourceError> {
    pci.reset(allocator)?;

    let addrs = pci.iter_device_addrs().collect::<Vec<_>>();
    for addr in addrs {
        let Some(cfg) = pci.device_config(addr) else { continue };
        let mut command: u16 = 0;
        for bar in 0u8..6u8 {
            let Some(def) = cfg.bar_definition(bar) else { continue };
            match def.kind() {
