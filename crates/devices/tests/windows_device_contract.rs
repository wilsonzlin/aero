...
fn windows_device_contract_virtio_input_inf_uses_distinct_keyboard_mouse_device_descs() {
    ...
    for section in ["Aero.NTx86", "Aero.NTamd64"] {
        ...
        // The canonical INF (`aero_virtio_input.inf`) includes a strict contract-gated fallback
        // match (no SUBSYS) so Windows can still bind the driver if subsystem IDs are not
        // exposed/recognized. The tablet INF is more specific (`SUBSYS_0012...`) so it wins when
        // present; see `drivers/windows7/virtio-input/docs/pci-hwids.md` for details.
