...
const MAX_BITMAP_BYTES: u64 = 32 * 1024 * 1024; // 32 MiB
                                                ...
                                                // `block_size` bytes).
                                                //
                                                // This cap intentionally matches the Aero sparse disk block-size cap (`MAX_BLOCK_SIZE_BYTES` in
    ...
    pub fn open(mut backend: B) -> Result<Self> {
        ...
        match footer.disk_type {
            ...
            VHD_DISK_TYPE_DYNAMIC => {
                ...
                // Collect allocated block starts (in 512-byte sectors, same units as the BAT)
                // so the overlap check doesn't require an extra `Vec<u64>` proportional to the
                // BAT size.
