...
fn lsr_reports_thr_empty_and_data_ready() {
    ...
    let lsr = uart.port_read(0x3F8 + 5, 1) as u8;
    ...
    let byte = uart.port_read(0x3F8, 1) as u8;
    assert_eq!(byte, 0x41);
    let lsr = uart.port_read(0x3F8 + 5, 1) as u8;
    assert_eq!(lsr & 0x01, 0);
}
...
fn dlab_switches_data_registers() {
    ...
    uart.port_write(0x3F8 + 1, 1, 0xAA);
    assert_eq!(uart.port_read(0x3F8 + 1, 1) as u8, 0xAA);
}
...
fn thr_write_invokes_tx_callback() {
    ...
    uart.port_write(0x3F8, 1, b'H' as u32);
    uart.port_write(0x3F8, 1, b'i' as u32);
...
fn iir_fifo_bits_reflect_fcr_enable() {
    ...
    uart.port_write(0x3F8 + 2, 1, 0x01);
    let iir = uart.port_read(0x3F8 + 2, 1) as u8;
