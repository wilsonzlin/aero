...
fn pci_vga_device_enumerates_with_lfb_bar() {
    ...
    bus.insert_function(0, 2, 0, VgaPciFunction::new());

    bus.io_write(CONFIG_ADDRESS_PORT, 4, cfg_addr(0, 2, 0, 0x00));
    ...
    bus.io_write(CONFIG_ADDRESS_PORT, 4, cfg_addr(0, 2, 0, 0x08));
    ...
    bus.io_write(CONFIG_ADDRESS_PORT, 4, cfg_addr(0, 2, 0, 0x0C));
    let hdr = bus.io_read(CONFIG_DATA_PORT, 4);
    let header_type = ((hdr >> 16) & 0xFF) as u8;
    assert_eq!(header_type, 0x00);

    bus.io_write(CONFIG_ADDRESS_PORT, 4, cfg_addr(0, 2, 0, 0x10));
    ...
    let vga = bus
        .function_mut_typed::<VgaPciFunction>(0, 2, 0)
