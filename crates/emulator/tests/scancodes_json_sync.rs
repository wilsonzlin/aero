use emulator::io::input::ps2_set2_bytes_for_key_event;
...
fn generated_scancodes_match_source_json() {
    ...
    for (code, entry) in ps2_set2.iter() {
        ...
        let actual_make = ps2_set2_bytes_for_key_event(code, true).unwrap_or_else(|| {
        ...
        let actual_break = ps2_set2_bytes_for_key_event(code, false).unwrap_or_else(|| {
            panic!("Missing generated scancode mapping for browser code {code:?}. {REGEN_HINT}")
        });
        assert_eq!(
            actual_break,
