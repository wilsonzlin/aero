...
fn uhci_usbcmd_roundtrips_extended_bits_and_halted_tracks_rs() {
    ...
    uhci.write(REG_USBCMD, 2, (USBCMD_RS | USBCMD_CF | USBCMD_MAXP) as u32);
    assert_eq!(
        uhci.read(REG_USBCMD, 2) as u16,
        USBCMD_RS | USBCMD_CF | USBCMD_MAXP
    );
    assert_eq!(uhci.read(REG_USBSTS, 2) as u16 & USBSTS_HCHALTED, 0);
...
fn uhci_hcreset_restores_default_register_state() {
    ...
    uhci.write(REG_USBCMD, 2, (USBCMD_RS | USBCMD_CF | USBCMD_MAXP) as u32);
    uhci.write(REG_USBINTR, 2, 0x0f);
    uhci.write(REG_FRNUM, 2, 0x0555);
    uhci.write(REG_FLBASEADD, 4, 0x1234_5000);
    uhci.write(REG_SOFMOD, 1, 12);

    // Host controller reset is write-1-to-reset and self-clears in USBCMD.
    uhci.write(REG_USBCMD, 2, USBCMD_HCRESET as u32);

    assert_eq!(uhci.read(REG_USBCMD, 2) as u16, USBCMD_MAXP);
    assert_ne!(uhci.read(REG_USBSTS, 2) as u16 & USBSTS_HCHALTED, 0);
    assert_eq!(uhci.read(REG_USBINTR, 2) as u16, 0);
    assert_eq!(uhci.read(REG_FRNUM, 2) as u16, 0);
    assert_eq!(uhci.read(REG_FLBASEADD, 4), 0);
    assert_eq!(uhci.read(REG_SOFMOD, 1) as u8, 64);
}
...
fn uhci_usbsts_write_1_to_clear_clears_latched_interrupt_bits() {
    ...
    let mut uhci = new_test_uhci(0);
    uhci.write(REG_FLBASEADD, 4, FRAME_LIST_BASE);
    uhci.write(REG_USBINTR, 2, USBINTR_TIMEOUT_CRC as u32);
    uhci.write(REG_USBCMD, 2, USBCMD_RS as u32);

    uhci.tick_1ms(&mut mem);

    let sts = uhci.read(REG_USBSTS, 2) as u16;
    assert_ne!(sts & USBSTS_USBERRINT, 0);
    assert!(uhci.irq_level());

    // W1C should clear the latched status and drop IRQ.
    uhci.write(REG_USBSTS, 2, USBSTS_USBERRINT as u32);
    let sts = uhci.read(REG_USBSTS, 2) as u16;
...
fn uhci_usbint_sets_even_when_interrupts_disabled() {
    ...
    uhci.write(REG_FLBASEADD, 4, FRAME_LIST_BASE);
    uhci.write(REG_USBINTR, 2, 0);
    uhci.write(REG_USBCMD, 2, (USBCMD_RS | USBCMD_MAXP) as u32);

    uhci.tick_1ms(&mut mem);

    assert_ne!(uhci.read(REG_USBSTS, 2) as u16 & USBSTS_USBINT, 0);
    assert!(!uhci.irq_level());

    // Enabling IOC interrupts after the fact should raise IRQ immediately (level-triggered).
    uhci.write(REG_USBINTR, 2, USBINTR_IOC as u32);
    assert!(uhci.irq_level());
}
...
fn uhci_short_packet_does_not_irq_when_short_interrupt_disabled() {
    ...
    uhci.write(REG_USBINTR, 2, (USBINTR_IOC | USBINTR_SHORT_PACKET) as u32);
    assert!(uhci.irq_level());
}
...
fn uhci_usbsts_byte_writes_do_not_cross_clear_w1c_bits() {
    ...
    assert_ne!(uhci.read(REG_USBSTS, 2) as u16 & USBSTS_USBERRINT, 0);

    // Writing the USBSTS high byte should not clear low-byte W1C bits.
    uhci.write(REG_USBSTS + 1, 1, 0xff);
    assert_ne!(uhci.read(REG_USBSTS, 2) as u16 & USBSTS_USBERRINT, 0);

    // Clear via an 8-bit W1C write to the low byte.
    uhci.write(REG_USBSTS, 1, USBSTS_USBERRINT as u32);
...
fn uhci_pci_bar_relocation_updates_io_window() {
    ...
    uhci.write(0x1000 + REG_USBCMD, 2, USBCMD_CF as u32);
    assert_eq!(uhci.read(0x1000 + REG_USBCMD, 2) as u16, USBCMD_CF);
    ...
    assert_eq!(uhci.read(0x1000 + REG_USBCMD, 2), u32::from(u16::MAX));

    // New window works.
    uhci.write(0x2000 + REG_USBCMD, 2, (USBCMD_CF | USBCMD_MAXP) as u32);
    assert_eq!(
        uhci.read(0x2000 + REG_USBCMD, 2) as u16,
...
fn uhci_register_block_supports_byte_accesses() {
    ...
    assert_eq!(uhci.read(REG_USBCMD, 1) as u8, USBCMD_MAXP as u8);
    assert_eq!(uhci.read(REG_USBCMD + 1, 1) as u8, 0);

    // Set RS via an 8-bit write (must include MAXP to preserve it since MAXP is in the same byte).
    uhci.write(REG_USBCMD, 1, (USBCMD_MAXP | USBCMD_RS) as u32);
    assert_eq!(
        uhci.read(REG_USBCMD, 2) as u16 & (USBCMD_MAXP | USBCMD_RS),
        USBCMD_MAXP | USBCMD_RS
    );
    assert_eq!(uhci.read(REG_USBSTS, 2) as u16 & USBSTS_HCHALTED, 0);

    // USBINTR is also byte-accessible (low byte only).
    uhci.write(REG_USBINTR, 1, USBINTR_TIMEOUT_CRC as u32);
    assert_eq!(uhci.read(REG_USBINTR, 2) as u16, USBINTR_TIMEOUT_CRC);
}
...
fn uhci_reserved_register_bytes_read_as_zero() {
    ...
    assert_eq!(uhci.read(REG_SOFMOD, 2) as u16, 0x0040);
    assert_eq!(uhci.read(REG_SOFMOD, 4), 0x0000_0040);
}
...
fn uhci_portsc_high_byte_write_does_not_clear_change_bits() {
    ...
    let before = uhci.read(REG_PORTSC1, 2) as u16;
    assert_ne!(before & PORTSC_CSC, 0);
    assert_ne!(before & PORTSC_PEDC, 0);

    // SUSP is bit12, i.e. bit4 of the high byte.
    uhci.write(REG_PORTSC1 + 1, 1, 0x10);

    let after = uhci.read(REG_PORTSC1, 2) as u16;
...
fn uhci_fgr_latches_resume_detect_and_can_irq() {
    ...
    uhci.write(REG_USBINTR, 2, USBINTR_RESUME as u32);
    assert!(!uhci.irq_level());
...
fn uhci_suspended_hid_device_can_remote_wake_and_trigger_resume_irq() {
    ...
    uhci.write(REG_USBINTR, 2, USBINTR_RESUME as u32);
    uhci.write(REG_PORTSC1, 2, (PORTSC_PED | PORTSC_SUSP) as u32);
...
fn uhci_greset_resets_attached_devices() {
    ...
    uhci.write(REG_USBCMD, 2, (USBCMD_GRESET | USBCMD_MAXP) as u32);
    ...
    uhci.write(REG_USBCMD, 2, USBCMD_MAXP as u32);
    uhci.write(REG_USBCMD, 2, (USBCMD_GRESET | USBCMD_MAXP) as u32);
...
fn uhci_egsm_suspends_frame_counter() {
    ...
    uhci.write(REG_USBCMD, 2, (USBCMD_RS | USBCMD_MAXP) as u32);
    let fr0 = uhci.read(REG_FRNUM, 2) as u16;
    uhci.tick_1ms(&mut mem);
    let fr1 = uhci.read(REG_FRNUM, 2) as u16;
    ...
    let fr_before = uhci.read(REG_FRNUM, 2) as u16;
    for _ in 0..5 {
        uhci.tick_1ms(&mut mem);
    }
    let fr_after = uhci.read(REG_FRNUM, 2) as u16;
    assert_eq!(fr_after, fr_before);

    // Resume: FRNUM advances again.
    uhci.write(REG_USBCMD, 2, (USBCMD_RS | USBCMD_MAXP) as u32);
    uhci.tick_1ms(&mut mem);
    let fr_resume = uhci.read(REG_FRNUM, 2) as u16;
...
fn uhci_portsc_suspend_resume_bits_roundtrip() {
    ...
    uhci.write(REG_PORTSC1, 2, (PORTSC_PED | PORTSC_SUSP) as u32);
    assert_ne!(uhci.read(REG_PORTSC1, 2) as u16 & PORTSC_SUSP, 0);
    ...
    let st = uhci.read(REG_PORTSC1, 2) as u16;
    assert_ne!(st & PORTSC_SUSP, 0);
    assert_ne!(st & PORTSC_RESUME, 0);

    // Clearing both bits.
    uhci.write(REG_PORTSC1, 2, PORTSC_PED as u32);
    let st = uhci.read(REG_PORTSC1, 2) as u16;
...
fn uhci_portsc_suspend_resume_bits_clear_on_detach() {
    ...
    assert_ne!(
        uhci.read(REG_PORTSC1, 2) as u16 & (PORTSC_SUSP | PORTSC_RESUME),
        0
    );
...
fn uhci_regs_power_on_defaults() {
    ...
    let usbcmd = uhci.read(REG_USBCMD, 2) as u16;
    assert_eq!(usbcmd, USBCMD_MAXP);

    let usbsts = uhci.read(REG_USBSTS, 2) as u16;
    assert_eq!(usbsts, USBSTS_HCHALTED);
    assert_eq!(usbsts & !USBSTS_READ_MASK, 0);

    let usbintr = uhci.read(REG_USBINTR, 2) as u16;
    assert_eq!(usbintr, 0);

    let frnum = uhci.read(REG_FRNUM, 2) as u16;
    assert_eq!(frnum, 0);

    let flbaseadd = uhci.read(REG_FLBASEADD, 4);
    assert_eq!(flbaseadd, 0);

    let sofmod = uhci.read(REG_SOFMOD, 1) as u8;
...
fn uhci_regs_frnum_flbaseadd_sofmod_masks() {
    ...
    uhci.write(REG_FRNUM, 2, 0xffff);
    assert_eq!(uhci.read(REG_FRNUM, 2) as u16, 0x07ff);

    // FLBASEADD is 4KiB aligned.
    uhci.write(REG_FLBASEADD, 4, 0x12345);
    assert_eq!(uhci.read(REG_FLBASEADD, 4), 0x12000);

    // SOFMOD is read/write.
    uhci.write(REG_SOFMOD, 1, 0x12);
    assert_eq!(uhci.read(REG_SOFMOD, 1) as u8, 0x12);
}
...
fn uhci_regs_w1c_usb_sts() {
    ...
    uhci.write(REG_USBSTS, 2, 0);
    assert_eq!(uhci.read(REG_USBSTS, 2) as u16, usbsts);

    // Clear remaining W1C bits.
    uhci.write(REG_USBSTS, 2, u32::from(USBSTS_W1C_MASK));
    assert_eq!(uhci.read(REG_USBSTS, 2) as u16, USBSTS_HCHALTED);
}
...
fn uhci_regs_hcreset_self_clears_and_resets_registers() {
    ...
    let portsc_before = uhci.read(REG_PORTSC1, 2) as u16;
    ...
    uhci.write(REG_USBINTR, 2, u32::from(USBINTR_MASK));
    uhci.write(REG_FRNUM, 2, 0x07ff);
    uhci.write(REG_FLBASEADD, 4, 0x12345);
    uhci.write(REG_SOFMOD, 1, 0x12);
    uhci.controller
    ...
    uhci.write(REG_USBCMD, 2, u32::from(USBCMD_HCRESET));
    ...
    assert_eq!(uhci.read(REG_FRNUM, 2) as u16, 0);
    assert_eq!(uhci.read(REG_FLBASEADD, 4), 0);
    assert_eq!(uhci.read(REG_SOFMOD, 1) as u8, 0x40);
    assert_eq!(uhci.read(REG_USBSTS, 2) as u16, USBSTS_HCHALTED);
    assert!(!uhci.irq_level());

    // Host controller reset should not detach the root-hub device.
    let portsc_after = uhci.read(REG_PORTSC1, 2) as u16;
...
fn uhci_regs_irq_gating() {
    ...
    uhci.write(REG_USBINTR, 2, u32::from(USBINTR_IOC));
    assert!(uhci.irq_level());

    uhci.write(REG_USBINTR, 2, 0);
    assert!(!uhci.irq_level());

    uhci.write(REG_USBINTR, 2, u32::from(USBINTR_SHORT_PACKET));
    assert!(uhci.irq_level());

    uhci.write(REG_USBSTS, 2, u32::from(USBSTS_USBINT));
    assert!(!uhci.irq_level());
    ...
    uhci.write(REG_USBINTR, 2, u32::from(USBINTR_TIMEOUT_CRC));
    assert!(uhci.irq_level());

    uhci.write(REG_USBSTS, 2, u32::from(USBSTS_USBERRINT));
    assert!(!uhci.irq_level());
    ...
    uhci.write(REG_USBINTR, 2, u32::from(USBINTR_RESUME));
    assert!(uhci.irq_level());

    uhci.write(REG_USBSTS, 2, u32::from(USBSTS_RESUMEDETECT));
    assert!(!uhci.irq_level());
}
