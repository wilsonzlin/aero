...
#[test]
fn cmd_exec_d3d11_texture_sampling_respects_sampler_repeat() {
    fn pack_bgra(b: u8, g: u8, r: u8, a: u8) -> u32 {
        (a as u32) << 24 | (r as u32) << 16 | (g as u32) << 8 | (b as u32)
    }
    ...
    const VTX_STRIDE: u32 = 16;
    let mut vb_payload = Vec::with_capacity(4 * (VTX_STRIDE as usize));
    // Keep UVs constant across the quad so every pixel samples the same coordinate.
    // u=1.25 should wrap with Repeat and clamp with ClampToEdge.
    let uv_u = 1.25;
    let uv_v = 0.25;
    // Top-left
    push_f32(&mut vb_payload, -1.0);
    push_f32(&mut vb_payload, 1.0);
    push_f32(&mut vb_payload, uv_u);
    push_f32(&mut vb_payload, uv_v);
    // Top-right
    push_f32(&mut vb_payload, 1.0);
    push_f32(&mut vb_payload, 1.0);
    push_f32(&mut vb_payload, uv_u);
    ...
    // CREATE_SAMPLER (handle 6): point sampling + repeat U.
    push_u32(&mut stream, cmd::AerogpuCmdOpcode::CreateSampler as u32);
    push_u32(&mut stream, 28);
    push_u32(&mut stream, 6);
    push_u32(&mut stream, cmd::AerogpuSamplerFilter::Nearest as u32);
    push_u32(&mut stream, cmd::AerogpuSamplerAddressMode::Repeat as u32);
    push_u32(
        &mut stream,
        cmd::AerogpuSamplerAddressMode::ClampToEdge as u32,
    );
    ...
    // SET_SAMPLERS: PS s0 = sampler 6 (repeat U).
    push_u32(&mut stream, cmd::AerogpuCmdOpcode::SetSamplers as u32);
    push_u32(&mut stream, 28);
    push_u32(&mut stream, cmd::AerogpuShaderStage::Pixel as u32);
    push_u32(&mut stream, 0); // start_slot
    ...
    let pixel = read_pixel_bgra(&mut mem, rt_alloc_gpa, rt_pitch, 8, 8);
    assert_eq!(pixel & 0x00FF_FFFF, 0x0000_FFFF);
}
