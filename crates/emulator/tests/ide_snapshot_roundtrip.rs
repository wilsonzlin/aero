...
fn ide_atapi_pio_read10_snapshot_roundtrip_mid_data_phase() {
    ...
    // Canonical ATAPI device restore drops the host ISO backend; reattach for the remainder of the
    // command (and for future packets after restore).
    restored.attach_secondary_master_atapi_backend_for_restore(Box::new(iso2));
...
fn ide_dma_is_gated_on_pci_bus_master_enable() {
    ...
    let command = ide.config_read(0x04, 2) as u16;
    ide.config_write(0x04, 2, u32::from(command & !(1 << 2)));
