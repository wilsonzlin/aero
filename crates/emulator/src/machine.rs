...
//! `aero_machine`), rather than using the deterministic SMP/APIC model types in `aero_smp_model`
//! (exposed via `emulator::smp` only when `--features legacy-smp-model` is enabled).
