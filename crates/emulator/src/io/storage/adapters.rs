...
//!
//! These adapters are intentionally lightweight and only preserve a subset of error
//! information when crossing crate boundaries. In particular, we ensure that:
//! - out-of-bounds/out-of-range errors stay out-of-bounds/out-of-range, and
//! - unaligned buffer/length errors stay unaligned.
//!
//! Everything else may be collapsed into an I/O error on the destination side.
...
fn emu_err_from_aero_err(err: aero_storage::DiskError) -> DiskError {
    match err {
...
impl<S: ByteStorage> aero_storage::StorageBackend for StorageBackendFromByteStorage<S> {
    fn len(&mut self) -> aero_storage::Result<u64> {
        self.0.len().map_err(|e| aero_err_from_emu_byte_err(e, None, None, None))
    }

    fn set_len(&mut self, len: u64) -> aero_storage::Result<()> {
        self.0
            .set_len(len)
            .map_err(|e| aero_err_from_emu_byte_err(e, None, None, Some(len)))
    }
    ...
    fn read_at(&mut self, offset: u64, buf: &mut [u8]) -> aero_storage::Result<()> {
        match self.0.read_at(offset, buf) {
            ...
            Err(err) => {
                ...
                Err(aero_err_from_emu_byte_err(
                    err,
                    Some(offset),
                    Some(buf.len()),
                    capacity,
    ...
    fn flush(&mut self) -> aero_storage::Result<()> {
        self.0
            .flush()
            .map_err(|e| aero_err_from_emu_byte_err(e, None, None, None))
    }
}
...
impl<B: aero_storage::StorageBackend> ByteStorage for ByteStorageFromStorageBackend<B> {
    fn read_at(&mut self, offset: u64, buf: &mut [u8]) -> DiskResult<()> {
        self.0.read_at(offset, buf).map_err(emu_err_from_aero_err)
    }

    fn write_at(&mut self, offset: u64, buf: &[u8]) -> DiskResult<()> {
        self.0
            .write_at(offset, buf)
            .map_err(emu_err_from_aero_err)
    }

    fn flush(&mut self) -> DiskResult<()> {
        self.0.flush().map_err(emu_err_from_aero_err)
    }

    fn len(&mut self) -> DiskResult<u64> {
        self.0.len().map_err(emu_err_from_aero_err)
    }

    fn set_len(&mut self, len: u64) -> DiskResult<()> {
        self.0.set_len(len).map_err(emu_err_from_aero_err)
    }
}
...
impl<D: aero_storage::VirtualDisk> DiskBackend for EmuDiskBackendFromVirtualDisk<D> {
    ...
    fn read_sectors(&mut self, lba: u64, buf: &mut [u8]) -> DiskResult<()> {
        ...
        self.0.read_at(offset, buf).map_err(|err| match err {
            aero_storage::DiskError::UnalignedLength { len, alignment } => DiskError::UnalignedBuffer {
                len,
            ...
            other => emu_err_from_aero_err(other),
        })
    }
    ...
    fn write_sectors(&mut self, lba: u64, buf: &[u8]) -> DiskResult<()> {
        ...
        self.0.write_at(offset, buf).map_err(|err| match err {
            aero_storage::DiskError::UnalignedLength { len, alignment } => DiskError::UnalignedBuffer {
                len,
                sector_size: alignment.try_into().unwrap_or(sector_size),
            },
    ...
    fn read_at(&mut self, offset: u64, buf: &mut [u8]) -> aero_storage::Result<()> {
        ...
        if first_off != 0 {
            ...
            self.0
                ...
                .map_err(|e| aero_err_from_emu_byte_err(e, Some(cur_offset), Some(remaining.len()), Some(capacity)))?;
        ...
        if !remaining.is_empty() {
            let aligned_len = remaining.len() / aero_storage::SECTOR_SIZE * aero_storage::SECTOR_SIZE;
            if aligned_len > 0 {
                let lba = cur_offset / sector_size_u64;
                self.0
                    .read_sectors(lba, &mut remaining[..aligned_len])
                    .map_err(|e| aero_err_from_emu_byte_err(e, Some(cur_offset), Some(aligned_len), Some(capacity)))?;
    ...
    fn write_at(&mut self, offset: u64, buf: &[u8]) -> aero_storage::Result<()> {
        ...
        if first_off != 0 {
            ...
            self.0
                ...
                .map_err(|e| aero_err_from_emu_byte_err(e, Some(cur_offset), Some(to_copy), Some(capacity)))?;
