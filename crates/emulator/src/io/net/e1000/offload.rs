...
//!
//! Windows (and Linux) e1000 drivers may emit *advanced* transmit descriptors when
//! checksum offloads / large send offload are enabled:
...
//!
//! Real hardware performs checksum insertion and TCP segmentation offload (TSO) in
//! silicon. In the emulator we translate those requests into fully-formed Ethernet
//! frames before handing them to the host/network stack.
