...
#[test]
fn decodes_and_translates_gt_float_compare_shader_from_dxbc() {
    ...
    // gt o0, l(1.0), l(2.0)
    body.push(opcode_token(OPCODE_GT, 1 + 2 + 2 + 2));
    body.extend_from_slice(&reg_dst(OPERAND_TYPE_OUTPUT, 0, WriteMask::XYZW));
    body.extend_from_slice(&imm32_scalar(1.0f32.to_bits()));
    body.extend_from_slice(&imm32_scalar(2.0f32.to_bits()));
    body.push(opcode_token(OPCODE_RET, 1));
    ...
    assert!(
        translated.wgsl.contains(" > "),
        "expected gt compare operator in WGSL:\n{}",
        translated.wgsl
    );
    assert!(translated
        .wgsl
...
#[test]
fn decodes_and_translates_le_float_compare_shader_from_dxbc() {
    // Float compare opcodes (`lt/le/gt/ge/eq/ne`) consume numeric f32 values but still write
    // predicate mask bits (0xffffffff / 0) into the untyped register file.
    let mut body = Vec::<u32>::new();

    // le o0, l(1.0), l(2.0)
    body.push(opcode_token(OPCODE_LE, 1 + 2 + 2 + 2));
    body.extend_from_slice(&reg_dst(OPERAND_TYPE_OUTPUT, 0, WriteMask::XYZW));
    body.extend_from_slice(&imm32_scalar(1.0f32.to_bits()));
    body.extend_from_slice(&imm32_scalar(2.0f32.to_bits()));
    body.push(opcode_token(OPCODE_RET, 1));
    ...
    assert!(
        translated.wgsl.contains(" <= "),
        "expected le compare operator in WGSL:\n{}",
        translated.wgsl
    );
    assert!(translated
        .wgsl
