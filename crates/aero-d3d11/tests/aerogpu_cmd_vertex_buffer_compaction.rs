...
#[test]
fn aerogpu_cmd_compacts_sparse_vertex_buffer_slots() {
    pollster::block_on(async {
        ...
        let allocs = [
            AerogpuAllocEntry {
                ...
                flags: 0,
                gpa: 0x100,
                size_bytes: pos_bytes.len() as u64,
                reserved0: 0,
            },
            AerogpuAllocEntry {
                alloc_id: 2,
                flags: 0,
                gpa: 0x200,
                size_bytes: color_bytes.len() as u64,
                reserved0: 0,
            },
        ];

        // ILAY blob with POSITION0 in slot 0 and COLOR0 in slot 15.
        ...
        // POSITION0 @ v0 from slot 0.
        ilay.extend_from_slice(&pos_hash.to_le_bytes());
        ...
        // COLOR0 @ v1 from slot 15.
        ilay.extend_from_slice(&color_hash.to_le_bytes());
        ilay.extend_from_slice(&0u32.to_le_bytes()); // semantic_index
        ...
        // CREATE_BUFFER (VB_COLOR)
        let start = begin_cmd(&mut stream, OPCODE_CREATE_BUFFER);
        stream.extend_from_slice(&VB_COLOR.to_le_bytes());
        stream.extend_from_slice(&(AEROGPU_RESOURCE_USAGE_VERTEX_BUFFER).to_le_bytes());
        stream.extend_from_slice(&(color_bytes.len() as u64).to_le_bytes());
        stream.extend_from_slice(&2u32.to_le_bytes()); // backing_alloc_id
        stream.extend_from_slice(&0u32.to_le_bytes()); // backing_offset_bytes
        stream.extend_from_slice(&0u64.to_le_bytes()); // reserved0
        end_cmd(&mut stream, start);

        // RESOURCE_DIRTY_RANGE (full VB_POS)
        let start = begin_cmd(&mut stream, OPCODE_RESOURCE_DIRTY_RANGE);
        stream.extend_from_slice(&VB_POS.to_le_bytes());
        stream.extend_from_slice(&0u32.to_le_bytes()); // reserved0
        stream.extend_from_slice(&0u64.to_le_bytes()); // offset_bytes
        stream.extend_from_slice(&(pos_bytes.len() as u64).to_le_bytes()); // size_bytes
        end_cmd(&mut stream, start);

        // RESOURCE_DIRTY_RANGE (full VB_COLOR)
        let start = begin_cmd(&mut stream, OPCODE_RESOURCE_DIRTY_RANGE);
        stream.extend_from_slice(&VB_COLOR.to_le_bytes());
        stream.extend_from_slice(&0u32.to_le_bytes()); // reserved0
        stream.extend_from_slice(&0u64.to_le_bytes()); // offset_bytes
        stream.extend_from_slice(&(color_bytes.len() as u64).to_le_bytes()); // size_bytes
        end_cmd(&mut stream, start);
        ...
        // SET_VERTEX_BUFFERS (slot 0 -> VB_POS)
        ...
        stream.extend_from_slice(&VB_POS.to_le_bytes());
        stream.extend_from_slice(&(std::mem::size_of::<PosVertex>() as u32).to_le_bytes()); // stride
        stream.extend_from_slice(&0u32.to_le_bytes()); // offset
        stream.extend_from_slice(&0u32.to_le_bytes()); // reserved0
        end_cmd(&mut stream, start);

        // SET_VERTEX_BUFFERS (slot 15 -> VB_COLOR)
        let start = begin_cmd(&mut stream, OPCODE_SET_VERTEX_BUFFERS);
        stream.extend_from_slice(&15u32.to_le_bytes()); // start_slot
        stream.extend_from_slice(&1u32.to_le_bytes()); // buffer_count
        stream.extend_from_slice(&VB_COLOR.to_le_bytes());
        stream.extend_from_slice(&(std::mem::size_of::<ColorVertex>() as u32).to_le_bytes()); // stride
        stream.extend_from_slice(&0u32.to_le_bytes()); // offset
        stream.extend_from_slice(&0u32.to_le_bytes()); // reserved0
        end_cmd(&mut stream, start);
