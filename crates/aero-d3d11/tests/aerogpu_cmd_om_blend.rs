...
fn build_stream(blend_constant: [f32; 4], sample_mask: u32) -> Vec<u8> {
    const VB: u32 = 1;
    const RT: u32 = 2;
    const VS: u32 = 10;
    const PS: u32 = 11;
    ...
    for c in blend_constant {
        stream.extend_from_slice(&c.to_bits().to_le_bytes());
    }
    stream.extend_from_slice(&sample_mask.to_le_bytes());
    end_cmd(&mut stream, start);
...
#[test]
fn aerogpu_cmd_blend_factor_constant() {
    pollster::block_on(async {
        let mut exec = match AerogpuD3d11Executor::new_for_tests().await {
            Ok(exec) => exec,
            Err(e) => {
                eprintln!("wgpu unavailable ({e:#}); skipping blend_factor_constant test");
                return;
            }
        };
...
#[test]
fn aerogpu_cmd_sample_mask_discards_draw() {
    pollster::block_on(async {
        let mut exec = match AerogpuD3d11Executor::new_for_tests().await {
            Ok(exec) => exec,
            Err(e) => {
                eprintln!("wgpu unavailable ({e:#}); skipping sample_mask test");
                return;
            }
        };
