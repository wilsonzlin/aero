...
fn d3d11_runtime_triangle_strip_draw_indexed_restart_after_copy_buffer_to_buffer_u16() {
    pollster::block_on(async {
        let test_name = concat!(
            module_path!(),
            "::d3d11_runtime_triangle_strip_draw_indexed_restart_after_copy_buffer_to_buffer_u16"
        );
        ...
        // Same vertex setup as the baseline u16 test so the restart index is in-bounds if restart
        // is broken, making the gap pixel check deterministic.
        let mut vertices = vec![VertexPos2 { pos: [0.0; 2] }; 65_536];
        vertices[0] = VertexPos2 { pos: [-0.9, -0.5] };
        vertices[1] = VertexPos2 { pos: [-0.1, -0.5] };
        vertices[2] = VertexPos2 { pos: [-0.5, 0.5] };
        ...
        // Upload indices into a staging buffer first, then copy into the actual index buffer.
        // This validates that primitive-restart emulation sees updated index data after
        // CopyBufferToBuffer.
        w.create_buffer(
            SRC,
            std::mem::size_of_val(&indices) as u64,
        ...
        w.update_buffer(SRC, 0, bytemuck::cast_slice(&indices));
        ...
        w.copy_buffer_to_buffer(SRC, 0, IB, 0, std::mem::size_of_val(&indices) as u64);
...
#[test]
fn d3d11_runtime_triangle_strip_draw_indexed_restart_after_copy_buffer_to_buffer_u32() {
    pollster::block_on(async {
        let test_name = concat!(
            module_path!(),
            "::d3d11_runtime_triangle_strip_draw_indexed_restart_after_copy_buffer_to_buffer_u32"
        );
        ...
        // Triangle strip indices with a primitive-restart value between strips.
        //
        // As in the baseline Uint32 restart test, we rely on robust out-of-bounds access if
        // primitive restart is broken.
        let indices: [u32; 7] = [0, 1, 2, 0xFFFF_FFFF, 3, 4, 5];
        ...
        w.set_pipeline(PipelineKind::Render, PIPE);
        w.set_vertex_buffer(0, VB, 0);
        w.set_index_buffer(IB, IndexFormat::Uint32, 0);
        w.draw_indexed(7, 1, 0, 0, 0);
        w.end_render_pass();
