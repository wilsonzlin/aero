...
fn aerogpu_cmd_depth_only_compute_prepass_writes_depth() {
    pollster::block_on(async {
        ...
        // Use an adjacency topology to force the compute-prepass path. Temporarily unbind the input
        // layout so the placeholder prepass does not attempt vertex pulling (which can exceed
        // storage-buffer limits on some adapters).
        writer.set_input_layout(0);
        ...
        // TriangleListAdj consumes 6 vertices per primitive; request a single primitive so the
        // placeholder prepass emits its fullscreen triangle.
        writer.draw(6, 1, 0, 0);
