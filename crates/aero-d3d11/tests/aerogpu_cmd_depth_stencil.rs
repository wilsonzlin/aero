...
#[test]
fn aerogpu_cmd_scissor_rect_is_clamped_to_render_target() {
    pollster::block_on(async {
        let mut exec = match AerogpuD3d11Executor::new_for_tests().await {
            Ok(exec) => exec,
            Err(e) => {
        ...
        // SET_SCISSOR (3,3)-(7,7) => clamped to a 1x1 rect at the bottom-right pixel.
        let start = begin_cmd(&mut stream, AerogpuCmdOpcode::SetScissor as u32);
        stream.extend_from_slice(&3i32.to_le_bytes());
        stream.extend_from_slice(&3i32.to_le_bytes());
        stream.extend_from_slice(&4i32.to_le_bytes());
        stream.extend_from_slice(&4i32.to_le_bytes());
        end_cmd(&mut stream, start);
...
#[test]
fn aerogpu_cmd_viewport_is_clamped_to_render_target() {
    pollster::block_on(async {
        let mut exec = match AerogpuD3d11Executor::new_for_tests().await {
            Ok(exec) => exec,
            Err(e) => {
        ...
        // VIEWPORT (-2,-2)-(2,2) => clamped to a 2x2 region at the top-left.
        let start = begin_cmd(&mut stream, AerogpuCmdOpcode::SetViewport as u32);
