...
fn build_gs_instance_colored_split(count: u32) -> Vec<u8> {
    ...
    tokens.push(opcode_token(sm4_opcode::OPCODE_DCL_GS_INSTANCE_COUNT, 2));
    tokens.push(count);
    ...
    // Instance 0 => red, instance 1 => green.
    ...
    // Emit a triangle. For GS instancing, `gs_instance_id` shifts the triangle horizontally so
    // both instances are visible.
    //
...
fn aerogpu_cmd_supports_gs_instance_count() {
    pollster::block_on(async {
        ...
        // One point input with a GS that emits two triangles (one per GS instance).
        #[repr(C)]
        ...
        writer.create_shader_dxbc_ex(
            ...
            &build_gs_instance_colored_split(2),
        );
        ...
        // Instance 0 (left) should be red.
        assert_eq!(px(width / 4, height / 2), [255, 0, 0, 255]);
        // Instance 1 (right) should be green.
        assert_eq!(px(width * 3 / 4, height / 2), [0, 255, 0, 255]);
        // Corners should remain clear.
