...
fn imm32_scalar(value_bits: u32) -> Vec<u32> {
    vec![
        operand_token(
            OPERAND_TYPE_IMMEDIATE32,
            1,
...
fn build_gs_point_to_triangle_color_from_structured_srv_t3() -> Vec<u8> {
    // gs_4_0 that:
    // - Declares point input + triangle strip output + maxvertexcount=3.
    // - Declares + reads a structured SRV buffer at t3 via ld_structured.
    ...
    // dcl_resource_structured t3, stride=16
    let t3 = reg_src(
    ...
    tokens.push(opcode_token(
        ...
        (1 + t3.len() as u32 + 1) as u32,
    ));
    tokens.extend_from_slice(&t3);
    tokens.push(16); // stride_bytes

    // ld_structured r0.xyzw, l(1), l(0), t3
    let index = imm32_scalar(1);
    let offset = imm32_scalar(0);
    let mut ld = vec![opcode_token(
        OPCODE_LD_STRUCTURED,
        (1 + 2 + index.len() as u32 + offset.len() as u32 + t3.len() as u32) as u32,
    )];
    ld.extend_from_slice(&reg_dst(OPERAND_TYPE_TEMP, 0, WriteMask::XYZW));
    ld.extend_from_slice(&index);
    ld.extend_from_slice(&offset);
    ld.extend_from_slice(&t3);
    tokens.extend_from_slice(&ld);
...
#[test]
fn aerogpu_cmd_geometry_shader_reads_structured_srv_buffer_translated_prepass() {
    pollster::block_on(async {
        let test_name = concat!(
            module_path!(),
            "::aerogpu_cmd_geometry_shader_reads_structured_srv_buffer_translated_prepass"
        );
        ...
        // Two structured elements (stride=16):
        //   element 0 = blue
        //   element 1 = green (this is what the shader loads via `ld_structured index=1`)
        let elem0: [u32; 4] = [
            0.0f32.to_bits(),
            0.0f32.to_bits(),
            1.0f32.to_bits(),
        ...
        let srv_words: [u32; 8] = [
            elem0[0], elem0[1], elem0[2], elem0[3], elem1[0], elem1[1], elem1[2], elem1[3],
        ];
        ...
        writer.create_shader_dxbc_ex(
            ...
            &build_gs_point_to_triangle_color_from_structured_srv_t3(),
        );
        ...
        //
        // The shader declares t3, so bind it starting at slot 3.
        ...
        // The structured SRV buffer element 1 contains green, so the triangle should render green
        // at the center pixel.
