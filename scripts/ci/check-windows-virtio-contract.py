...
def _check_profiles_against_contract(
    *,
    ...
    virtio_bars_re = re.compile(
        r"pub const VIRTIO_BARS:\s*\[PciBarProfile;\s*\d+\]\s*=\s*\[\s*PciBarProfile::mem(?P<bits>32|64)\(\s*(?P<index>[A-Z0-9_]+|0x[0-9a-fA-F_]+|\d+)\s*,\s*(?P<size>[A-Z0-9_]+|0x[0-9a-fA-F_]+|\d+)\s*,\s*(?P<pref>true|false)\s*(?:,\s*)?\)\s*(?:,\s*)?\]\s*;"
    )
