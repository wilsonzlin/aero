...
- `aerogpu_dbgctl --query-version` (alias: `--query-device`)  
  ...
  Also prints a fence snapshot, a scanout0 snapshot (cached vs MMIO), a scanout0 vblank timing snapshot (when available), and a
  short CreateAllocation trace summary (`write_index` / `entry_count`).

- `aerogpu_dbgctl --status` *(alias for `--query-version`)*  
  Prints a short combined snapshot (device/ABI + fences + scanout0 vblank + CreateAllocation trace summary).
