...
static int RunD3D11TextureSamplingSanity(int argc, char** argv) {
  ...
  if (!aerogpu_test::CompileHlslToBytecode(kTextureHlsl,
                                           strlen(kTextureHlsl),
                                           "d3d11_texture_sampling_sanity.hlsl",
                                           "vs_main",
                                           "vs_4_0",
                                           &vs_bytes,
                                           &shader_err)) {
  ...
  if (FAILED(hr)) {
    return reporter.FailHresult("CreateTexture2D(src texture)", hr);
  }
  ...
  if (dump) {
    const std::wstring bmp_path = aerogpu_test::JoinPath(dir, L"d3d11_texture_sampling_sanity.bmp");
    std::string err;
    ...
    DumpBytesToFile(kTestName,
                    &reporter,
                    L"d3d11_texture_sampling_sanity.bin",
                    &tight[0],
                    (UINT)tight.size());
  }
