...
static int RunD3D11GeometryShaderRestartStrip(int argc, char** argv) {
  const char* kTestName = "d3d11_geometry_shader_restart_strip";
  ...
  if (!aerogpu_test::CompileHlslToBytecode(kRestartStripHlsl,
                                           strlen(kRestartStripHlsl),
                                           "d3d11_geometry_shader_restart_strip.hlsl",
                                           "vs_main",
                                           "vs_4_0",
                                           &vs_bytes,
                                           &shader_err)) {
  ...
  if (dump) {
    DumpBytesToFile(kTestName,
                    &reporter,
                    L"d3d11_geometry_shader_restart_strip_vs.dxbc",
                    &vs_bytes[0],
                    (UINT)vs_bytes.size());
    DumpBytesToFile(kTestName,
                    &reporter,
                    L"d3d11_geometry_shader_restart_strip_gs.dxbc",
                    &gs_bytes[0],
                    (UINT)gs_bytes.size());
    DumpBytesToFile(kTestName,
                    &reporter,
                    L"d3d11_geometry_shader_restart_strip_ps.dxbc",
                    &ps_bytes[0],
                    (UINT)ps_bytes.size());
  }
  ...
  // Disable culling so the test detects strip-conversion issues even when triangle winding/parity
  // is wrong (the bug we're chasing is missing strip cuts, not rasterizer cull state).
  ...
  if (FAILED(hr)) {
    return reporter.FailHresult("CreateRasterizerState", hr);
  }
  ...
  if (dump) {
    const std::wstring bmp_path = aerogpu_test::JoinPath(dir, L"d3d11_geometry_shader_restart_strip.bmp");
    std::string err;
    ...
    for (int yy = 0; yy < kHeight; ++yy) {
      const uint8_t* src_row = (const uint8_t*)map.pData + (size_t)yy * (size_t)map.RowPitch;
      memcpy(&tight[(size_t)yy * (size_t)kWidth * 4u], src_row, (size_t)kWidth * 4u);
    }
    DumpBytesToFile(
        kTestName, &reporter, L"d3d11_geometry_shader_restart_strip.bin", &tight[0], (UINT)tight.size());
  }

  context->Unmap(staging.get(), 0);

  if ((left_px & 0x00FFFFFFu) != (expected_left & 0x00FFFFFFu)) {
    PrintDeviceRemovedReasonIfAny(kTestName, device.get());
    return reporter.Fail("left quad pixel mismatch: got 0x%08lX expected ~0x%08lX",
                         (unsigned long)left_px,
                         (unsigned long)expected_left);
  }
  if ((right_px & 0x00FFFFFFu) != (expected_right & 0x00FFFFFFu)) {
    PrintDeviceRemovedReasonIfAny(kTestName, device.get());
    return reporter.Fail("right quad pixel mismatch: got 0x%08lX expected ~0x%08lX",
