...
static int RunD3D10RSOMStateSanity(int argc, char** argv) {
  const char* kTestName = "d3d10_rs_om_state_sanity";
  ...
  // This test is specifically intended to exercise the D3D10 runtime path (d3d10.dll).
  if (!GetModuleHandleW(L"d3d10.dll")) {
    return reporter.Fail("d3d10.dll is not loaded");
  }

  aerogpu_test::PrintfStdout("INFO: %s: D3D10 device created", kTestName);
  ...
  if (require_umd || (!allow_microsoft && !allow_non_aerogpu)) {
    ...
    // This test is explicitly intended to cover the D3D10 UMD entrypoint path (`OpenAdapter10`).
  ...
  // Blend states.
  ComPtr<ID3D10BlendState> alpha_blend;
  ComPtr<ID3D10BlendState> green_write_mask;
  ComPtr<ID3D10BlendState> blend_factor_state;
  {
    ...
    hr = device->CreateBlendState(&desc, alpha_blend.put());
    if (FAILED(hr)) {
    ...
    desc.RenderTargetWriteMask[0] = D3D10_COLOR_WRITE_ENABLE_GREEN;
    ...
    desc.DestBlendAlpha = D3D10_BLEND_ZERO;
    desc.BlendOpAlpha = D3D10_BLEND_OP_ADD;
    hr = device->CreateBlendState(&desc, green_write_mask.put());
    if (FAILED(hr)) {
    ...
    desc.DestBlendAlpha = D3D10_BLEND_ZERO;
    desc.BlendOpAlpha = D3D10_BLEND_OP_ADD;
    hr = device->CreateBlendState(&desc, blend_factor_state.put());
    if (FAILED(hr)) {
  ...
  // Subtest 1: Scissor enable.
  {
    ...
    int rb = Readback(NULL,
                      L"d3d10_rs_om_state_sanity_scissor.bmp",
                      L"d3d10_rs_om_state_sanity_scissor.bin",
                      &center,
                      &corner);
    if (rb != 0) {
      return rb;
  ...
  // Subtest 2: Cull mode + FrontCounterClockwise.
  {
    ...
    uint32_t center = 0;
    int rb = Readback(NULL,
                      L"d3d10_rs_om_state_sanity_cull_cw.bmp",
                      L"d3d10_rs_om_state_sanity_cull_cw.bin",
                      &center,
                      NULL);
    if (rb != 0) {
      return rb;
    ...
    uint32_t center2 = 0;
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_cull_ccw.bmp",
                  L"d3d10_rs_om_state_sanity_cull_ccw.bin",
                  &center2,
                  NULL);
    if (rb != 0) {
      return rb;
  ...
  // Subtest 3: DepthClipEnable.
  {
    ...
    uint32_t center = 0;
    int rb = Readback(NULL,
                      L"d3d10_rs_om_state_sanity_depth_clip_disabled.bmp",
                      L"d3d10_rs_om_state_sanity_depth_clip_disabled.bin",
                      &center,
                      NULL);
    if (rb != 0) {
      return rb;
    ...
    uint32_t center2 = 0;
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_depth_clip_null_state.bmp",
                  L"d3d10_rs_om_state_sanity_depth_clip_null_state.bin",
                  &center2,
                  NULL);
    if (rb != 0) {
      return rb;
  ...
  // Subtest 4: Blend state.
  {
    ...
    uint32_t center = 0;
    int rb = Readback(NULL,
                      L"d3d10_rs_om_state_sanity_blend.bmp",
                      L"d3d10_rs_om_state_sanity_blend.bin",
                      &center,
                      NULL);
    if (rb != 0) {
      return rb;
    ...
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_blend_disabled.bmp",
                  L"d3d10_rs_om_state_sanity_blend_disabled.bin",
                  &center_disabled,
                  NULL);
    if (rb != 0) {
      return rb;
    ...
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_write_mask.bmp",
                  L"d3d10_rs_om_state_sanity_write_mask.bin",
                  &center_mask,
                  NULL);
    if (rb != 0) {
      return rb;
    ...
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_blend_factor.bmp",
                  L"d3d10_rs_om_state_sanity_blend_factor.bin",
                  &center_bf,
                  NULL);
    if (rb != 0) {
      return rb;
    ...
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_sample_mask_0.bmp",
                  L"d3d10_rs_om_state_sanity_sample_mask_0.bin",
                  &center_sm0,
                  NULL);
    if (rb != 0) {
      return rb;
  ...
  // Subtest 5: Depth/stencil state (depth func).
  {
    ...
    int rb = Readback(dsv.get(),
                      L"d3d10_rs_om_state_sanity_depth_less.bmp",
                      L"d3d10_rs_om_state_sanity_depth_less.bin",
                      &center,
                      NULL);
    if (rb != 0) {
      return rb;
    ...
    uint32_t center2 = 0;
    rb = Readback(dsv.get(),
                  L"d3d10_rs_om_state_sanity_depth_greater.bmp",
                  L"d3d10_rs_om_state_sanity_depth_greater.bin",
                  &center2,
                  NULL);
    if (rb != 0) {
      return rb;
  ...
  // ClearState call, causing clipped/incorrect rendering.
  {
    ...
    uint32_t corner = 0;
    int rb = Readback(NULL,
                      L"d3d10_rs_om_state_sanity_clear_state.bmp",
                      L"d3d10_rs_om_state_sanity_clear_state.bin",
                      &center,
                      &corner);
    if (rb != 0) {
      return rb;
    ...
    if (dump) {
      const std::wstring bmp_path =
          aerogpu_test::JoinPath(dir, L"d3d10_rs_om_state_sanity_clear_state_topology.bmp");
      std::string err;
      ...
      DumpTightBgra32(kTestName,
                      &reporter,
                      L"d3d10_rs_om_state_sanity_clear_state_topology.bin",
                      topo_map.pData,
                      topo_map.RowPitch,
                      kWidth,
                      kHeight);
    ...
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_clear_state_cull.bmp",
                  L"d3d10_rs_om_state_sanity_clear_state_cull.bin",
                  &cull_center,
                  NULL);
    if (rb != 0) {
      return rb;
    ...
    rb = Readback(NULL,
                  L"d3d10_rs_om_state_sanity_clear_state_depth_clip.bmp",
                  L"d3d10_rs_om_state_sanity_clear_state_depth_clip.bin",
                  &depth_clip_center,
                  NULL);
    if (rb != 0) {
      return rb;
  ...
  // Subtest 7: ClearState resets depth-stencil state.
  {
    ...
    int rb = Readback(dsv.get(),
                      L"d3d10_rs_om_state_sanity_clear_state_depth_before.bmp",
                      L"d3d10_rs_om_state_sanity_clear_state_depth_before.bin",
                      &before_clear,
                      NULL);
    if (rb != 0) {
      return rb;
    ...
    rb = Readback(dsv.get(),
                  L"d3d10_rs_om_state_sanity_clear_state_depth_after.bmp",
                  L"d3d10_rs_om_state_sanity_clear_state_depth_after.bin",
                  &after_clear,
                  NULL);
    if (rb != 0) {
      return rb;
...
int main(int argc, char** argv) {
  ...
  return RunD3D10RSOMStateSanity(argc, argv);
}
