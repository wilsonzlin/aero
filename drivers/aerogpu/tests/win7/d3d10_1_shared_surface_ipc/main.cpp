...
static int CreateD3D10_1Device(aerogpu_test::TestReporter* reporter,
                               const char* test_name,
                               ComPtr<ID3D10Device1>* out_device) {
  if (!out_device) {
    return reporter ? reporter->Fail("CreateD3D10_1Device: out_device is NULL") : 1;
  }
  ...
  if (FAILED(hr)) {
    return FailD3D10WithRemovedReason(reporter, test_name, "D3D10CreateDevice1(HARDWARE)", hr, NULL);
  }

  // Sanity-check the negotiated level.
  const D3D10_FEATURE_LEVEL1 actual_level = device->GetFeatureLevel();
  if (actual_level != chosen_level) {
    return reporter ? reporter->Fail("ID3D10Device1::GetFeatureLevel returned 0x%04X (expected 0x%04X)",
  ...
  if (!GetModuleHandleW(L"d3d10_1.dll")) {
    return reporter ? reporter->Fail("d3d10_1.dll is not loaded") : 1;
  }
...
static int RunConsumer(int argc, char** argv) {
  const char* kTestName = "d3d10_1_shared_surface_ipc_consumer";
  ...
  const std::wstring dump_bmp_path =
      aerogpu_test::JoinPath(aerogpu_test::GetModuleDir(), L"d3d10_1_shared_surface_ipc.bmp");
  ...
  if (has_expected_debug_token) {
    uint32_t token = 0;
    std::string map_err;
    if (!aerogpu_test::kmt::MapSharedHandleDebugTokenPrimary(shared_handle, &token, &map_err)) {
  ...
  ComPtr<ID3D10Device1> device;
  int rc = CreateD3D10_1Device(&reporter, kTestName, &device);
  if (rc != 0) {
    return rc;
  }
  ...
  if (FAILED(hr)) {
    ...
    if (FAILED(hr_res)) {
      return reporter.FailHresult("OpenSharedResource(ID3D10Texture2D/ID3D10Resource)", hr_res);
    }
...
static int RunProducer(int argc, char** argv) {
  const char* kTestName = "d3d10_1_shared_surface_ipc";
  if (aerogpu_test::HasHelpArg(argc, argv)) {
    aerogpu_test::PrintfStdout(
  ...
  std::string map_err;
  const bool have_debug_token = aerogpu_test::kmt::MapSharedHandleDebugTokenPrimary(shared, &debug_token, &map_err);
  if (have_debug_token) {
