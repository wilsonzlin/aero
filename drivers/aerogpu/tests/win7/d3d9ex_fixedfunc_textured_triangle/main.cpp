...
static HRESULT CreateTestTexture2x2(IDirect3DDevice9Ex* dev, IDirect3DTexture9** out_tex) {
  ...
  // Distinct colors; we sample the bottom-right texel.
...
static int RunD3D9ExFixedFuncTexturedTriangle(int argc, char** argv) {
  const char* kTestName = "d3d9ex_fixedfunc_textured_triangle";
  ...
  HWND hwnd = aerogpu_test::CreateBasicWindow(L"AeroGPU_D3D9ExFixedFuncTexturedTriangle",
                                              L"AeroGPU D3D9Ex FixedFunc Textured Triangle",
                                              kWidth,
                                              kHeight,
                                              !hidden);
  if (!hwnd) {
  ...
  // Set up fixed-function state (D3D9 default texture stage state is modulate).
  ...
  // Cyan vertex color: combined with magenta texel -> blue.
  const DWORD kCyan = D3DCOLOR_XRGB(0, 255, 255);
  ...
  // Magenta texel modulated by cyan vertex color -> blue.
...
int main(int argc, char** argv) {
  ...
  int rc = RunD3D9ExFixedFuncTexturedTriangle(argc, argv);
