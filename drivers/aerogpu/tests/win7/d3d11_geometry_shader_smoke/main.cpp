...
static int RunD3D11GeometryShaderSmoke(int argc, char** argv) {
  ...
  if (chosen_level < D3D_FEATURE_LEVEL_10_0) {
    aerogpu_test::PrintfStdout(
        "SKIP: %s: feature level 0x%04X is below D3D_FEATURE_LEVEL_10_0 (0x%04X)",
        kTestName,
  ...
  const std::wstring vs_path = aerogpu_test::JoinPath(dir, L"d3d11_geometry_shader_smoke_vs.cso");
  const std::wstring gs_path = aerogpu_test::JoinPath(dir, L"d3d11_geometry_shader_smoke_gs.cso");
  const std::wstring ps_path = aerogpu_test::JoinPath(dir, L"d3d11_geometry_shader_smoke_ps.cso");
  ...
  if (!aerogpu_test::ReadFileBytes(gs_path, &gs_bytes, &file_err)) {
    return aerogpu_test::Fail(kTestName, "failed to read %ls: %s", gs_path.c_str(), file_err.c_str());
  }
  ...
  if (FAILED(hr)) {
    return aerogpu_test::FailHresult(kTestName, "CreateGeometryShader", hr);
  }
  ...
  if (dump) {
    std::string err;
    if (!aerogpu_test::WriteBmp32BGRA(aerogpu_test::JoinPath(dir, L"d3d11_geometry_shader_smoke.bmp"),
                                      kWidth,
                                      kHeight,
                                      map.pData,
                                      (int)map.RowPitch,
