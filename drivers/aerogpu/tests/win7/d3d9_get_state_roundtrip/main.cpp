...
static int TestBeginEndStateBlockRoundtrip(aerogpu_test::TestReporter& reporter, const char* test_name, IDirect3DDevice9Ex* dev, const D3DCAPS9& caps, bool strict_checks, int width, int height, const D3DMATRIX& world_a) {
  if (!dev) {
    return reporter.Fail("TestBeginEndStateBlockRoundtrip: device is null");
  }
  ...
  hr = dev->CreateRenderTarget(width, height, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, rt_sb.put(), NULL);
  if (FAILED(hr) || !rt_sb) {
    return reporter.FailHresult("CreateRenderTarget (stateblock rt_sb)", hr);
  }
  ComPtr<IDirect3DSurface9> rt_clobber;
  hr = dev->CreateRenderTarget(width, height, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, rt_clobber.put(), NULL);
  ...
  // Validate restored values.
  {
    ...
    if (clip_status_test) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping StateBlock ClipStatus validate (Get failed hr=0x%08lX)",
                                   test_name,
                                   (unsigned long)hr);
        clip_status_test = false;
      }
      if (clip_status_test && !ClipStatusEqual(got_cs, clip_status_sb)) {
        if (strict_checks) {
          return reporter.Fail("stateblock restore mismatch: ClipStatus");
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping StateBlock ClipStatus validate (mismatch)", test_name);
      }
    }
    ...
    if (palette_test) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping StateBlock PaletteEntries validate (Get failed hr=0x%08lX)",
                                   test_name,
                                   (unsigned long)hr);
        palette_test = false;
      }
      if (palette_test && std::memcmp(got_entries, palette_entries_sb, sizeof(palette_entries_sb)) != 0) {
        if (strict_checks) {
          return reporter.Fail("stateblock restore mismatch: PaletteEntries");
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping StateBlock PaletteEntries validate (mismatch)", test_name);
        palette_test = false;
      }
    }
    ...
    if (current_palette_test) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping StateBlock CurrentTexturePalette validate (Get failed hr=0x%08lX)",
                                   test_name,
                                   (unsigned long)hr);
        current_palette_test = false;
      }
      if (current_palette_test && got != palette_idx_sb) {
        if (strict_checks) {
          return reporter.Fail("stateblock restore mismatch: CurrentTexturePalette got=%u expected=%u",
                               (unsigned)got,
                               (unsigned)palette_idx_sb);
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping StateBlock CurrentTexturePalette validate (mismatch)", test_name);
        current_palette_test = false;
      }
    }
...
static int TestCreateStateBlockVertexRoundtrip(aerogpu_test::TestReporter& reporter, const char* test_name, IDirect3DDevice9Ex* dev, bool strict_checks, const D3DMATRIX& world_a) {
  if (!dev) {
    return reporter.Fail("TestCreateStateBlockVertexRoundtrip: device is null");
  }
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    // needing an explicit Capture() call.
    {
      ...
      if (clip_status_ok) {
        ...
        if (FAILED(hr)) {
          ...
          aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock ClipStatus baseline check (Get failed hr=0x%08lX)",
                                     test_name,
                                     (unsigned long)hr);
          clip_status_ok = false;
        }
        if (clip_status_ok && !ClipStatusEqual(got_cs, clip_status_0)) {
          if (strict_checks) {
            return reporter.Fail("CreateStateBlock baseline mismatch: ClipStatus");
          }
          aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock ClipStatus baseline check (mismatch)", test_name);
          clip_status_ok = false;
        }
      }
    }
    ...
    if (clip_status_ok) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock ClipStatus check (Get failed hr=0x%08lX)",
                                   test_name,
                                   (unsigned long)hr);
        clip_status_ok = false;
      }
      if (clip_status_ok && !ClipStatusEqual(got_cs, clip_status_1)) {
        if (strict_checks) {
          return reporter.Fail("CreateStateBlock restore mismatch: ClipStatus");
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock ClipStatus check (mismatch)", test_name);
        clip_status_ok = false;
      }
    }
...
static int TestCreateStateBlockPixelRoundtrip(aerogpu_test::TestReporter& reporter, const char* test_name, IDirect3DDevice9Ex* dev, bool strict_checks, int width, int height) {
  if (!dev) {
    return reporter.Fail("TestCreateStateBlockPixelRoundtrip: device is null");
  }
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    // Verify CreateStateBlock captured the baseline without needing Capture().
    {
      ...
      if (palette_ok) {
        ...
        if (FAILED(hr)) {
          ...
          aerogpu_test::PrintfStdout("INFO: %s: disabling CreateStateBlock PaletteEntries baseline check (Get failed hr=0x%08lX)",
                                     test_name,
                                     (unsigned long)hr);
          palette_ok = false;
        }
        if (palette_ok && std::memcmp(got_pal, palette_0, sizeof(palette_0)) != 0) {
          if (strict_checks) {
            return reporter.Fail("CreateStateBlock baseline mismatch: PaletteEntries");
          }
          aerogpu_test::PrintfStdout("INFO: %s: disabling CreateStateBlock PaletteEntries baseline check (mismatch)", test_name);
          palette_ok = false;
        }
      }
      ...
      if (current_palette_ok) {
        ...
        if (FAILED(hr)) {
          ...
          aerogpu_test::PrintfStdout(
              "INFO: %s: disabling CreateStateBlock CurrentTexturePalette baseline check (Get failed hr=0x%08lX)",
              test_name,
              (unsigned long)hr);
          current_palette_ok = false;
        }
        if (current_palette_ok && got != current_palette_0) {
          if (strict_checks) {
            return reporter.Fail("CreateStateBlock baseline mismatch: CurrentTexturePalette got=%u expected=%u",
                                 (unsigned)got,
                                 (unsigned)current_palette_0);
          }
          aerogpu_test::PrintfStdout("INFO: %s: disabling CreateStateBlock CurrentTexturePalette baseline check (mismatch)",
                                     test_name);
          current_palette_ok = false;
        }
      }
    ...
    if (palette_ok) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock PaletteEntries validate (Get failed hr=0x%08lX)",
                                   test_name,
                                   (unsigned long)hr);
        palette_ok = false;
      }
      if (palette_ok && std::memcmp(got_pal, palette_1, sizeof(palette_1)) != 0) {
        if (strict_checks) {
          return reporter.Fail("CreateStateBlock restore mismatch: PaletteEntries");
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock PaletteEntries validate (mismatch)", test_name);
        palette_ok = false;
      }
    }
    ...
    if (current_palette_ok) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock CurrentTexturePalette validate (Get failed hr=0x%08lX)",
                                   test_name,
                                   (unsigned long)hr);
        current_palette_ok = false;
      }
      if (current_palette_ok && got != current_palette_1) {
        if (strict_checks) {
          return reporter.Fail("CreateStateBlock restore mismatch: CurrentTexturePalette got=%u expected=%u",
                               (unsigned)got,
                               (unsigned)current_palette_1);
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping CreateStateBlock CurrentTexturePalette validate (mismatch)", test_name);
        current_palette_ok = false;
      }
    }
...
static int RunD3D9GetStateRoundtrip(int argc, char** argv) {
  ...
  // fixed-function path; treat failures as a supported skip in permissive mode.
  {
    ...
    if (FAILED(hr)) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping Set/GetClipStatus (Get failed hr=0x%08lX)",
                                   kTestName,
                                   (unsigned long)hr);
      }
      if (SUCCEEDED(hr) && !ClipStatusEqual(got_cs, set_cs)) {
        if (strict_checks) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping Set/GetClipStatus (mismatch)", kTestName);
      }
    }
  }
  ...
  // D3DERR_INVALIDCALL as a supported skip.
  {
    ...
    if (FAILED(hr)) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping Set/GetPaletteEntries (Get failed hr=0x%08lX)",
                                   kTestName,
                                   (unsigned long)hr);
      }
      if (SUCCEEDED(hr) && std::memcmp(entries_got, entries_set, sizeof(entries_set)) != 0) {
        if (strict_checks) {
          return reporter.Fail("GetPaletteEntries mismatch");
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping Set/GetPaletteEntries (mismatch)", kTestName);
      }
    }
    ...
    if (FAILED(hr)) {
      ...
      if (FAILED(hr)) {
        ...
        aerogpu_test::PrintfStdout("INFO: %s: skipping Set/GetCurrentTexturePalette (Get failed hr=0x%08lX)",
                                   kTestName,
                                   (unsigned long)hr);
      }
      if (SUCCEEDED(hr) && got != kPalette) {
        if (strict_checks) {
          return reporter.Fail("GetCurrentTexturePalette mismatch: got=%u expected=%u", (unsigned)got, (unsigned)kPalette);
        }
        aerogpu_test::PrintfStdout("INFO: %s: skipping Set/GetCurrentTexturePalette (mismatch)", kTestName);
      }
    }
  }
  ...
  // ---------------------------------------------------------------------------
  {
    const int rc = TestBeginEndStateBlockRoundtrip(reporter, kTestName, dev.get(), caps, strict_checks, kWidth, kHeight, world_a);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  // ---------------------------------------------------------------------------
  {
    const int rc = TestCreateStateBlockVertexRoundtrip(reporter, kTestName, dev.get(), strict_checks, world_a);
    if (rc != 0) {
      return rc;
    }
  }
  ...
  // ---------------------------------------------------------------------------
  {
    const int rc = TestCreateStateBlockPixelRoundtrip(reporter, kTestName, dev.get(), strict_checks, kWidth, kHeight);
    if (rc != 0) {
      return rc;
    }
  }
