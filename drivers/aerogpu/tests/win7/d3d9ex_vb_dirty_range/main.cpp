...
static int RunD3D9ExVertexBufferDirtyRange(int argc, char** argv) {
  const char* kTestName = "d3d9ex_vb_dirty_range";
  ...
  Vertex verts[3];
  // Same triangle as d3d9ex_triangle: center pixel covered; top-left corner left untouched.
  ...
  hr = vb->Unlock();
  if (FAILED(hr)) {
    return aerogpu_test::FailHresult(kTestName, "IDirect3DVertexBuffer9::Unlock", hr);
  }
  ...
  if (FAILED(hr)) {
    dev->EndScene();
    return aerogpu_test::FailHresult(kTestName, "IDirect3DDevice9Ex::SetStreamSource", hr);
  }
...
int main(int argc, char** argv) {
  ...
  int rc = RunD3D9ExVertexBufferDirtyRange(argc, argv);
  Sleep(30);
  return rc;
}
