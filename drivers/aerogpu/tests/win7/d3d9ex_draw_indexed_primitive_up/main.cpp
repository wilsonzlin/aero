...
static int RunD3D9ExDrawIndexedPrimitiveUP(int argc, char** argv) {
  const char* kTestName = "d3d9ex_draw_indexed_primitive_up";
  ...
  HWND hwnd = aerogpu_test::CreateBasicWindow(L"AeroGPU_D3D9ExDrawIndexedPrimitiveUP",
                                              L"AeroGPU D3D9Ex DrawIndexedPrimitiveUP",
                                              kWidth,
                                              kHeight,
                                              !hidden);
  if (!hwnd) {
  ...
  hr = dev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST,
                                   /*MinVertexIndex=*/0,
                                   /*NumVertices=*/3,
                                   /*PrimitiveCount=*/1,
                                   indices,
  ...
  if (FAILED(hr)) {
    dev->EndScene();
    return reporter.FailHresult("IDirect3DDevice9Ex::DrawIndexedPrimitiveUP", hr);
  }
  ...
  if ((center & 0x00FFFFFFu) != (expected & 0x00FFFFFFu) ||
    ...
    if (dump) {
      ...
      const std::wstring bmp_path =
          aerogpu_test::JoinPath(aerogpu_test::GetModuleDir(), L"d3d9ex_draw_indexed_primitive_up.bmp");
      ...
      DumpTightBgra32(kTestName,
                      &reporter,
                      L"d3d9ex_draw_indexed_primitive_up.bin",
                      lr.pBits,
                      (int)lr.Pitch,
                      (int)desc.Width,
                      (int)desc.Height);
...
int main(int argc, char** argv) {
  ...
  int rc = RunD3D9ExDrawIndexedPrimitiveUP(argc, argv);
