...
* `d3d9_get_state_roundtrip` validates D3D9Ex fixed-function state caching and getter/setter roundtrips (`GetRenderState`, `GetSamplerState`, `GetViewport`, `GetTransform`, `GetTextureStageState`) and exercises `IDirect3DStateBlock9` record/apply behavior (expects all queried values to round-trip exactly)
