...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WORLD/VIEW/PROJECTION transforms for textured geometry by applying non-identity WORLD/VIEW/PROJECTION transforms (translation split across W/V/P) and confirming the center pixel samples the expected texel (**yellow**) via readback
...
* `d3d9_process_vertices_smoke` calls `IDirect3DDevice9::ProcessVertices` with non-zero `SrcStartIndex`/`DestIndex`, then draws from the processed vertex buffer and confirms via readback: **corner clear + center blue**, plus sentinel triangles (**green** + **yellow**) to catch `DestIndex` bugs and out-of-bounds writes
