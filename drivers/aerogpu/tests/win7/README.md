...
* `perf_state_sanity` queries the KMD `QUERY_PERF` escape repeatedly and validates basic invariants, including that the appended `flags` field is present and has the `VALID` bit set. On newer KMD builds it also validates appended counters such as GetScanLine telemetry and contig-pool hit/miss/bytes-saved (skips if the escape is not supported)
