...
* `d3d9ex_instancing_sanity` validates basic D3D9Ex instancing via `SetStreamSourceFreq` by drawing two instanced triangles with per-instance offsets/colors and confirming **left red + right green** via backbuffer readback (optional `--dump` writes `d3d9ex_instancing_sanity.bmp`)
