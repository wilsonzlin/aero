...
* `d3d9_fixedfunc_multitexture` validates multi-texture stage chaining: binds two textures and uses stage0 `SELECTARG1(TEXTURE)` then stage1 `MODULATE(TEXTURE, CURRENT)` to produce a deterministic modulated color (point-sampled 1×1 textures); validates the center pixel via readback (supports `--dump` to emit `d3d9_fixedfunc_multitexture.bmp`).
