...
* `d3d9_get_state_roundtrip` validates core D3D9 state setter/getter roundtrips (render states, sampler states, viewport/scissor, transforms, and other `Get*` state) to ensure the UMD returns consistent values after `Set*` calls
...
* `d3d9_get_state_roundtrip` validates D3D9Ex fixed-function state caching and getter/setter roundtrips (`GetRenderState`, `GetSamplerState`, `GetViewport`, `GetTransform`, `GetTextureStageState`) and exercises `IDirect3DStateBlock9` record/apply restore behavior (expects all queried values to round-trip exactly)
