...
* `d3d9_patch_rendering_smoke` exercises the D3D9 patch path (`DrawRectPatch` / `DeletePatch` / `DrawTriPatch`, including handle=0 no-cache paths, cache-hit draw-twice paths, and redraw after `DeletePatch`) and validates that the patch outputs expected colors over a clear background via backbuffer readback (skips if patch caps are not advertised)
