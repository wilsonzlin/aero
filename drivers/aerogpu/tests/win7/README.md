...
* `d3d9_fixedfunc_lighting_directional` validates fixed-function directional lighting by rendering the same geometry twice with opposite `D3DLIGHT_DIRECTIONAL` directions and requiring that the center pixel is significantly brighter for the “lit” direction and near-black for the “dark” direction (failure implies broken fixed-function lighting/normal handling; supports `--dump` to emit `d3d9_fixedfunc_lighting_directional_{lit,dark}.bmp`)
