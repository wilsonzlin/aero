...
* `d3d9_caps_smoke` validates a baseline set of D3D9Ex `GetDeviceCaps` fields and capability bits (shader versions, resource sharing, scissor test, etc.) and fails fast on unexpected caps that could break common apps
...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WVP transforms for textured geometry by drawing an off-center textured quad via a `D3DTS_WORLD` translation and confirming the expected texel appears at the screen center
* `d3d9_shader_stage_interop` validates D3D9 stage binding interop by drawing with a user vertex shader + **NULL pixel shader** and confirming fixed-function stage0 texture sampling still works (expects **corner red + center magenta** via readback)
