...
* `dbgctl_selftest_sanity` runs the KMD dbgctl selftest escape (`AEROGPU_ESCAPE_OP_SELFTEST`) and expects PASS; if the adapter is busy (`GPU_BUSY`), retries briefly and then skips (also skips if the escape is not supported)
