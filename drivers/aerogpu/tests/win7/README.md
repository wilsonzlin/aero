...
* `d3d9_fixedfunc_wvp_triangle` draws two fixed-function D3D9 triangles and validates WORLD/VIEW/PROJECTION transform application + stateblock restore: a first **blue** triangle covers the center pixel, then a stateblock applies a WORLD+PROJECTION transform to move the second **green** triangle to the right (expects **corner red + center blue + right green** via readback); also exercises `GetPresentStats`
...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WORLD/VIEW/PROJECTION transforms for textured geometry by applying non-identity WORLD/VIEW/PROJECTION transforms (WORLD/VIEW translation plus a PROJECTION scale+translation) and confirming the center pixel samples the expected texel (**yellow**) via readback (covers both `SetFVF` and `SetVertexDeclaration` fixed-function paths)
...
* `d3d9_process_vertices_smoke` calls `IDirect3DDevice9::ProcessVertices` with non-zero `SrcStartIndex`/`DestIndex` (and a non-zero stream offset), then draws from the processed vertex buffer and confirms via readback: **corner clear + center blue**, plus sentinel triangles (**green** + **yellow**) to catch `DestIndex` bugs and out-of-bounds writes (supports `--dump` to emit `*.bmp` and tight-packed BGRA `*.bin`)
