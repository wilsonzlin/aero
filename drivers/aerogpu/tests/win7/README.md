...
* `d3d9_caps_smoke` sanity-checks key `D3DCAPS9` bits reported by `IDirect3DDevice9Ex::GetDeviceCaps` (windowed rendering, shareable resources, shader model level, HW T&L, scissor/compare capabilities, NPOT textures) and asserts patch caps are not unexpectedly advertised (fails fast on unexpected caps that could break common apps)
...
* `perf_state_sanity` queries the KMD `QUERY_PERF` escape repeatedly and validates basic invariants, including that the appended `flags` field is present and has the `VALID` bit set (skips if the escape is not supported)
...
* `dbgctl_selftest_sanity` runs the KMD dbgctl selftest escape (`AEROGPU_ESCAPE_OP_SELFTEST`) and requires it to pass; retries briefly on `GPU_BUSY` and skips if the adapter remains busy (skips if the escape is not supported)
* `alloc_table_readonly_sanity` produces an AGPU submission, reads back its per-submit allocation table via dbgctl `READ_GPA`, and validates that it contains both `AEROGPU_ALLOC_FLAG_READONLY` and writable entries (skips if not supported or not running on an AGPU ring)
...
* `d3d9_patch_rendering_smoke` exercises the D3D9 patch path (`DrawRectPatch`, `DeletePatch`, `DrawTriPatch`, including redraw after `DeletePatch`) and validates that the patch outputs expected colors over a clear background via backbuffer readback (smoke test for patch caching/invalidation paths)
* `d3d9_patch_sanity` probes D3D9 patch caps and (when supported) creates and renders a cubic Bezier tri/rect patch and validates **corner red + center blue** via readback (otherwise the test is skipped)
...
* `d3d11_geometry_shader_restart_strip` validates `TriangleStream.RestartStrip()` (`cut`) semantics when the GS outputs multiple triangle strips (two quads emitted from a single invocation) that must be converted into a triangle list (expects **two disjoint quads** with the center gap remaining clear)
