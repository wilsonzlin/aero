...
* `vblank_state_sanity` queries the KMD `QUERY_VBLANK` escape repeatedly and validates monotonic vblank sequence/timestamps and that the measured cadence roughly matches the reported `vblank_period_ns` (`--samples=N`, `--interval-ms=N`)
