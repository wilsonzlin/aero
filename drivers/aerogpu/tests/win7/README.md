...
* `d3d9ex_fixedfunc_state_roundtrip` validates D3D9Ex fixed-function state roundtrips (`SetTransform`/`GetTransform`, `SetTextureStageState`/`GetTextureStageState`) and that `IDirect3DStateBlock9::Apply` restores the recorded values exactly
...
* `d3d9ex_fixedfunc_state_roundtrip` validates D3D9Ex fixed-function state caching and `Set*`/`Get*` roundtrips (`GetTransform`, `GetTextureStageState`) and exercises `IDirect3DStateBlock9` record/apply restore behavior for these cached states (no rendering required)
