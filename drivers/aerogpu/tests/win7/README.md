...
* `modeset_roundtrip_sanity` exercises basic mode-setting via VidPn commit and validates the committed mode can be queried back consistently (catches missing/broken `DxgkDdiCommitVidPn` bookkeeping; skipped under RDP unless `--allow-remote` is set)
