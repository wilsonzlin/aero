...
* `d3d9_mipmapped_texture_smoke` uploads a small mipmapped texture, renders with it, and validates mip selection/sampling via readback; it also validates per-mip `LockRect` pitch reporting on a SYSTEMMEM mip chain and the `UpdateTexture(SYSTEMMEM→DEFAULT)` upload path (sanity-check for basic D3D9 mip upload + sampling paths)
