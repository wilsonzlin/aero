...
* `d3d9_dynamic_vb_lock_semantics` exercises D3D9 dynamic vertex buffer `Lock(DISCARD)`/`Lock(NOOVERWRITE)` semantics by drawing multiple triangles and validating readback colors (catches regressions where DISCARD overwrites in-flight data or NOOVERWRITE corrupts previously written vertices)
