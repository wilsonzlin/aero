...
* `d3d9_fixedfunc_vertexdecl_patterns` validates fixed-function vertex declaration element mapping/order by drawing a textured quad using a non-canonical `IDirect3DVertexDeclaration9` element ordering (TEXCOORD before POSITION/COLOR) and stage0 `D3DTOP_MODULATE` (TEXTURE×DIFFUSE), then confirming the expected 2×2 texture pattern appears at four sample points via readback (supports `--dump` to write `d3d9_fixedfunc_vertexdecl_patterns.bmp` and `d3d9_fixedfunc_vertexdecl_patterns.bin`)
...
* `d3d9_fixedfunc_lighting_multi_directional` validates fixed-function multi-light accumulation (HW T&L): renders a triangle once with a bright red `D3DLIGHT_DIRECTIONAL` enabled, then again with an additional green directional light, and requires that the center pixel gains significant green (i.e. changes from **red** toward **yellow**) (supports `--dump` to emit `d3d9_fixedfunc_lighting_multi_directional_{red,red_green}.bmp` and `*.bin` dumps for debugging)
* `d3d9_fixedfunc_lighting_point` validates fixed-function point lighting: renders a fullscreen quad with a `D3DLIGHT_POINT` near the center and requires that the center pixel is significantly brighter than a corner pixel (supports `--dump` to emit `d3d9_fixedfunc_lighting_point.bmp` and `d3d9_fixedfunc_lighting_point.bin`)
