...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WORLD/VIEW/PROJECTION transforms for textured geometry by translating a textured quad via `SetTransform(D3DTS_WORLD)` and confirming the center pixel samples the expected texel (**yellow**) via readback
