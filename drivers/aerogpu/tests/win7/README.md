...
* `d3d9_caps_smoke` sanity-checks key `D3DCAPS9` bits reported by `IDirect3DDevice9Ex::GetDeviceCaps` (windowed rendering, shareable resources, shader model level, HW T&L, scissor/compare capabilities) and asserts patch caps are not unexpectedly advertised (fails fast on unexpected caps that could break common apps)
...
* `d3d9_fixedfunc_xyz_diffuse` draws a fixed-function triangle using `D3DFVF_XYZ|D3DFVF_DIFFUSE` (world/view/projection transforms) and validates output via backbuffer readback (expects **corner clear + center diffuse**, within tolerance)
...
* `d3d9_patch_sanity` probes D3D9 patch caps and (when supported) renders a simple Bezier tri-patch and validates **corner red + center blue** via readback (otherwise the test is skipped)
...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WORLD/VIEW/PROJECTION transforms for textured geometry by translating a textured quad via `SetTransform(D3DTS_WORLD)` and confirming the center pixel samples the expected texel (**yellow**)
* `d3d9_shader_stage_interop` validates D3D9 stage binding interop by drawing with a user vertex shader + **NULL pixel shader** while forcing stage0 to select the bound texture (expects **corner clear red + center magenta** via readback)
...
* `d3d11_geometry_shader_restart_strip` validates `TriangleStream.RestartStrip()` (`cut`) semantics when the GS outputs multiple triangle strips that must be converted into a triangle list (expects **two disjoint quads** with the center gap remaining clear)
