...
* `d3d9_fixedfunc_textured_wvp` validates fixed-function WORLD/VIEW/PROJECTION transforms for textured geometry by applying non-identity WORLD/VIEW/PROJECTION transforms (translation split across W/V/P) and confirming the center pixel samples the expected texel (**yellow**) via readback (covers both `SetFVF` and `SetVertexDeclaration` fixed-function paths)
