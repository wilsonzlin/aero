...
* `d3d9_dynamic_vb_lock_semantics` exercises D3D9 dynamic vertex buffer `Lock(D3DLOCK_DISCARD)`/`Lock(D3DLOCK_NOOVERWRITE)` semantics by drawing multiple triangles and validating readback colors (catches regressions where DISCARD overwrites in-flight data or NOOVERWRITE corrupts previously written vertices)
...
* `d3d9_dynamic_vb_lock_semantics` validates D3D9 dynamic vertex-buffer lock semantics: repeated `Lock(D3DLOCK_DISCARD)` updates + draws must not corrupt earlier draws, and `Lock(D3DLOCK_NOOVERWRITE)` appends must preserve previously written vertices (verified via render-target readback)
