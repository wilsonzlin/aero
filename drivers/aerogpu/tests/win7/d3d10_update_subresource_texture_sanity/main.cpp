...
static int RunD3D10UpdateSubresourceTextureSanity(int argc, char** argv) {
  const char* kTestName = "d3d10_update_subresource_texture_sanity";
  ...
  // Also exercise the boxed update path (non-NULL D3D10_BOX).
  ...
  if (dump) {
    ...
    const std::wstring bmp_path =
        aerogpu_test::JoinPath(dir, L"d3d10_update_subresource_texture_sanity.bmp");
    std::string err;
    ...
    DumpTightBgra32(kTestName,
                    &reporter,
                    L"d3d10_update_subresource_texture_sanity.bin",
                    map.pData,
                    map.RowPitch,
                    kWidth,
                    kHeight);
...
int main(int argc, char** argv) {
  ...
  return RunD3D10UpdateSubresourceTextureSanity(argc, argv);
}
