...
static DWORD WINAPI MapThreadProc(LPVOID param) {
  ...
  args->hr = args->ctx->Map(args->tex, 0, D3D11_MAP_READ, args->map_flags, &mapped);
...
static int RunD3D11MapDoNotWait(int argc, char** argv) {
  const char* kTestName = "d3d11_map_do_not_wait";
  ...
  // Use a moderately large surface to increase the likelihood GPU work is still in-flight when we
  // attempt Map(DO_NOT_WAIT).
  ...
  // Map with DO_NOT_WAIT should never block. On typical async drivers it should return
  // DXGI_ERROR_WAS_STILL_DRAWING; if it succeeds immediately that's fine, but it still must
  // return promptly and yield correct pixels.
  {
    MapThreadArgs args;
    ZeroMemory(&args, sizeof(args));
    args.ctx = context.get();
    args.tex = staging.get();
    args.map_flags = D3D11_MAP_FLAG_DO_NOT_WAIT;
    args.hr = E_FAIL;
    context->AddRef();
    staging->AddRef();
    ...
    if (args.hr == DXGI_ERROR_WAS_STILL_DRAWING) {
      ...
      return FailD3D11WithRemovedReason(&reporter, kTestName, "Map(DO_NOT_WAIT)", args.hr, device.get());
    }
  }
  ...
  if (FAILED(args.hr)) {
    return FailD3D11WithRemovedReason(&reporter, kTestName, "Map(READ)", args.hr, device.get());
  }
  if (!args.has_pixel) {
...
int main(int argc, char** argv) {
  ...
  return RunD3D11MapDoNotWait(argc, argv);
}
