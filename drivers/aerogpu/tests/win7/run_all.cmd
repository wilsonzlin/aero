...
echo   --ring-id affects ring_state_sanity: which ring ID to dump (default 0).
  echo   --allow-remote skips tests that are not meaningful in RDP sessions ^(SM_REMOTESESSION=1^): device_state_sanity, segment_budget_sanity, d3d9ex_dwm_probe, d3d9ex_submit_fence_stress, fence_state_sanity, ring_state_sanity, dwm_flush_pacing, vblank_wait, wait_vblank_pacing, vblank_wait_pacing, vblank_wait_sanity, vblank_state_sanity, get_scanline_sanity, scanout_state_sanity, cursor_state_sanity, dump_createalloc_sanity, umd_private_sanity, transfer_feature_sanity, d3d9_raster_status_sanity, d3d9_raster_status_pacing.
