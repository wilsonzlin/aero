...
static int RunD3D9ProcessVerticesSanity(int argc, char** argv) {
  ...
  // Validate Flags=D3DPV_DONOTCOPYDATA for an XYZRHW input without DIFFUSE: only
  // POSITIONT should be written; DIFFUSE bytes must be preserved.
  {
    const VertexXyzrhw src = {10.0f, 20.0f, 0.5f, 2.0f};

    ComPtr<IDirect3DVertexBuffer9> src_vb;
    hr = dev->CreateVertexBuffer(sizeof(src), 0, D3DFVF_XYZRHW, D3DPOOL_DEFAULT, src_vb.put(), NULL);
    if (FAILED(hr) || !src_vb) {
      return reporter.FailHresult("CreateVertexBuffer(src_xyzw_flags)", hr);
    }
    void* src_ptr = NULL;
    hr = src_vb->Lock(0, sizeof(src), &src_ptr, 0);
    if (FAILED(hr) || !src_ptr) {
      return reporter.FailHresult("src_xyzw_flags->Lock", hr);
    }
    memcpy(src_ptr, &src, sizeof(src));
    hr = src_vb->Unlock();
    if (FAILED(hr)) {
      return reporter.FailHresult("src_xyzw_flags->Unlock", hr);
    }
    ...
    hr = dev->CreateVertexBuffer(sizeof(Vertex),
                                 ...
                                 NULL);
    if (FAILED(hr) || !dst_vb) {
      return reporter.FailHresult("CreateVertexBuffer(dst_xyzw_flags)", hr);
    }
    void* dst_init_ptr = NULL;
    hr = dst_vb->Lock(0, sizeof(Vertex), &dst_init_ptr, 0);
    if (FAILED(hr) || !dst_init_ptr) {
      return reporter.FailHresult("dst_xyzw_flags->Lock (init)", hr);
    }
    memset(dst_init_ptr, 0xCD, sizeof(Vertex));
    hr = dst_vb->Unlock();
    if (FAILED(hr)) {
      return reporter.FailHresult("dst_xyzw_flags->Unlock (init)", hr);
    }

    hr = dev->SetFVF(D3DFVF_XYZRHW);
    if (FAILED(hr)) {
      return reporter.FailHresult("SetFVF(XYZRHW) (flags)", hr);
    }
    hr = dev->SetStreamSource(0, src_vb.get(), 0, sizeof(VertexXyzrhw));
    if (FAILED(hr)) {
      return reporter.FailHresult("SetStreamSource(src_xyzw_flags)", hr);
    }
    ...
    if (FAILED(hr)) {
      return reporter.FailHresult("IDirect3DDevice9::ProcessVertices(xyzw flags)", hr);
    }

    void* dst_ptr = NULL;
    hr = dst_vb->Lock(0, sizeof(Vertex), &dst_ptr, D3DLOCK_READONLY);
    if (FAILED(hr) || !dst_ptr) {
      return reporter.FailHresult("dst_xyzw_flags->Lock (read)", hr);
    }

    const Vertex expected = {src.x, src.y, src.z, src.rhw, 0xCDCDCDCDu};
    const bool bytes_match = (memcmp(dst_ptr, &expected, sizeof(expected)) == 0);

    hr = dst_vb->Unlock();
    if (FAILED(hr)) {
      return reporter.FailHresult("dst_xyzw_flags->Unlock (read)", hr);
    }

    if (!bytes_match) {
      return reporter.Fail("ProcessVertices XYZRHW Flags=D3DPV_DONOTCOPYDATA mismatch (non-position bytes were modified)");
    }
  }
  ...
  // deterministic and exactly representable as IEEE floats.
  {
    ...
    // Validate Flags=D3DPV_DONOTCOPYDATA for an XYZ input without DIFFUSE: only
    // POSITIONT should be written; DIFFUSE bytes must be preserved.
    {
      const VertexXyz src_white = {0.0f, 0.0f, 0.0f};
      ...
      hr = dev->CreateVertexBuffer(sizeof(src_white),
                                   ...
                                   NULL);
      if (FAILED(hr) || !src_white_vb) {
        return reporter.FailHresult("CreateVertexBuffer(src_xyz_flags)", hr);
      }

      void* src_white_ptr = NULL;
      hr = src_white_vb->Lock(0, sizeof(src_white), &src_white_ptr, 0);
      if (FAILED(hr) || !src_white_ptr) {
        return reporter.FailHresult("src_xyz_flags->Lock", hr);
      }
      memcpy(src_white_ptr, &src_white, sizeof(src_white));
      hr = src_white_vb->Unlock();
      if (FAILED(hr)) {
        return reporter.FailHresult("src_xyz_flags->Unlock", hr);
      }
      ...
      if (FAILED(hr) || !dst_flags_vb) {
        return reporter.FailHresult("CreateVertexBuffer(dst_xyz_flags)", hr);
      }

      void* dst_flags_init_ptr = NULL;
      hr = dst_flags_vb->Lock(0, sizeof(Vertex), &dst_flags_init_ptr, 0);
      if (FAILED(hr) || !dst_flags_init_ptr) {
        return reporter.FailHresult("dst_xyz_flags->Lock (init)", hr);
      }
      memset(dst_flags_init_ptr, 0xCD, sizeof(Vertex));
      hr = dst_flags_vb->Unlock();
      if (FAILED(hr)) {
        return reporter.FailHresult("dst_xyz_flags->Unlock (init)", hr);
      }

      hr = dev->SetFVF(D3DFVF_XYZ);
      if (FAILED(hr)) {
        return reporter.FailHresult("SetFVF(XYZ) (flags)", hr);
      }
      hr = dev->SetStreamSource(0, src_white_vb.get(), 0, sizeof(VertexXyz));
      if (FAILED(hr)) {
        return reporter.FailHresult("SetStreamSource(src_xyz_flags)", hr);
      }
      ...
      hr = dev->ProcessVertices(/*SrcStartIndex=*/0,
                                ...
                                /*Flags=*/D3DPV_DONOTCOPYDATA);
      if (FAILED(hr)) {
        return reporter.FailHresult("IDirect3DDevice9::ProcessVertices(xyz flags)", hr);
      }

      void* dst_flags_ptr = NULL;
      hr = dst_flags_vb->Lock(0, sizeof(Vertex), &dst_flags_ptr, D3DLOCK_READONLY);
      if (FAILED(hr) || !dst_flags_ptr) {
        return reporter.FailHresult("dst_xyz_flags->Lock (read)", hr);
      }

      const Vertex expected = {0.5f, 0.5f, 0.0f, 1.0f, 0xCDCDCDCDu};
      const bool bytes_match = (memcmp(dst_flags_ptr, &expected, sizeof(expected)) == 0);

      hr = dst_flags_vb->Unlock();
      if (FAILED(hr)) {
        return reporter.FailHresult("dst_xyz_flags->Unlock (read)", hr);
      }

      if (!bytes_match) {
        return reporter.Fail("ProcessVertices XYZ Flags=D3DPV_DONOTCOPYDATA mismatch (non-position bytes were modified)");
      }
    }
  }

  // Validate XYZW -> XYZRHW fixed-function transform and that the input `w`
  // influences the output RHW (rhw = 1/w when WVP is identity).
  {
    const VertexXyzwDiffuse src = {0.0f, 0.0f, 0.0f, 2.0f, D3DCOLOR_XRGB(10, 20, 30)};
    ...
    hr = dev->CreateVertexBuffer(sizeof(src),
                                 ...
                                 NULL);
    if (FAILED(hr) || !src_vb) {
      return reporter.FailHresult("CreateVertexBuffer(src_xyzw)", hr);
    }

    void* src_ptr = NULL;
    hr = src_vb->Lock(0, sizeof(src), &src_ptr, 0);
    if (FAILED(hr) || !src_ptr) {
      return reporter.FailHresult("src_xyzw->Lock", hr);
    }
    memcpy(src_ptr, &src, sizeof(src));
    hr = src_vb->Unlock();
    if (FAILED(hr)) {
      return reporter.FailHresult("src_xyzw->Unlock", hr);
    }
    ...
    hr = dev->CreateVertexBuffer(sizeof(Vertex),
                                 ...
                                 NULL);
    if (FAILED(hr) || !dst_vb) {
      return reporter.FailHresult("CreateVertexBuffer(dst_xyzw)", hr);
    }

    void* dst_init_ptr = NULL;
    hr = dst_vb->Lock(0, sizeof(Vertex), &dst_init_ptr, 0);
    if (FAILED(hr) || !dst_init_ptr) {
      return reporter.FailHresult("dst_xyzw->Lock (init)", hr);
    }
    memset(dst_init_ptr, 0xCD, sizeof(Vertex));
    hr = dst_vb->Unlock();
    if (FAILED(hr)) {
      return reporter.FailHresult("dst_xyzw->Unlock (init)", hr);
    }

    hr = dev->SetFVF(D3DFVF_XYZW | D3DFVF_DIFFUSE);
    if (FAILED(hr)) {
      return reporter.FailHresult("SetFVF(XYZW|DIFFUSE)", hr);
    }
    hr = dev->SetStreamSource(0, src_vb.get(), 0, sizeof(VertexXyzwDiffuse));
    if (FAILED(hr)) {
      return reporter.FailHresult("SetStreamSource(src_xyzw)", hr);
    }
    ...
    hr = dev->ProcessVertices(/*SrcStartIndex=*/0,
                              ...
                              /*Flags=*/0);
    if (FAILED(hr)) {
      return reporter.FailHresult("IDirect3DDevice9::ProcessVertices(xyzw)", hr);
    }

    void* dst_ptr = NULL;
    hr = dst_vb->Lock(0, sizeof(Vertex), &dst_ptr, D3DLOCK_READONLY);
    if (FAILED(hr) || !dst_ptr) {
      return reporter.FailHresult("dst_xyzw->Lock (read)", hr);
    }

    const Vertex expected = {0.5f, 0.5f, 0.0f, 0.5f, src.color};
    const bool bytes_match = (memcmp(dst_ptr, &expected, sizeof(expected)) == 0);

    hr = dst_vb->Unlock();
    if (FAILED(hr)) {
      return reporter.FailHresult("dst_xyzw->Unlock (read)", hr);
    }

    if (!bytes_match) {
      return reporter.Fail("ProcessVertices XYZW->XYZRHW output bytes did not match expected output");
    }
  }
  ...
  // Validate TEX1 variant: XYZ|DIFFUSE|TEX1 -> XYZRHW|DIFFUSE|TEX1.
  {
    ...
    // Validate XYZ|TEX1 with TEXCOORDSIZE3(0) (float3 texcoords).
    {
      const VertexXyzTex1f3 src_white = {0.0f, 0.0f, 0.0f, 0.25f, 0.75f, 0.125f};
      ...
      hr = dev->CreateVertexBuffer(sizeof(src_white),
                                   ...
                                   NULL);
      if (FAILED(hr) || !src_white_vb) {
        return reporter.FailHresult("CreateVertexBuffer(src_white_f3)", hr);
      }

      void* src_white_ptr = NULL;
      hr = src_white_vb->Lock(0, sizeof(src_white), &src_white_ptr, 0);
      if (FAILED(hr) || !src_white_ptr) {
        return reporter.FailHresult("src_white_f3->Lock", hr);
      }
      memcpy(src_white_ptr, &src_white, sizeof(src_white));
      hr = src_white_vb->Unlock();
      if (FAILED(hr)) {
        return reporter.FailHresult("src_white_f3->Unlock", hr);
      }

      ComPtr<IDirect3DVertexBuffer9> dst_white_vb;
      hr = dev->CreateVertexBuffer(sizeof(VertexXyzrhwDiffuseTex1f3),
                                   0,
                                   D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1 | D3DFVF_TEXCOORDSIZE3(0),
                                   D3DPOOL_SYSTEMMEM,
                                   dst_white_vb.put(),
                                   NULL);
      if (FAILED(hr) || !dst_white_vb) {
        return reporter.FailHresult("CreateVertexBuffer(dst_white_f3)", hr);
      }

      void* dst_white_init_ptr = NULL;
      hr = dst_white_vb->Lock(0, sizeof(VertexXyzrhwDiffuseTex1f3), &dst_white_init_ptr, 0);
      if (FAILED(hr) || !dst_white_init_ptr) {
        return reporter.FailHresult("dst_white_f3->Lock (init)", hr);
      }
      memset(dst_white_init_ptr, 0xCD, sizeof(VertexXyzrhwDiffuseTex1f3));
      hr = dst_white_vb->Unlock();
      if (FAILED(hr)) {
        return reporter.FailHresult("dst_white_f3->Unlock (init)", hr);
      }

      const D3DVERTEXELEMENT9 decl_tex_f3_elems[] = {
          {0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITIONT, 0},
          {0, 16, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR, 0},
          {0, 20, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 0},
          D3DDECL_END(),
      };
      ComPtr<IDirect3DVertexDeclaration9> decl_tex_f3;
      hr = dev->CreateVertexDeclaration(decl_tex_f3_elems, decl_tex_f3.put());
      if (FAILED(hr) || !decl_tex_f3) {
        return reporter.FailHresult("CreateVertexDeclaration(tex_f3)", hr);
      }

      hr = dev->SetFVF(D3DFVF_XYZ | D3DFVF_TEX1 | D3DFVF_TEXCOORDSIZE3(0));
      if (FAILED(hr)) {
        return reporter.FailHresult("SetFVF(XYZ|TEX1|F3)", hr);
      }
      hr = dev->SetStreamSource(0, src_white_vb.get(), 0, sizeof(VertexXyzTex1f3));
      if (FAILED(hr)) {
        return reporter.FailHresult("SetStreamSource(src_white_f3)", hr);
      }
      ...
      hr = dev->ProcessVertices(/*SrcStartIndex=*/0,
                                ...
                                /*Flags=*/0);
      if (FAILED(hr)) {
        return reporter.FailHresult("IDirect3DDevice9::ProcessVertices(xyz|tex1 f3)", hr);
      }

      void* dst_white_ptr = NULL;
      hr = dst_white_vb->Lock(0, sizeof(VertexXyzrhwDiffuseTex1f3), &dst_white_ptr, D3DLOCK_READONLY);
      if (FAILED(hr) || !dst_white_ptr) {
        return reporter.FailHresult("dst_white_f3->Lock (read)", hr);
      }

      const VertexXyzrhwDiffuseTex1f3 expected_white = {0.5f,
                                                        0.5f,
                                                        0.0f,
                                                        1.0f,
                                                        D3DCOLOR_XRGB(255, 255, 255),
      ...
      hr = dst_white_vb->Unlock();
      if (FAILED(hr)) {
        return reporter.FailHresult("dst_white_f3->Unlock (read)", hr);
      }

      if (!white_bytes_match) {
        return reporter.Fail("ProcessVertices XYZ|TEX1(TEXCOORDSIZE3) missing-diffuse case did not match expected output");
      }
    }
