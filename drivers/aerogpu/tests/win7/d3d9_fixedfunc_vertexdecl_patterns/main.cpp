...
static int RunD3D9FixedFuncVertexDeclPatterns(int argc, char** argv) {
  const char* kTestName = "d3d9_fixedfunc_vertexdecl_patterns";
  ...
  HWND hwnd = aerogpu_test::CreateBasicWindow(L"AeroGPU_D3D9FixedFuncVertexDeclPatterns",
                                              L"AeroGPU D3D9 FixedFunc VertexDecl Patterns",
                                              kWidth,
                                              kHeight,
                                              !hidden);
  if (!hwnd) {
  ...
  // Vertex declaration: XYZ | DIFFUSE | TEX1, but with non-canonical element ordering.
  // (TEXCOORD0 appears before COLOR0.)
  ...
  hr = dev->CreateVertexBuffer(sizeof(quad),
  ...
  void* vb_ptr = NULL;
  hr = vb->Lock(0, sizeof(quad), &vb_ptr, D3DLOCK_DISCARD);
  if (FAILED(hr) || !vb_ptr) {
    return reporter.FailHresult("VertexBuffer Lock", FAILED(hr) ? hr : E_FAIL);
  }
  memcpy(vb_ptr, quad, sizeof(quad));
  vb->Unlock();
  ...
  hr = dev->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 2);
  if (FAILED(hr)) {
    dev->EndScene();
  ...
  if (!ok_tl || !ok_tr || !ok_bl || !ok_br) {
    if (dump) {
      ...
      const std::wstring bmp_path = aerogpu_test::JoinPath(
          aerogpu_test::GetModuleDir(), L"d3d9_fixedfunc_vertexdecl_patterns.bmp");
      ...
      DumpTightBgra32(kTestName,
                      &reporter,
                      L"d3d9_fixedfunc_vertexdecl_patterns.bin",
                      lr.pBits,
                      (int)lr.Pitch,
                      (int)desc.Width,
                      (int)desc.Height);
...
int main(int argc, char** argv) {
  ...
  int rc = RunD3D9FixedFuncVertexDeclPatterns(argc, argv);
  Sleep(30);
  return rc;
}
