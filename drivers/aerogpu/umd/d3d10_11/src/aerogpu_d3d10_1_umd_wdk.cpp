...
bool AllocateSharedAllocIds(uint32_t* out_alloc_id, uint64_t* out_share_token) {
...
HRESULT AeroGpuWaitForFence(AeroGpuDevice* dev, uint64_t fence, uint32_t timeout_ms) {
  ...
  if (dev->kmt_fence_syncobj) {
    ...
    if (have_wait_cb) {
      ...
      FillWaitForSyncObjectArgs(&args, dev->kmt_context, /*hAdapter=*/0, handles, fence_values, fence, timeout);
    ...
    if (procs.pfn_wait_for_syncobj) {
      ...
      FillWaitForSyncObjectArgs(&args, dev->kmt_context, dev->kmt_adapter, handles, fence_values, fence, timeout);
...
uint64_t submit_locked(AeroGpuDevice* dev, bool want_present, HRESULT* out_hr) {
  ...
  auto deallocate_buffers = [&](const D3DDDICB_ALLOCATE& alloc, void* dma_priv_ptr, size_t dma_priv_size) {
    ...
    __if_exists(D3DDDICB_DEALLOCATE::DmaBufferPrivateDataSize) {
      dealloc.DmaBufferPrivateDataSize = static_cast<UINT>(dma_priv_size);
    }
...
HRESULT AEROGPU_APIENTRY CreateResource(D3D10DDI_HDEVICE hDevice,
  ...
  if (pDesc->ResourceDimension == D3D10DDIRESOURCE_BUFFER) {
    ...
    HRESULT alloc_hr = allocate_one(alloc_size, cpu_visible, is_rt, is_ds, is_shared);
    if (FAILED(alloc_hr)) {
  ...
  if (pDesc->ResourceDimension == D3D10DDIRESOURCE_TEXTURE2D) {
    ...
    HRESULT alloc_hr = allocate_one(total_bytes, cpu_visible, is_rt, is_ds, is_shared);
    if (FAILED(alloc_hr)) {
      set_error(dev, alloc_hr);
      deallocate_if_needed();
      res->~AeroGpuResource();
...
HRESULT map_resource_locked(AeroGpuDevice* dev,
  ...
  if (res->kind == ResourceKind::Texture2D) {
    ...
    const uint32_t slice = res->mapped_wddm_slice_pitch ? res->mapped_wddm_slice_pitch : pitch * res->height;
