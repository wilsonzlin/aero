...
void flush_locked(AeroGpuDevice* dev) {
  if (dev) {
    (void)aerogpu::d3d10_11::EmitFlushCmdLocked(dev);
  }
  HRESULT hr = S_OK;
...
void emit_dirty_range_locked(AeroGpuDevice* dev,
  ...
  (void)aerogpu::d3d10_11::EmitResourceDirtyRangeCmdLocked(
      dev, res->handle, offset_bytes, size_bytes, [&](HRESULT hr) { set_error(dev, hr); });
}
...
void AEROGPU_APIENTRY DestroyResource(D3D10DDI_HDEVICE hDevice, D3D10DDI_HRESOURCE hResource) {
  ...
  for (uint32_t slot = 0; slot < dev->current_vs_srvs.size(); ++slot) {
    if (dev->current_vs_srvs[slot] == res) {
      ...
      if (!oom) {
        if (!aerogpu::d3d10_11::EmitSetTextureCmdLocked(dev,
...
void unmap_resource_locked(AeroGpuDevice* dev, AeroGpuResource* res, uint32_t subresource) {
  ...
  if (res->mapped_wddm_ptr && res->mapped_wddm_allocation) {
    if (emit_ok && res->backing_alloc_id != 0) {
      ...
      TrackWddmAllocForSubmitLocked(dev, res, /*write=*/false);
      if (!dev->wddm_submit_allocation_list_oom) {
        dirty_emitted_on_unmap = aerogpu::d3d10_11::EmitResourceDirtyRangeCmdLocked(
            dev, res->handle, upload_offset, upload_size);
      }
...
void AEROGPU_APIENTRY ClearDepthStencilView(D3D10DDI_HDEVICE hDevice,
  ...
  (void)aerogpu::d3d10_11::EmitClearCmdLocked(dev,
                                              flags,
...
void AEROGPU_APIENTRY SetBlendState(D3D10DDI_HDEVICE hDevice,
  ...
  state.enable = base.enable ? 1u : 0u;
  state.src_factor = base.src_factor;
  state.dst_factor = base.dst_factor;
  state.blend_op = base.blend_op;
  state.color_write_mask = static_cast<uint8_t>(base.color_write_mask & kD3DColorWriteMaskAll);
  state.reserved0[0] = 0;
  state.reserved0[1] = 0;
  state.reserved0[2] = 0;
  state.src_factor_alpha = base.src_factor_alpha;
  state.dst_factor_alpha = base.dst_factor_alpha;
  state.blend_op_alpha = base.blend_op_alpha;
  state.blend_constant_rgba_f32[0] = f32_bits(blend_factor ? blend_factor[0] : 1.0f);
  state.blend_constant_rgba_f32[1] = f32_bits(blend_factor ? blend_factor[1] : 1.0f);
  state.blend_constant_rgba_f32[2] = f32_bits(blend_factor ? blend_factor[2] : 1.0f);
  state.blend_constant_rgba_f32[3] = f32_bits(blend_factor ? blend_factor[3] : 1.0f);
  state.sample_mask = sample_mask;
  (void)aerogpu::d3d10_11::EmitSetBlendStateCmdLocked(dev, state, [&](HRESULT hr) { set_error(dev, hr); });
}
...
void AEROGPU_APIENTRY SetRasterizerState(D3D10DDI_HDEVICE hDevice, D3D10DDI_HRASTERIZERSTATE hState) {
  ...
  state.fill_mode = aerogpu::d3d10_11::D3DFillModeToAerogpu(fill_mode);
  state.cull_mode = aerogpu::d3d10_11::D3DCullModeToAerogpu(cull_mode);
  state.front_ccw = front_ccw ? 1u : 0u;
  state.scissor_enable = scissor_enable ? 1u : 0u;
  state.depth_bias = depth_bias;
  state.flags = depth_clip_enable ? AEROGPU_RASTERIZER_FLAG_NONE : AEROGPU_RASTERIZER_FLAG_DEPTH_CLIP_DISABLE;
  (void)aerogpu::d3d10_11::EmitSetRasterizerStateCmdLocked(dev, state, [&](HRESULT hr) { set_error(dev, hr); });
}
...
void AEROGPU_APIENTRY SetDepthStencilState(D3D10DDI_HDEVICE hDevice,
  ...
  state.depth_func = aerogpu::d3d10_11::D3DCompareFuncToAerogpu(depth_func);
  ...
  state.reserved0[0] = 0;
  state.reserved0[1] = 0;
  (void)aerogpu::d3d10_11::EmitSetDepthStencilStateCmdLocked(dev, state, [&](HRESULT hr) { set_error(dev, hr); });
}
...
void SetShaderResourcesCommon(D3D10DDI_HDEVICE hDevice,
  ...
  for (UINT i = 0; i < num_views; i++) {
    ...
    if (!aerogpu::d3d10_11::EmitSetTextureCmdLocked(
            dev, shader_stage, slot, tex, [&](HRESULT hr) { set_error(dev, hr); })) {
      return;
    }
...
void AEROGPU_APIENTRY ClearState(D3D10DDI_HDEVICE hDevice) {
  ...
  aerogpu_blend_state bs_state{};
  bs_state.enable = 0;
  bs_state.src_factor = AEROGPU_BLEND_ONE;
  bs_state.dst_factor = AEROGPU_BLEND_ZERO;
  bs_state.blend_op = AEROGPU_BLEND_OP_ADD;
  bs_state.color_write_mask = kD3DColorWriteMaskAll;
  bs_state.reserved0[0] = 0;
  bs_state.reserved0[1] = 0;
  bs_state.reserved0[2] = 0;
  bs_state.src_factor_alpha = AEROGPU_BLEND_ONE;
  bs_state.dst_factor_alpha = AEROGPU_BLEND_ZERO;
  bs_state.blend_op_alpha = AEROGPU_BLEND_OP_ADD;
  bs_state.blend_constant_rgba_f32[0] = f32_bits(1.0f);
  bs_state.blend_constant_rgba_f32[1] = f32_bits(1.0f);
  bs_state.blend_constant_rgba_f32[2] = f32_bits(1.0f);
  bs_state.blend_constant_rgba_f32[3] = f32_bits(1.0f);
  bs_state.sample_mask = kD3DSampleMaskAll;
  if (!aerogpu::d3d10_11::EmitSetBlendStateCmdLocked(dev, bs_state, [&](HRESULT hr) { set_error(dev, hr); })) {
    return;
  }

  // Reset depth/stencil state to D3D10 defaults (depth enabled, write enabled, LESS, stencil disabled).
  aerogpu_depth_stencil_state dss_state{};
  dss_state.depth_enable = 1u;
  dss_state.depth_write_enable = 1u;
  dss_state.depth_func = AEROGPU_COMPARE_LESS;
  dss_state.stencil_enable = 0u;
  dss_state.stencil_read_mask = kD3DStencilMaskAll;
  dss_state.stencil_write_mask = kD3DStencilMaskAll;
  dss_state.reserved0[0] = 0;
  dss_state.reserved0[1] = 0;
  if (!aerogpu::d3d10_11::EmitSetDepthStencilStateCmdLocked(
          dev, dss_state, [&](HRESULT hr) { set_error(dev, hr); })) {
    return;
  }

  // Reset rasterizer state to D3D10 defaults (solid, cull back, depth clip enabled).
  aerogpu_rasterizer_state rs_state{};
  rs_state.fill_mode = AEROGPU_FILL_SOLID;
  rs_state.cull_mode = AEROGPU_CULL_BACK;
  rs_state.front_ccw = 0;
  rs_state.scissor_enable = 0;
  rs_state.depth_bias = 0;
  rs_state.flags = AEROGPU_RASTERIZER_FLAG_NONE;
  if (!aerogpu::d3d10_11::EmitSetRasterizerStateCmdLocked(dev, rs_state, [&](HRESULT hr) { set_error(dev, hr); })) {
    return;
  }
...
void AEROGPU_APIENTRY Draw(D3D10DDI_HDEVICE hDevice, UINT vertex_count, UINT start_vertex) {
  ...
  (void)aerogpu::d3d10_11::EmitDrawCmdLocked(
      dev, vertex_count, /*instance_count=*/1, start_vertex, /*first_instance=*/0, [&](HRESULT hr) { set_error(dev, hr); });
}
...
void AEROGPU_APIENTRY DrawInstanced(D3D10DDI_HDEVICE hDevice,
  ...
  (void)aerogpu::d3d10_11::EmitDrawCmdLocked(dev,
                                            vertex_count_per_instance,
                                            instance_count,
                                            start_vertex_location,
                                            start_instance_location,
...
void AEROGPU_APIENTRY DrawIndexed(D3D10DDI_HDEVICE hDevice, UINT index_count, UINT start_index, INT base_vertex) {
  ...
  (void)aerogpu::d3d10_11::EmitDrawIndexedCmdLocked(dev,
                                                    index_count,
                                                    /*instance_count=*/1,
                                                    start_index,
                                                    base_vertex,
...
HRESULT AEROGPU_APIENTRY Present(D3D10DDI_HDEVICE hDevice, const D3D10DDIARG_PRESENT* pPresent) {
  ...
  if (!aerogpu::d3d10_11::EmitPresentCmdLocked(dev, /*scanout_id=*/0, flags)) {
    dev->cmd.reset();
    dev->wddm_submit_allocation_handles.clear();
    dev->wddm_submit_allocation_list_oom = false;
    dev->pending_staging_writes.clear();
...
void AEROGPU_APIENTRY UpdateSubresourceUP(D3D10DDI_HDEVICE hDevice,
  ...
  if (res->kind == ResourceKind::Texture2D) {
    ...
    if (!aerogpu::d3d10_11::EmitResourceDirtyRangeCmdLocked(dev, res->handle, dirty_offset, dirty_size)) {
      restore_storage_from_allocation();
      dev->cmd.rollback(cmd_checkpoint);
      alloc_checkpoint.rollback();
      D3DDDICB_UNLOCK unlock_args = {};
...
void AEROGPU_APIENTRY RotateResourceIdentities(D3D10DDI_HDEVICE hDevice,
  ...
  for (uint32_t slot = 0; slot < dev->current_vs_srvs.size(); ++slot) {
    ...
    const aerogpu_handle_t texture = dev->current_vs_srvs[slot] ? dev->current_vs_srvs[slot]->handle : 0;
    if (!aerogpu::d3d10_11::EmitSetTextureCmdLocked(dev,
                                                    AEROGPU_SHADER_STAGE_VERTEX,
                                                    slot,
                                                    texture,
  ...
  for (uint32_t slot = 0; slot < dev->current_ps_srvs.size(); ++slot) {
    ...
    const aerogpu_handle_t texture = dev->current_ps_srvs[slot] ? dev->current_ps_srvs[slot]->handle : 0;
    if (!aerogpu::d3d10_11::EmitSetTextureCmdLocked(dev,
                                                    AEROGPU_SHADER_STAGE_PIXEL,
                                                    slot,
                                                    texture,
  ...
  for (uint32_t slot = 0; slot < dev->current_gs_srvs.size(); ++slot) {
    ...
    const aerogpu_handle_t texture = dev->current_gs_srvs[slot] ? dev->current_gs_srvs[slot]->handle : 0;
    if (!aerogpu::d3d10_11::EmitSetTextureCmdLocked(dev,
                                                    AEROGPU_SHADER_STAGE_GEOMETRY,
                                                    slot,
                                                    texture,
