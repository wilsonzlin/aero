...
struct AeroGpuAdapter {
  ...
  // Minimal, portable equivalent of the Win7 `DXGKQAITYPE_UMDRIVERPRIVATE` blob
  // used by the WDK build. This allows the portable UMD and unit tests to run
  // the same format/feature gating logic (BC formats, sRGB formats, transfer
  // commands) as the real WDK UMDs.
...
void track_current_state_allocs_for_submit_locked(AeroGpuDevice* dev) {
  ...
  for (uint32_t i = 0; i < kMaxShaderResourceSlots; i++) {
    track_resource_alloc_for_submit_locked(dev, FindLiveResourceByHandleLocked(dev, dev->vs_srvs[i]), /*write=*/false);
    track_resource_alloc_for_submit_locked(dev, FindLiveResourceByHandleLocked(dev, dev->ps_srvs[i]), /*write=*/false);
  }
}
...
bool set_render_targets_locked(AeroGpuDevice* dev,
                               D3D10DDI_HDEVICE hDevice,
                               uint32_t rtv_count,
                               AeroGpuResource* const* rtv_resources,
                               AeroGpuResource* dsv_res) {
  if (!dev) {
    return false;
  }
  ...
  for (uint32_t i = 0; i < count; ++i) {
    AeroGpuResource* res = rtv_resources ? rtv_resources[i] : nullptr;
...
bool emit_set_render_targets_locked(AeroGpuDevice* dev) {
  ...
  // Keep cached handles in sync with the resource pointers. This matters for
  // RotateResourceIdentities, which swaps the underlying handle identity while
  // keeping the same Resource* pointers live in the runtime.
  for (uint32_t i = 0; i < dev->current_rtv_count && i < AEROGPU_MAX_RENDER_TARGETS; ++i) {
...
HRESULT map_resource_locked(AeroGpuDevice* dev,
  ...
  if (is_guest_backed && res->alloc_handle != 0 && dev->device_callbacks && dev->device_callbacks->pfnMapAllocation &&
    ...
    if (map_type == AEROGPU_DDI_MAP_WRITE_DISCARD && mapped_size <= static_cast<uint64_t>(SIZE_MAX)) {
      ...
      std::memset(data + static_cast<size_t>(mapped_offset), 0, static_cast<size_t>(mapped_size));
    }
...
HRESULT AEROGPU_APIENTRY CreateShaderResourceView(D3D10DDI_HDEVICE hDevice,
  ...
  // Portable build: treat shader-resource views as aliases of the whole underlying
  // Texture2D and ignore view-dimension slicing. This allows array textures to be
  // bound even when the test ABI does not model TEXTURE2DARRAY explicitly.
  if (pDesc->MostDetailedMip != 0) {
    return E_NOTIMPL;
  }
  const uint32_t mip_levels = pDesc->MipLevels ? pDesc->MipLevels : res->mip_levels;
  if (mip_levels != res->mip_levels) {
    return E_NOTIMPL;
  }
...
HRESULT AEROGPU_APIENTRY CreateBlendState(D3D10DDI_HDEVICE hDevice,
  ...
  if (pDesc) {
    ...
    if ((pDesc->color_write_mask & ~0xFu) != 0) {
      AEROGPU_D3D10_RET_HR(E_INVALIDARG);
    }
    ...
    state.color_write_mask = static_cast<uint8_t>(pDesc->color_write_mask & 0xFu);
...
void AEROGPU_APIENTRY SetRenderTargets(D3D10DDI_HDEVICE hDevice,
  ...
  AeroGpuResource* rtvs[AEROGPU_MAX_RENDER_TARGETS] = {};
  for (uint32_t i = 0; i < count; ++i) {
    if (ph_views && ph_views[i].pDrvPrivate) {
      rtvs[i] = FromHandle<D3D10DDI_HRENDERTARGETVIEW, AeroGpuRenderTargetView>(ph_views[i])->resource;
    }
  }
  ...
  if (!set_render_targets_locked(dev, hDevice, count, (count ? rtvs : nullptr), dsv_res)) {
    return;
  }
}
...
void AEROGPU_APIENTRY SetScissorRects(D3D10DDI_HDEVICE hDevice,
  ...
  const int32_t w = aerogpu::d3d10_11::clamp_i64_to_i32(static_cast<int64_t>(r.right) - static_cast<int64_t>(r.left));
  const int32_t h = aerogpu::d3d10_11::clamp_i64_to_i32(static_cast<int64_t>(r.bottom) - static_cast<int64_t>(r.top));
  auto* cmd = dev->cmd.append_fixed<aerogpu_cmd_set_scissor>(AEROGPU_CMD_SET_SCISSOR);
  if (!cmd) {
    ReportDeviceErrorLocked(dev, hDevice, E_OUTOFMEMORY);
    return;
...
static void EmitRasterizerStateLocked(AeroGpuDevice* dev, D3D10DDI_HDEVICE hDevice, const AeroGpuRasterizerState* rs) {
  if (!dev) {
    return;
  }
...
static void EmitBlendStateLocked(AeroGpuDevice* dev, D3D10DDI_HDEVICE hDevice, const AeroGpuBlendState* bs) {
  if (!dev) {
    return;
  }
  ...
  state.reserved0[1] = 0;
  state.reserved0[2] = 0;
  state.blend_constant_rgba_f32[0] = f32_bits(dev->current_blend_factor[0]);
  state.blend_constant_rgba_f32[1] = f32_bits(dev->current_blend_factor[1]);
  state.blend_constant_rgba_f32[2] = f32_bits(dev->current_blend_factor[2]);
  state.blend_constant_rgba_f32[3] = f32_bits(dev->current_blend_factor[3]);
  state.sample_mask = dev->current_sample_mask;
  cmd->state = state;
}

static void EmitDepthStencilStateLocked(AeroGpuDevice* dev, D3D10DDI_HDEVICE hDevice, const AeroGpuDepthStencilState* dss) {
  if (!dev) {
    return;
  }
...
void AEROGPU_APIENTRY SetBlendState(D3D10DDI_HDEVICE hDevice,
  ...
  EmitBlendStateLocked(dev, hDevice, dev->current_bs);
}
...
void AEROGPU_APIENTRY SetRasterizerState(D3D10DDI_HDEVICE hDevice, D3D10DDI_HRASTERIZERSTATE hState) {
  ...
  EmitRasterizerStateLocked(dev, hDevice, dev->current_rs);
}
...
void AEROGPU_APIENTRY SetDepthStencilState(D3D10DDI_HDEVICE hDevice, D3D10DDI_HDEPTHSTENCILSTATE hState, UINT stencil_ref) {
  ...
  EmitDepthStencilStateLocked(dev, hDevice, dev->current_dss);
}
