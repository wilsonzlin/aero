...
HRESULT InitKernelDeviceContext(AeroGpuDevice* dev, D3D10DDI_HADAPTER hAdapter) {
  ...
  const HRESULT hr =
      dev->wddm_submit.Init(cb, hAdapter.pDrvPrivate, dev->hrt_device.pDrvPrivate, /*kmt_adapter_for_debug=*/0);
  if (FAILED(hr)) {
...
HRESULT APIENTRY CreateResource(D3D10DDI_HDEVICE hDevice,
  ...
  const auto allocate_one = [&](uint64_t size_bytes,
    ...
    if (is_shared) {
      ...
      priv.share_token = share_token ? share_token : static_cast<uint64_t>(alloc_id);
    } else {
      priv.share_token = 0;
    }
