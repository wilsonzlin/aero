    ...
    if (emit_ok && res->mapped_write && res->mapped_size != 0 && res->backing_alloc_id != 0) {
      ...
      if (!dev->wddm_submit_allocation_list_oom) {
        dirty_emitted_on_unmap = aerogpu::d3d10_11::EmitResourceDirtyRangeCmdLocked(
            dev, res->handle, upload_offset, upload_size_storage);
      }
  ...
  if (res->kind == ResourceKind::Buffer) {
    ...
    if (res->backing_alloc_id != 0 && pUpdate->pDstBox) {
      ...
      if (!aerogpu::d3d10_11::EmitResourceDirtyRangeCmdLocked(
              dev, res->handle, dirty_offset, dirty_size)) {
        restore_storage_from_allocation();
        dev->cmd.rollback(cmd_checkpoint);
        alloc_checkpoint.rollback();
        D3DDDICB_UNLOCK unlock_args = {};
...
void APIENTRY ClearState(D3D10DDI_HDEVICE hDevice) {
  ...
  if (!aerogpu::d3d10_11::EmitSetBlendStateCmdLocked(dev, bs_state, [&](HRESULT hr) { SetError(hDevice, hr); })) {
    return;
  }
  ...
  if (!aerogpu::d3d10_11::EmitSetDepthStencilStateCmdLocked(
          dev, dss_state, [&](HRESULT hr) { SetError(hDevice, hr); })) {
    return;
  }
  ...
  if (!aerogpu::d3d10_11::EmitSetRasterizerStateCmdLocked(dev, rs_state, [&](HRESULT hr) { SetError(hDevice, hr); })) {
    return;
  }
...
static bool EmitRasterizerStateLocked(D3D10DDI_HDEVICE hDevice, AeroGpuDevice* dev, const AeroGpuRasterizerState* rs) {
  ...
  return aerogpu::d3d10_11::EmitSetRasterizerStateCmdLocked(
      dev, state, [&](HRESULT hr) { SetError(hDevice, hr); });
}
...
static bool EmitBlendStateLocked(D3D10DDI_HDEVICE hDevice,
  ...
  state.color_write_mask = base.color_write_mask;
  state.reserved0[0] = 0;
  state.reserved0[1] = 0;
  state.reserved0[2] = 0;
  ...
  return aerogpu::d3d10_11::EmitSetBlendStateCmdLocked(dev, state, [&](HRESULT hr) { SetError(hDevice, hr); });
}
...
void APIENTRY Draw(D3D10DDI_HDEVICE hDevice, UINT vertexCount, UINT startVertex) {
  ...
  (void)aerogpu::d3d10_11::EmitDrawCmdLocked(
      dev, vertexCount, /*instance_count=*/1, startVertex, /*first_instance=*/0, [&](HRESULT hr) { SetError(hDevice, hr); });
}
