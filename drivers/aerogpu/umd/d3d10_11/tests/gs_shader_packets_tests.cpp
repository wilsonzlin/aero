...
bool TestGeometryShaderCreateAndBindPackets() {
  ...
  constexpr aerogpu_handle_t kHsHandle = 0xCAFE5678u;
  constexpr aerogpu_handle_t kDsHandle = 0xCAFE90ABu;
  constexpr uint8_t kDxbc[] = {
      0x44, 0x58, 0x42, 0x43,  // "DXBC"
      0x01, 0x02, 0x03,        // payload bytes (intentionally not 4-byte aligned)
  };

  // Encode shader creation using:
  // - the direct GEOMETRY stage encoding for GS (legacy stage enum includes GEOMETRY = 3), and
  // - the stage_ex ABI extension for non-legacy stages like HS/DS.
  ...
  auto* create_hs = w.append_with_payload<aerogpu_cmd_create_shader_dxbc>(
      AEROGPU_CMD_CREATE_SHADER_DXBC, kDxbc, sizeof(kDxbc));
  if (!Check(create_hs != nullptr, "append CREATE_SHADER_DXBC (HS)")) {
    return false;
  }
  create_hs->shader_handle = kHsHandle;
  create_hs->stage = AEROGPU_SHADER_STAGE_COMPUTE;
  create_hs->dxbc_size_bytes = static_cast<uint32_t>(sizeof(kDxbc));
  create_hs->reserved0 = AEROGPU_SHADER_STAGE_EX_HULL;

  auto* create_ds = w.append_with_payload<aerogpu_cmd_create_shader_dxbc>(
      AEROGPU_CMD_CREATE_SHADER_DXBC, kDxbc, sizeof(kDxbc));
  if (!Check(create_ds != nullptr, "append CREATE_SHADER_DXBC (DS)")) {
    return false;
  }
  create_ds->shader_handle = kDsHandle;
  create_ds->stage = AEROGPU_SHADER_STAGE_COMPUTE;
  create_ds->dxbc_size_bytes = static_cast<uint32_t>(sizeof(kDxbc));
  create_ds->reserved0 = AEROGPU_SHADER_STAGE_EX_DOMAIN;
  ...
  auto* destroy_hs = w.append_fixed<aerogpu_cmd_destroy_shader>(AEROGPU_CMD_DESTROY_SHADER);
  if (!Check(destroy_hs != nullptr, "append DESTROY_SHADER (HS)")) {
    return false;
  }
  destroy_hs->shader_handle = kHsHandle;
  destroy_hs->reserved0 = 0;

  auto* destroy_ds = w.append_fixed<aerogpu_cmd_destroy_shader>(AEROGPU_CMD_DESTROY_SHADER);
  if (!Check(destroy_ds != nullptr, "append DESTROY_SHADER (DS)")) {
    return false;
  }
  destroy_ds->shader_handle = kDsHandle;
  destroy_ds->reserved0 = 0;
  ...
  // CREATE_SHADER_DXBC (GS)
  {
    ...
    if (!Check(
            std::memcmp(buf + offset + sizeof(aerogpu_cmd_create_shader_dxbc), kDxbc, sizeof(kDxbc)) ==
                0,
  ...
  {
    if (!Check(offset + sizeof(aerogpu_cmd_hdr) <= len, "CREATE_SHADER_DXBC (HS) header in-bounds")) {
      return false;
    }
    const auto* create_hdr = reinterpret_cast<const aerogpu_cmd_hdr*>(buf + offset);
    if (!Check(create_hdr->opcode == AEROGPU_CMD_CREATE_SHADER_DXBC, "CREATE_SHADER_DXBC (HS) opcode")) {
      return false;
    }
    if (!Check(create_hdr->size_bytes == expected_create_size, "CREATE_SHADER_DXBC (HS) size_bytes")) {
      return false;
    }
    const auto* create_cmd = reinterpret_cast<const aerogpu_cmd_create_shader_dxbc*>(create_hdr);
    if (!Check(create_cmd->stage == AEROGPU_SHADER_STAGE_COMPUTE,
               "CREATE_SHADER_DXBC (HS) stage==COMPUTE (stage_ex encoding)")) {
      return false;
    }
    if (!Check(create_cmd->reserved0 == AEROGPU_SHADER_STAGE_EX_HULL,
               "CREATE_SHADER_DXBC (HS) reserved0==HULL stage_ex")) {
      return false;
    }
    if (!Check(create_cmd->shader_handle == kHsHandle, "CREATE_SHADER_DXBC (HS) shader_handle")) {
      return false;
    }
    offset += create_hdr->size_bytes;
  }
  ...
  {
    if (!Check(offset + sizeof(aerogpu_cmd_hdr) <= len, "CREATE_SHADER_DXBC (DS) header in-bounds")) {
      return false;
    }
    const auto* create_hdr = reinterpret_cast<const aerogpu_cmd_hdr*>(buf + offset);
    if (!Check(create_hdr->opcode == AEROGPU_CMD_CREATE_SHADER_DXBC, "CREATE_SHADER_DXBC (DS) opcode")) {
      return false;
    }
    if (!Check(create_hdr->size_bytes == expected_create_size, "CREATE_SHADER_DXBC (DS) size_bytes")) {
      return false;
    }
    const auto* create_cmd = reinterpret_cast<const aerogpu_cmd_create_shader_dxbc*>(create_hdr);
    if (!Check(create_cmd->stage == AEROGPU_SHADER_STAGE_COMPUTE,
               "CREATE_SHADER_DXBC (DS) stage==COMPUTE (stage_ex encoding)")) {
      return false;
    }
    if (!Check(create_cmd->reserved0 == AEROGPU_SHADER_STAGE_EX_DOMAIN,
               "CREATE_SHADER_DXBC (DS) reserved0==DOMAIN stage_ex")) {
      return false;
    }
    if (!Check(create_cmd->shader_handle == kDsHandle, "CREATE_SHADER_DXBC (DS) shader_handle")) {
      return false;
    }
    offset += create_hdr->size_bytes;
  }
  ...
  const auto* bind_ex_payload = reinterpret_cast<const aerogpu_handle_t*>(
      buf + offset + sizeof(aerogpu_cmd_bind_shaders));
  if (!Check(bind_ex_payload[0] == kGsHandle, "BIND_SHADERS ex.gs handle")) {
    return false;
  }
  if (!Check(bind_ex_payload[1] == kHsHandle, "BIND_SHADERS ex.hs handle")) {
    return false;
  }
  if (!Check(bind_ex_payload[2] == kDsHandle, "BIND_SHADERS ex.ds handle")) {
    return false;
  }
  offset += bind_hdr->size_bytes;

  // DESTROY_SHADER
  const aerogpu_handle_t kDestroyed[] = {kGsHandle, kHsHandle, kDsHandle};
  for (size_t i = 0; i < (sizeof(kDestroyed) / sizeof(kDestroyed[0])); i += 1) {
    ...
    if (!Check(destroy_hdr->size_bytes == sizeof(aerogpu_cmd_destroy_shader),
               "DESTROY_SHADER size_bytes")) {
      return false;
    }
    const auto* destroy_cmd = reinterpret_cast<const aerogpu_cmd_destroy_shader*>(destroy_hdr);
    if (!Check(destroy_cmd->shader_handle == kDestroyed[i], "DESTROY_SHADER shader_handle")) {
      return false;
    }
