...
| Geometry shaders (GS) | **Supported (partial)**: GS create + bind (GS handle carried via `BIND_SHADERS.reserved0`; GS resource bindings stubbed) | **Supported (partial)**: GS create + bind (GS handle carried via `BIND_SHADERS.reserved0`; GS resource bindings stubbed) | **Supported (partial)**: GS create + bind (GS handle carried via `BIND_SHADERS.reserved0`) |
...
- Geometry shaders (GS):
  ...
  - Known unsupported / not yet implemented:
    ...
    - Adjacency primitive semantics and tessellation (HS/DS) beyond the placeholder prepass path.
