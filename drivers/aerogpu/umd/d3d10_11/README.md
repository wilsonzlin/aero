...
- Geometry shaders (GS):
  ...
  - Host/WebGPU execution:
    ...
    - Prepass implementations:
      ...
      - Point-/triangle-list GS prepass paths that can execute a translated SM4 GS DXBC subset for `PointList` and `TriangleList` draws (`Draw` and `DrawIndexed`) (bring-up limitations apply: it does **not** execute guest VS DXBC yet; GS inputs are pulled directly from IA buffers).
    - A minimal SM4 GS DXBC→WGSL compute translator exists in `crates/aero-d3d11/src/runtime/gs_translate.rs` and is partially wired into the executor:
      ...
      - `PointList` and `TriangleList` draws (`Draw` and `DrawIndexed`) can execute the translated prepass when translation succeeds (see `exec_geometry_shader_prepass_pointlist` and `exec_geometry_shader_prepass_trianglelist`).
    ...
    - Translator-backed GS prepass supported subset (covered by `crates/aero-d3d11/tests/gs_translate.rs`; executed for point/triangle-list draws when supported):
      - End-to-end execution path: `PointList` and `TriangleList` input draws (`Draw` and `DrawIndexed`) only.
