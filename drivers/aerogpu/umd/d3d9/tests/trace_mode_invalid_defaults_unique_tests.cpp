...
int main() {
  ...
  const std::string out_path = make_unique_log_path("aerogpu_d3d9_trace_mode_invalid_defaults_unique_tests");
  if (!std::freopen(out_path.c_str(), "w", stderr)) {
    return fail("freopen(stderr) failed");
  }

  set_env("AEROGPU_D3D9_TRACE", "1");
  // Unknown mode should be treated as the default (unique), not as "all".
  set_env("AEROGPU_D3D9_TRACE_MODE", "  does_not_exist  ");
  ...
  // Second call to the same entrypoint should be suppressed if mode defaults to unique.
  {
  ...
  if (output.find("mode=unique") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected mode=unique for invalid TRACE_MODE (log=%s)\n", out_path.c_str());
    return 1;
  }
  ...
  if (output.find("a0=0x222") != std::string::npos) {
    std::fprintf(stdout, "FAIL: did not expect a0=0x222 under invalid TRACE_MODE (log=%s)\n", out_path.c_str());
    return 1;
  }
