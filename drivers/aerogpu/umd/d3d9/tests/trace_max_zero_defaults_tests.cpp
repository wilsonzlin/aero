...
int main() {
  ...
  const std::string out_path = make_unique_log_path("aerogpu_d3d9_trace_max_zero_defaults_tests");
  ...
  // `0` should mean "use the default" (not "0 entries").
  set_env("AEROGPU_D3D9_TRACE_MAX", "0");
  ...
  // With max=default, both calls should be recorded in TRACE_MODE=all.
  {
  ...
  if (output.find("entries=2") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected entries=2 (TRACE_MAX=0 should use default) (log=%s)\n", out_path.c_str());
    return 1;
  }
  if (output.find("Device::CreateResource") == std::string::npos || output.find("a0=0x111") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected first entry to be recorded (log=%s)\n", out_path.c_str());
    return 1;
  }
  if (output.find("Device::ValidateDevice") == std::string::npos || output.find("a0=0x222") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected second entry to be recorded (log=%s)\n", out_path.c_str());
    return 1;
  }
