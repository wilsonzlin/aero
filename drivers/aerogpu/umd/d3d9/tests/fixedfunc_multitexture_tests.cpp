...
size_t CountOpcode(const aerogpu::Shader* shader, uint32_t opcode) {
  if (!shader) {
    return 0;
  }
  const size_t size = shader->bytecode.size();
...
bool TestFixedfuncStage1Modulate2xEmitsMulAndAdd() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1: CURRENT = 2x(tex1 * CURRENT). This must lower to:
  // - 1 mul (tex1 * current)
  // - 1 add (doubling)
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopModulate2x);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=MODULATE2X")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg1, kD3dTaTexture);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG1=TEXTURE")) {
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(MODULATE2X)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "MODULATE2X => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x3u, "MODULATE2X => PS texld uses samplers s0 and s1")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpMul) == 1, "MODULATE2X => PS contains exactly 1 mul")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpAdd) == 1, "MODULATE2X => PS contains exactly 1 add")) {
      return false;
    }
  }
...
bool TestFixedfuncStage1Modulate4xEmitsMulAndTwoAdds() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1: CURRENT = 4x(tex1 * CURRENT). This must lower to:
  // - 1 mul (tex1 * current)
  // - 2 adds (doubling twice)
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopModulate4x);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=MODULATE4X")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg1, kD3dTaTexture);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG1=TEXTURE")) {
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(MODULATE4X)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "MODULATE4X => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x3u, "MODULATE4X => PS texld uses samplers s0 and s1")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpMul) == 1, "MODULATE4X => PS contains exactly 1 mul")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpAdd) == 2, "MODULATE4X => PS contains exactly 2 add")) {
      return false;
    }
  }
...
bool TestFixedfuncStage1SubtractUsesNegSourceModifier() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1: CURRENT = tex1 - CURRENT (subtract). The fixed-function builder lowers
  // SUBTRACT using an `add` instruction with a NEG source modifier on arg2.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopSubtract);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=SUBTRACT")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg1, kD3dTaTexture);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG1=TEXTURE")) {
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(SUBTRACT)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "SUBTRACT => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x3u, "SUBTRACT => PS texld uses samplers s0 and s1")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpMul) == 0, "SUBTRACT => PS contains no mul")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpAdd) == 1, "SUBTRACT => PS contains exactly 1 add")) {
      return false;
    }
    // `src_temp(reg=2, mod=NEG)` token: (1 << 24) | (0xE4 << 16) | 2.
    if (!Check(CountToken(dev->ps, 0x01E40002u) >= 1, "SUBTRACT => uses NEG source modifier for arg2")) {
      return false;
    }
  }
...
bool TestFixedfuncStage1AddSignedUsesBiasSourceModifier() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1: CURRENT = (tex1 + CURRENT) - 0.5 (add signed). The fixed-function builder lowers
  // ADDSIGNED by applying the ps_2_0 BIAS source modifier.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopAddSigned);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=ADDSIGNED")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg1, kD3dTaTexture);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG1=TEXTURE")) {
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(ADDSIGNED)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "ADDSIGNED => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x3u, "ADDSIGNED => PS texld uses samplers s0 and s1")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpMul) == 0, "ADDSIGNED => PS contains no mul")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpAdd) == 1, "ADDSIGNED => PS contains exactly 1 add")) {
      return false;
    }
    // `src_temp(reg=3, mod=BIAS)` token: (2 << 24) | (0xE4 << 16) | 3.
    if (!Check(CountToken(dev->ps, 0x02E40003u) >= 1, "ADDSIGNED => uses BIAS source modifier")) {
      return false;
    }
  }
...
bool TestFixedfuncBlendDiffuseAlphaDoesNotSampleTextureWhenArgsAreCurrent() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1 uses BLENDDIFFUSEALPHA but does not sample its stage1 texture when both args are CURRENT.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopBlendDiffuseAlpha);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=BLENDDIFFUSEALPHA")) {
    return false;
  }
  ...
  // Stage2 samples its texture. If stage1 incorrectly treats BLENDDIFFUSEALPHA as texture-sampling,
  // the chain would truncate before stage2.
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(BLENDDIFFUSEALPHA no sampling)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "BLENDDIFFUSEALPHA non-sampling => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x5u,
               "BLENDDIFFUSEALPHA non-sampling => PS texld uses samplers s0 and s2")) {
      return false;
    }
    // `BLENDDIFFUSEALPHA` lowers to: out = arg1 * diffuse.a + arg2 * (1 - diffuse.a)
    if (!Check(CountOpcode(dev->ps, kPsOpMul) == 2, "BLENDDIFFUSEALPHA => PS contains exactly 2 mul")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpAdd) == 1, "BLENDDIFFUSEALPHA => PS contains exactly 1 add")) {
      return false;
    }
    // Blend factor should come from diffuse alpha (input 0.w), not texture alpha (r0.w).
    if (!Check(CountToken(dev->ps, 0x10FF0000u) >= 1, "BLENDDIFFUSEALPHA => uses diffuse alpha")) {
      return false;
    }
    if (!Check(CountToken(dev->ps, 0x16FF0000u) >= 1, "BLENDDIFFUSEALPHA => uses (1 - diffuse alpha)")) {
      return false;
    }
    if (!Check(CountToken(dev->ps, 0x00FF0000u) == 0, "BLENDDIFFUSEALPHA => does not use texture alpha")) {
      return false;
    }
    if (!Check(CountToken(dev->ps, 0x06FF0000u) == 0, "BLENDDIFFUSEALPHA => does not use (1 - texture alpha)")) {
      return false;
    }
  }
...
bool TestFixedfuncStage1BlendTextureAlphaUsesTextureAlphaTokens() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1 uses BLENDTEXTUREALPHA. Even if both args are CURRENT, the blend
  // factor comes from texture alpha, so the generated PS must reference r0.w.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopBlendTextureAlpha);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=BLENDTEXTUREALPHA")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg1, kD3dTaCurrent);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG1=CURRENT")) {
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(BLENDTEXTUREALPHA)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "BLENDTEXTUREALPHA => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x3u, "BLENDTEXTUREALPHA => PS texld uses samplers s0 and s1")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpMul) == 2, "BLENDTEXTUREALPHA => PS contains exactly 2 mul")) {
      return false;
    }
    if (!Check(CountOpcode(dev->ps, kPsOpAdd) == 1, "BLENDTEXTUREALPHA => PS contains exactly 1 add")) {
      return false;
    }
    // Blend factor should come from texture alpha (r0.w), not diffuse alpha (input 0.w).
    if (!Check(CountToken(dev->ps, 0x00FF0000u) >= 1, "BLENDTEXTUREALPHA => uses texture alpha")) {
      return false;
    }
    if (!Check(CountToken(dev->ps, 0x06FF0000u) >= 1, "BLENDTEXTUREALPHA => uses (1 - texture alpha)")) {
      return false;
    }
    if (!Check(CountToken(dev->ps, 0x10FF0000u) == 0, "BLENDTEXTUREALPHA => does not use diffuse alpha")) {
      return false;
    }
    if (!Check(CountToken(dev->ps, 0x16FF0000u) == 0, "BLENDTEXTUREALPHA => does not use (1 - diffuse alpha)")) {
      return false;
    }
  }
...
bool TestFixedfuncStage1UsesTextureInAlphaOnlySamplesTexture() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1 uses texture1 only in the alpha path: COLOR = CURRENT, ALPHA = TEXTURE.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopSelectArg1);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=SELECTARG1")) {
    return false;
  }
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(stage1 alpha-only sampling)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 2, "stage1 alpha-only sampling => PS contains exactly 2 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x3u, "stage1 alpha-only sampling => PS texld uses samplers s0 and s1")) {
      return false;
    }
  }
...
bool TestFixedfuncStage2BlendTextureAlphaSamplesTextureEvenWhenArgsAreCurrent() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage2: BLENDTEXTUREALPHA consumes texture alpha as a blend factor even if
  // neither arg source is TEXTURE, so the shader must still sample stage2.
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(stage2 BLENDTEXTUREALPHA samples stage2)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 3,
               "stage2 BLENDTEXTUREALPHA => PS contains exactly 3 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x7u,
               "stage2 BLENDTEXTUREALPHA => PS texld uses samplers s0, s1, s2")) {
      return false;
    }
  }
...
bool TestFixedfuncUnboundStage0TextureTruncatesWhenStage0UsesTextureInAlphaOnly() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage0 intentionally left unbound. Bind a stage1 texture anyway to ensure it
  // is ignored when the chain truncates at stage0 due to alpha sampling.
  hr = cleanup.device_funcs.pfnSetTexture(cleanup.hDevice, /*stage=*/1, hTex1);
  if (!Check(hr == S_OK, "SetTexture(stage1)")) {
    return false;
  }

  // Stage0: color does not sample (CURRENT), but alpha selects TEXTURE. Since
  // stage0 texture is unbound, the driver must treat the chain as truncated and
  // fall back to the diffuse-only shader.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 0, kD3dTssColorOp, kD3dTopSelectArg1);
  if (!Check(hr == S_OK, "TSS stage0 COLOROP=SELECTARG1")) {
    return false;
  }
  ...
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 0, kD3dTssAlphaArg1, kD3dTaTexture);
  if (!Check(hr == S_OK, "TSS stage0 ALPHAARG1=TEXTURE (alpha sampling)")) {
    return false;
  }

  // Stage1 uses an unsupported op, but must be ignored because the stage chain is
  // already truncated due to stage0 missing its texture (via alpha sampling).
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopAddSmooth);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=ADDSMOOTH (unsupported, should be ignored)")) {
    return false;
  }
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(stage0 alpha texture missing)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 0, "stage0 alpha missing => PS contains no texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0, "stage0 alpha missing => PS uses no samplers")) {
      return false;
    }
  }
...
bool TestFixedfuncUnboundStage0AndStage1TexturesDoNotTruncateWhenTheyDoNotSampleAndStage2Samples() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage0 and stage1 intentionally left unbound. Bind a stage2 texture which will be sampled.
  hr = cleanup.device_funcs.pfnSetTexture(cleanup.hDevice, /*stage=*/2, hTex2);
  if (!Check(hr == S_OK, "SetTexture(stage2)")) {
    return false;
  }
  ...
  // Stage1 does not sample: CURRENT = CURRENT (stage1 active but texture unused).
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopSelectArg1);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=SELECTARG1")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg1, kD3dTaCurrent);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG1=CURRENT (no sampling)")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssAlphaOp, kD3dTopSelectArg1);
  if (!Check(hr == S_OK, "TSS stage1 ALPHAOP=SELECTARG1")) {
  ...
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssAlphaArg1, kD3dTaCurrent);
  if (!Check(hr == S_OK, "TSS stage1 ALPHAARG1=CURRENT (no sampling)")) {
    return false;
  }
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == S_OK, "DrawPrimitiveUP(stage0/1 missing but no sampling)")) {
    return false;
  }

  {
    ...
    if (!Check(CountToken(dev->ps, kPsOpTexld) == 1, "sparse sampling => PS contains exactly 1 texld")) {
      return false;
    }
    if (!Check(TexldSamplerMask(dev->ps) == 0x4u, "sparse sampling => PS texld uses only sampler s2")) {
      return false;
    }
  }
...
bool TestFixedfuncInvalidStage1Arg2FailsDrawWithInvalidCall() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage1 uses an op that consumes ARG2 (MODULATE), but supplies an invalid value
  // in the *used* arg2 slot. State-setting should succeed, but draws must fail with
  // INVALIDCALL.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorOp, kD3dTopModulate);
  if (!Check(hr == S_OK, "TSS stage1 COLOROP=MODULATE")) {
    return false;
  ...
  constexpr uint32_t kInvalidArg = 0x80000000u;
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssColorArg2, kInvalidArg);
  if (!Check(hr == S_OK, "TSS stage1 COLORARG2=invalid (unsupported at draw time)")) {
    return false;
  }
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 1, kD3dTssAlphaOp, kD3dTopSelectArg1);
  if (!Check(hr == S_OK, "TSS stage1 ALPHAOP=SELECTARG1")) {
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == kD3DErrInvalidCall, "DrawPrimitiveUP(invalid stage1 arg2) returns D3DERR_INVALIDCALL")) {
    return false;
  }
...
bool TestFixedfuncUnsupportedStage3OpFailsDrawWithInvalidCall() {
  CleanupDevice cleanup;
  if (!CreateDevice(&cleanup)) {
    return false;
  }
  ...
  // Stage3 uses an unsupported op. State-setting should succeed, but fixed-function draws must fail with INVALIDCALL.
  hr = aerogpu::device_set_texture_stage_state(cleanup.hDevice, 3, kD3dTssColorOp, kD3dTopAddSmooth);
  if (!Check(hr == S_OK, "TSS stage3 COLOROP=ADDSMOOTH (unsupported)")) {
    return false;
  }
  ...
  hr = cleanup.device_funcs.pfnDrawPrimitiveUP(
      cleanup.hDevice, D3DDDIPT_TRIANGLELIST, /*primitive_count=*/1, tri, sizeof(tri[0]));
  if (!Check(hr == kD3DErrInvalidCall, "DrawPrimitiveUP(unsupported stage3) returns D3DERR_INVALIDCALL")) {
    return false;
  }
...
int main() {
  ...
  if (!TestFixedfuncStage1Modulate2xEmitsMulAndAdd()) {
    return 1;
  }
  if (!TestFixedfuncStage1Modulate4xEmitsMulAndTwoAdds()) {
    return 1;
  }
  if (!TestFixedfuncStage1SubtractUsesNegSourceModifier()) {
    return 1;
  }
  if (!TestFixedfuncStage1AddSignedUsesBiasSourceModifier()) {
    return 1;
  }
  ...
  if (!TestFixedfuncBlendDiffuseAlphaDoesNotSampleTextureWhenArgsAreCurrent()) {
    return 1;
  }
  if (!TestFixedfuncStage1BlendTextureAlphaUsesTextureAlphaTokens()) {
    return 1;
  }
  if (!TestFixedfuncStage1UsesTextureInAlphaOnlySamplesTexture()) {
    return 1;
  }
  ...
  if (!TestFixedfuncUnboundStage0AndStage1TexturesDoNotTruncateWhenTheyDoNotSampleAndStage2Samples()) {
    return 1;
  }
  ...
  if (!TestFixedfuncStage2BlendTextureAlphaSamplesTextureEvenWhenArgsAreCurrent()) {
    return 1;
  }
  ...
  if (!TestFixedfuncInvalidStage1Arg2FailsDrawWithInvalidCall()) {
    return 1;
  }
  if (!TestFixedfuncUnsupportedStage1AlphaOpFailsDrawWithInvalidCall()) {
    return 1;
  }
  if (!TestFixedfuncUnsupportedStage3OpFailsDrawWithInvalidCall()) {
    return 1;
  }
