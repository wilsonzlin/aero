...
int main() {
  ...
  const std::string out_path = make_unique_log_path("aerogpu_d3d9_trace_filter_whitespace_only_disables_tests");
  ...
  // All-whitespace filter should be treated as "unset" (filter disabled).
  ...
  if (output.find("filter_on=0") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected filter_on=0 for whitespace-only TRACE_FILTER (log=%s)\n", out_path.c_str());
    return 1;
  }
