...
int main() {
  ...
  const std::string out_path = make_unique_log_path("aerogpu_d3d9_trace_filter_commas_only_disables_tests");
  ...
  // A filter that contains only delimiters/whitespace should be treated as "unset"
  // (filter disabled).
  set_env("AEROGPU_D3D9_TRACE_FILTER", " ,,, , ");
  ...
  // Calls should still be recorded because the filter is disabled.
  {
  ...
  if (output.find("filter_on=0") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected filter_on=0 for delimiter-only TRACE_FILTER (log=%s)\n", out_path.c_str());
    return 1;
  }
  if (output.find("Device::CreateResource") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected CreateResource to be recorded (log=%s)\n", out_path.c_str());
    return 1;
  }
