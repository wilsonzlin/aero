...
bool TestFvfXyzDiffuseTex1ReuploadsWvpAfterUserVsClobbersConstants() {
  ...
  hr = cleanup.device_funcs.pfnSetShaderConstF(create_dev.hDevice, kD3d9ShaderStageVs, 0, clobber, 4);
  if (!Check(hr == S_OK, "SetShaderConstF(VS, c0..c3)")) {
    return false;
  }
...
bool TestFvfXyzDiffuseDrawPrimitiveUpEmitsWvpConstants() {
  ...
  // VIEW: uniform scale by 2 so the uploaded matrix isn't all-identity.
  D3DMATRIX view{};
  view.m[0][0] = 2.0f;
  view.m[1][1] = 2.0f;
  view.m[2][2] = 2.0f;
  ...
  constexpr uint32_t kGreen = 0xFF00FF00u;
  Vertex verts[3]{};
  verts[0] = {-0.5f, -0.5f, 0.0f, kGreen};
  verts[1] = {0.5f, -0.5f, 0.0f, kGreen};
  verts[2] = {0.0f, 0.5f, 0.0f, kGreen};
  ...
  const CmdLoc consts = FindLastOpcodeBefore(buf, len, draw.offset, AEROGPU_CMD_SET_SHADER_CONSTANTS_F);
  if (!Check(consts.hdr != nullptr, "set_shader_constants_f emitted before draw")) {
    return false;
  }
  const auto* const_cmd = reinterpret_cast<const aerogpu_cmd_set_shader_constants_f*>(consts.hdr);
  if (!Check(const_cmd->stage == AEROGPU_SHADER_STAGE_VERTEX, "set_shader_constants_f stage==VERTEX")) {
    return false;
  }
  if (!Check(const_cmd->start_register == 0 && const_cmd->vec4_count == 4, "set_shader_constants_f covers c0..c3")) {
    return false;
  }
  ...
  if (!Check(std::fabs(m[5] - 2.0f) < 1e-6f, "WVP[1][1] = 2")) {
    return false;
  }
  if (!Check(std::fabs(m[10] - 2.0f) < 1e-6f, "WVP[2][2] = 2")) {
    return false;
  }
  if (!Check(std::fabs(m[15] - 1.0f) < 1e-6f, "WVP[3][3] = 1")) {
    return false;
  ...
  return Check(consts.offset < draw.offset, "set_shader_constants_f occurs before draw");
}
...
int main() {
  ...
  failures += !aerogpu::TestDeviceFuncsIncludePatchAndProcessVertices();
  ...
  failures += !aerogpu::TestLockMipmappedTextureReportsSubresourcePitch();
  ...
  failures += !aerogpu::TestLockSizeZeroClampsToBc2MipSubresourceFromInteriorOffset();
  ...
  failures += !aerogpu::TestGenerateMipSubLevelsBoxFilter2dArrayTexture();
  failures += !aerogpu::TestGenerateMipSubLevelsBoxFilter2dX1R5G5B5();
  failures += !aerogpu::TestGenerateMipSubLevelsBoxFilter2dR5G6B5();
  failures += !aerogpu::TestGenerateMipSubLevelsBoxFilter2dA1R5G5B5();
  failures += !aerogpu::TestGenerateMipSubLevelsBoxFilter2dBcUniform();
  ...
  failures += !aerogpu::TestCopyRects16BitToGuestBackedResourceEmitsDirtyRange();
  failures += !aerogpu::TestCopyRects16BitTransferEmitsCopyTexture2D();
  ...
  failures += !aerogpu::TestFixedFuncPsSelectionUnsetTextureUpdatesBoundPsToColorOnlyWithoutDraw();
  failures += !aerogpu::TestFixedFuncPsSelectionTex1OnlyFvfSetTextureUpdatesBoundPsToModulateWithoutDraw();
  failures += !aerogpu::TestFixedFuncPsSelectionTex1OnlyFvfSelectArg1TextureUsesTextureOnly();
  ...
  failures += !aerogpu::TestFixedFuncPsSelectionTextureBoundModulateDiffuseDiffuseUsesColorOnly();
  failures += !aerogpu::TestFixedFuncPsSelectionTextureBoundUnknownColorOpDiffuseArgsUsesColorOnly();
  ...
  failures += !aerogpu::TestFvfXyzDiffuseDrawPrimitiveUpDoesNotConvertVertices();
  failures += !aerogpu::TestFixedfuncStrideTooSmallFailsAndDoesNotEmitDraw();
  failures += !aerogpu::TestFixedfuncIndexedStrideTooSmallFailsAndDoesNotEmitDraw();
  ...
  failures += !aerogpu::TestGuestBackedUpdateSurface16BitEmitsDirtyRangeNotUpload();
  failures += !aerogpu::TestUpdateSurface16BitToHostBackedResourceEmitsUpload();
  failures += !aerogpu::TestGuestBackedUpdateTextureEmitsDirtyRangeNotUpload();
  failures += !aerogpu::TestGuestBackedUpdateTexture16BitEmitsDirtyRangeNotUpload();
