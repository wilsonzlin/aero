...
int main() {
  ...
  const std::string out_path = make_unique_log_path("aerogpu_d3d9_trace_dump_on_stub_second_tests");
  ...
  // Use a second trace-only entrypoint that is intentionally "(stub)"-tagged,
  // to ensure dump-on-stub is driven by the marker rather than a single hard-coded trace ID.
  {
    aerogpu::D3d9TraceCall trace(aerogpu::D3d9TraceFunc::TraceTestStub2, 0xabc, 0, 0, 0);
    trace.ret(S_OK);
  }

  const std::string output = slurp_file_after_closing_stderr(out_path);
  if (output.find("dump reason=Trace::TestStub2 (stub)") == std::string::npos) {
    std::fprintf(stdout, "FAIL: expected dump reason Trace::TestStub2 (stub) (log=%s)\n", out_path.c_str());
    return 1;
  }
  if (output.find("Trace::TestStub2 (stub)") == std::string::npos) {
