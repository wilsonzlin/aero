...
// -----------------------------------------------------------------------------
// AeroGPU portable D3D9UMDDI arg shims for WDK builds
// -----------------------------------------------------------------------------
//
// The core D3D9 translation layer in this repository is written against the
// portable ABI subset above (with repo-friendly member names like
// `offset_bytes`, `sync_interval`, etc).
//
// When building against the real Win7 WDK headers (`AEROGPU_UMD_USE_WDK_HEADERS=1`)
// the canonical `D3D9DDIARG_*` structs use the WDK member naming (e.g.
// `OffsetToLock`, `SyncInterval`). To keep the translation layer independent of
// those header details, we expose a small set of "compat" structs under the
// `AEROGPU_*` names and use WDK-typed wrappers to memcpy the prefix fields.
//
// In portable builds, these are simple aliases to the portable `D3D9DDIARG_*`
// definitions so repo-only callers can use either naming.

#if defined(_WIN32) && defined(AEROGPU_UMD_USE_WDK_HEADERS) && AEROGPU_UMD_USE_WDK_HEADERS
typedef struct _AEROGPU_D3D9DDIARG_CREATERESOURCE {
  uint32_t type;   // driver-defined
  uint32_t format; // driver-defined (D3DFORMAT numeric)
  uint32_t width;
  uint32_t height;
  ...
  uint32_t wddm_hAllocation; // optional (WDK: hAllocation)
} AEROGPU_D3D9DDIARG_CREATERESOURCE;

typedef struct _AEROGPU_D3D9DDIARG_OPENRESOURCE {
  const void* pPrivateDriverData;
  uint32_t private_driver_data_size; // WDK: PrivateDriverDataSize
  uint32_t type;
  uint32_t format;
  uint32_t width;
  uint32_t height;
...
typedef struct _AEROGPU_D3D9DDIARG_LOCK {
  D3DDDI_HRESOURCE hResource;
  uint32_t offset_bytes; // WDK: OffsetToLock
  uint32_t size_bytes;   // WDK: SizeToLock
  uint32_t flags;        // WDK: Flags
} AEROGPU_D3D9DDIARG_LOCK;

typedef struct _AEROGPU_D3D9DDIARG_UNLOCK {
  D3DDDI_HRESOURCE hResource;
  uint32_t offset_bytes; // WDK: OffsetToUnlock
  uint32_t size_bytes;   // WDK: SizeToUnlock
} AEROGPU_D3D9DDIARG_UNLOCK;

typedef struct _AEROGPU_D3D9DDIARG_PRESENT {
  D3DDDI_HRESOURCE hSrc;
  D3D9DDI_HSWAPCHAIN hSwapChain;
  HWND hWnd;
  uint32_t sync_interval; // WDK: SyncInterval
  uint32_t flags;         // WDK: Flags
} AEROGPU_D3D9DDIARG_PRESENT;

typedef struct _AEROGPU_D3D9DDIARG_PRESENTEX {
  D3DDDI_HRESOURCE hSrc;
  HWND hWnd;
  uint32_t sync_interval;      // WDK: SyncInterval
  uint32_t d3d9_present_flags; // WDK: Flags
} AEROGPU_D3D9DDIARG_PRESENTEX;

typedef D3DDDI_LOCKEDBOX AEROGPU_D3D9DDI_LOCKED_BOX;
#else
typedef D3D9DDIARG_CREATERESOURCE AEROGPU_D3D9DDIARG_CREATERESOURCE;
typedef D3D9DDIARG_OPENRESOURCE AEROGPU_D3D9DDIARG_OPENRESOURCE;
typedef D3D9DDIARG_LOCK AEROGPU_D3D9DDIARG_LOCK;
typedef D3D9DDIARG_UNLOCK AEROGPU_D3D9DDIARG_UNLOCK;
typedef D3D9DDIARG_PRESENT AEROGPU_D3D9DDIARG_PRESENT;
typedef D3D9DDIARG_PRESENTEX AEROGPU_D3D9DDIARG_PRESENTEX;
typedef D3DDDI_LOCKEDBOX AEROGPU_D3D9DDI_LOCKED_BOX;
#endif
