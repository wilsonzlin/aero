...
class VirtioBlkMsixHostMarkerTests(unittest.TestCase):
    @classmethod
    ...
    def _emit(self, tail: bytes) -> str:
        ...
        with contextlib.redirect_stdout(buf):
            self.harness._emit_virtio_blk_msix_host_marker(tail)
    ...
    def test_emits_skip_marker(self) -> None:
        tail = b"AERO_VIRTIO_SELFTEST|TEST|virtio-blk-msix|SKIP|reason=ioctl_payload_v1_or_truncated|returned_len=8\n"
        out = self._emit(tail)
        self.assertEqual(
            out,
