...
class QmpInputInjectionTests(unittest.TestCase):
    ...
    def test_falls_back_to_vscroll_and_hwheel_when_both_axes_rejected(self) -> None:
        h = self.harness
        ...
        def fake_send_command(sock, cmd):
            sent.append(cmd)
            # Simulate a QEMU build that rejects `axis=wheel` and `axis=hscroll` but accepts
            # `axis=vscroll` and `axis=hwheel`.
            evs = cmd.get("arguments", {}).get("events", [])
            for ev in evs:
                if ev.get("type") == "rel":
                    axis = ev.get("data", {}).get("axis")
                    if axis in ("wheel", "hscroll"):
                        raise RuntimeError(f"axis {axis} unsupported")
            return {"return": {}}
        ...
        try:
            ...
            # Ensure we attempted both original axes, then eventually used the dual-fallback pair.
            self.assertTrue(any("wheel" in axes for axes in rel_axes_sets))
            self.assertTrue(any("hscroll" in axes for axes in rel_axes_sets))
            self.assertTrue(any(("vscroll" in axes and "hwheel" in axes) for axes in rel_axes_sets))
        finally:
            h._qmp_connect = old_connect
            h._qmp_send_command = old_send
            h.time.sleep = old_sleep
