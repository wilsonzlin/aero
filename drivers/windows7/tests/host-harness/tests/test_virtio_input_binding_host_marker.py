...
class VirtioInputBindingHostMarkerTests(unittest.TestCase):
    @classmethod
    ...
    def test_emits_pass_marker(self) -> None:
        tail = (
            ...
            b"pnp_id=PCI\\\\VEN_1AF4&DEV_1052\\\\1234|devices=2|wrong_service=0|missing_service=0|problem=0\n"
        )
        out = self._emit(tail)
        self.assertEqual(
            out,
            "AERO_VIRTIO_WIN7_HOST|VIRTIO_INPUT_BIND|PASS|service=aero_virtio_input|"
            "pnp_id=PCI\\\\VEN_1AF4&DEV_1052\\\\1234|devices=2|wrong_service=0|missing_service=0|problem=0",
        )
    ...
    def test_emits_fail_marker(self) -> None:
        tail = (
            ...
            b"actual=vioinput|pnp_id=PCI\\\\VEN_1AF4&DEV_1052\\\\1234|devices=2|wrong_service=2|missing_service=0|problem=0\n"
        )
        out = self._emit(tail)
        self.assertEqual(
            out,
            "AERO_VIRTIO_WIN7_HOST|VIRTIO_INPUT_BIND|FAIL|reason=wrong_service|expected=aero_virtio_input|"
            "actual=vioinput|pnp_id=PCI\\\\VEN_1AF4&DEV_1052\\\\1234|devices=2|wrong_service=2|missing_service=0|problem=0",
        )
