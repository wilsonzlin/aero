...
class VirtioInputMsixRequireTests(unittest.TestCase):
    @classmethod
    ...
    def test_pass_when_mode_msix(self) -> None:
        ...
        ok, reason = self.harness._require_virtio_input_msix_marker(tail)
        self.assertTrue(ok, reason)

    def test_fails_when_mode_intx(self) -> None:
        tail = b"AERO_VIRTIO_SELFTEST|TEST|virtio-input-msix|PASS|mode=intx|messages=0\n"
        ok, reason = self.harness._require_virtio_input_msix_marker(tail)
        self.assertFalse(ok)
        self.assertIn("mode=intx", reason)

    def test_fails_when_mode_unknown(self) -> None:
        tail = b"AERO_VIRTIO_SELFTEST|TEST|virtio-input-msix|PASS|mode=unknown|messages=0\n"
        ok, reason = self.harness._require_virtio_input_msix_marker(tail)
        self.assertFalse(ok)
        self.assertIn("mode=unknown", reason)
    ...
    def test_fails_on_fail(self) -> None:
        tail = b"AERO_VIRTIO_SELFTEST|TEST|virtio-input-msix|FAIL|reason=query_failed|err=5\n"
        ok, reason = self.harness._require_virtio_input_msix_marker(tail)
        self.assertFalse(ok)
        self.assertIn("FAIL", reason)

    def test_fails_when_mode_missing(self) -> None:
        tail = b"AERO_VIRTIO_SELFTEST|TEST|virtio-input-msix|PASS|messages=3|mapping=all-on-vector0\n"
        ok, reason = self.harness._require_virtio_input_msix_marker(tail)
        self.assertFalse(ok)
