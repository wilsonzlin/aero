...
class VirtioBlkIrqMarkerTests(unittest.TestCase):
    ...
    def test_emits_irq_message_count_from_message_count_in_test_marker(self) -> None:
        tail = (
            b"AERO_VIRTIO_SELFTEST|TEST|virtio-blk|PASS|irq_mode=msix|message_count=2|msix_config_vector=0|"
            b"msix_queue_vector=1\n"
        )
        out = self._emit(tail)
        self.assertEqual(
