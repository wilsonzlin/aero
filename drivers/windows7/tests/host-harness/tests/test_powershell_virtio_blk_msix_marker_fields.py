...
class PowerShellVirtioBlkMsixMarkerFieldsTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_host_marker_emits_unknown_fields_sorted(self) -> None:
        ...
        fn = self._extract_function_text("Try-EmitAeroVirtioBlkMsixMarker")

        self.assertRegex(
            fn,
            r'\$ordered\s*=\s*@\(\s*"mode"\s*,\s*"messages"\s*,\s*"config_vector"\s*,\s*"queue_vector"',
        )
