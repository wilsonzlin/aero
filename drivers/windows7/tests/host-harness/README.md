   ...
   - Note: This requires that the virtio-input driver is installed and that the tablet device is bound so it exposes a
      HID interface.
        ...
        - When provisioning via `New-AeroWin7TestImage.ps1`, the tablet INF is installed by default when present; if you pass
          ...
          for generic fallback binding, include `virtio-input.inf` instead (and do not ship/install overlapping virtio-input INFs).
...
- virtio-input (keyboard + mouse): `drivers/windows7/virtio-input/inf/aero_virtio_input.inf` (binds `DEV_1052&REV_01`; uses `SUBSYS_0010/0011` for distinct naming)
