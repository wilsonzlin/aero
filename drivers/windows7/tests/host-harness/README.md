   ...
   - Note: This requires that the virtio-input driver is installed and that the tablet device is bound so it exposes a
      HID interface.
          ...
          - When provisioning via `New-AeroWin7TestImage.ps1`, the tablet INF is installed by default when present; if you pass
            ...
            to exercise the contract tablet binding specifically). If you are intentionally using the opt-in legacy alias INF
            for generic fallback binding, include the legacy alias INF instead (the one that matches `PCI\VEN_1AF4&DEV_1052&REV_01`).
...
- Logs artifact: `win7-virtio-harness-logs` (serial + harness output + HTTP request log; QEMU stderr sidecar when present; empty serial/HTTP logs are replaced with a placeholder line)
- Job summary: surfaces key PASS/FAIL markers, QEMU stderr/argv, and (on failure) collapsible tails of the logs above
...
to inspect the generated QEMU args. You can also leave `disk_image_path` empty in `dry_run` mode; the workflow will
create a temporary placeholder file under the logs directory and use that path for argument generation.
...
To attach a virtio-tablet device without enabling the tablet events selftest/injection path, set the workflow input
 ...
 either emulate the subsystem IDs to the contract values or opt into the legacy virtio-input INF alias (disabled-by-default)
 for generic fallback binding (`PCI\VEN_1AF4&DEV_1052&REV_01`).
