      ...
      - Note: This requires that the virtio-input driver is installed and that the tablet device is bound so it exposes a
        HID interface.
         ...
         - For **stock QEMU** `virtio-tablet-pci` devices (which typically use non-Aero subsystem IDs), the device is expected
           ...
           `drivers/windows7/virtio-input/inf/`, checked in as `*.inf.disabled`; drop the `.disabled` suffix to enable).
           When binding via the fallback entry, the driver classifies it as a tablet via `EV_BITS` (`EV_ABS` + `ABS_X`/`ABS_Y`).
...
This is informational only and does not affect overall PASS/FAIL unless `-RequireNetCsumOffload` / `--require-net-csum-offload` is enabled.
