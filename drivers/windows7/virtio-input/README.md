...
- Legacy filename alias (disabled by default): `inf/virtio-input.inf.disabled`
  ...
  - Enabling the alias **does** change HWID matching behavior; do **not** ship/install it alongside `aero_virtio_input.inf`
    (ship/install only one INF basename at a time).
