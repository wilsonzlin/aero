...
>   byte-for-byte identical to `inf/aero_virtio_input.inf` (only the leading banner/comments may differ; helper:
>   `scripts/check-inf-alias.py`; CI: `scripts/ci/check-windows7-virtio-contract-consistency.py`).
> - The alias does **not** change HWID matching behavior, and enabling it is **not** required for fallback binding.
>
> Enabling the alias is only required for filename compatibility with tooling that still expects `virtio-input.inf`.
>
...
To validate the optional legacy filename alias INF stays in sync with the canonical INF from the first section header (`[Version]`) onward
(ignoring only the leading banner/comments), run:
...
When binding via the fallback model line (`PCI\VEN_1AF4&DEV_1052&REV_01`), Device Manager will show the generic
...
ID), the generic fallback entry can also bind to tablet devices.

Legacy filename alias note: `inf/virtio-input.inf.disabled` is a filename alias only. From the first section header
(`[Version]`) onward it must remain byte-for-byte identical to `inf/aero_virtio_input.inf` (banner/comments may differ; see
`scripts/check-inf-alias.py`). It does not change HWID matching behavior. Do not ship/install it alongside
`aero_virtio_input.inf` (install only one basename at a time).
...
In that case, Windows can still bind via the strict generic fallback model line (`PCI\VEN_1AF4&DEV_1052&REV_01`) as long as
the device reports `REV_01`, but the devices will appear as the generic **Aero VirtIO Input Device** rather than distinct
keyboard/mouse names.

If you want distinct keyboard/mouse names in Device Manager, emulate the subsystem IDs to the contract values
(`SUBSYS_0010` / `SUBSYS_0011` / `SUBSYS_0012`).

Tablet devices bind via `inf/aero_virtio_tablet.inf` when that INF is installed/matches. The tablet HWID is more specific
...
for `virtio-input.inf`. It is a filename alias only and does not change HWID matching behavior; from the first section header
(`[Version]`) onward it must remain byte-for-byte identical to `inf/aero_virtio_input.inf` (banner/comments may differ; see
