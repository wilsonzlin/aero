...
use the legacy alias INF `inf/virtio-input.inf.disabled`, which includes a fallback
`PCI\VEN_1AF4&DEV_1052&REV_01` match (rename it to `virtio-input.inf` to enable it). Keep in mind that shipping
multiple overlapping INFs that match the same device can lead to confusing install behavior.
