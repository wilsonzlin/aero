...
> - The legacy alias INF is a filename-only alias:
>   - From the first section header (`[Version]`) onward, it is expected to stay **byte-for-byte identical** to
>     `inf/aero_virtio_input.inf` (leading banner/comments may differ; helper: `scripts/check-inf-alias.py`;
...
>     fallback binding).
>
> Do not ship/install the alias alongside `aero_virtio_input.inf`: duplicate overlapping INFs can lead to confusing PnP selection.
> Ship/install **only one** of the two basenames (`aero_virtio_input.inf` *or* `virtio-input.inf`) at a time.
...
When binding via the generic fallback model line (`PCI\VEN_1AF4&DEV_1052&REV_01`), Device Manager will show
...
To validate the optional legacy filename alias INF stays in sync with the canonical INF from the first section header
(`[Version]`) onward (banner/comments may differ), run:
...
`inf/aero_virtio_input.inf` includes subsystem-qualified keyboard/mouse HWIDs (`SUBSYS_0010`/`SUBSYS_0011`) for distinct Device
Manager names, plus the strict generic fallback HWID (`PCI\VEN_1AF4&DEV_1052&REV_01`, no `SUBSYS`).
...
generic fallback entry can also bind to tablet devices.

Legacy filename alias note: `inf/virtio-input.inf.disabled` (rename to `virtio-input.inf` to enable) is a filename alias only.
From `[Version]` onward, it is expected to match `inf/aero_virtio_input.inf` byte-for-byte (banner/comments may differ; see
`scripts/check-inf-alias.py`). Because it is identical, enabling it does **not** change HWID matching behavior. Do not
ship/install it alongside `aero_virtio_input.inf` (install only one INF basename at a time).
...
`inf/aero_virtio_input.inf` includes a strict revision-gated generic fallback match (`PCI\VEN_1AF4&DEV_1052&REV_01`, no
`SUBSYS`), so devices without the Aero subsystem IDs can still bind. When binding via the fallback entry, Device Manager will
show the generic **Aero VirtIO Input Device** rather than distinct keyboard/mouse names.

If you want distinct keyboard/mouse names under QEMU, emulate the subsystem IDs to the contract values
(`SUBSYS_0010` / `SUBSYS_0011`).
...
(or the device does not expose the tablet subsystem ID), the generic fallback entry can also bind to tablet devices.

The legacy filename alias INF (`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable) exists only for
compatibility with workflows/tools that still look for `virtio-input.inf`. It is a filename alias only: from `[Version]` onward
it is expected to match `inf/aero_virtio_input.inf` byte-for-byte (banner/comments may differ; see `scripts/check-inf-alias.py`).
Because it is identical, enabling it does **not** change HWID matching behavior.

Do not ship/install it alongside `aero_virtio_input.inf`: overlapping bindings can lead to confusing driver selection (ship only
one INF basename at a time).
