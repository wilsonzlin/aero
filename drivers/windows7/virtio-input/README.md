...
> - The legacy alias INF is a **filename alias only**. From the first section header (`[Version]`) onward, it is expected to be
>   byte-for-byte identical to `inf/aero_virtio_input.inf` (banner/comments may differ; helper:
...
>
> Do not ship/install the alias alongside `aero_virtio_input.inf`: the two INFs match the same HWIDs, which can lead to duplicate
> DriverStore entries and confusing PnP selection. Ship/install **only one** of the two basenames (`aero_virtio_input.inf` *or*
...
- Legacy filename alias (disabled by default): `inf/virtio-input.inf.disabled`
  - Rename it to `virtio-input.inf` to enable it (for workflows/tools that still reference `virtio-input.inf`).
  - Policy: filename alias only. From the first section header (`[Version]`) onward, expected to match
    `inf/aero_virtio_input.inf` byte-for-byte (banner/comments may differ; see `scripts/check-inf-alias.py`).
  - Enabling the alias does **not** change HWID matching behavior.
...
When binding via the generic fallback model line (`PCI\VEN_1AF4&DEV_1052&REV_01`, no `SUBSYS`), Device Manager will show
...
fallback entry in `inf/aero_virtio_input.inf` can also bind to tablet devices (but will use the generic device name).
...
To validate the optional legacy filename alias INF stays in sync with the canonical INF from the first section header (`[Version]`)
onward (banner/comments may differ), run:
...
`inf/aero_virtio_input.inf` (keyboard/mouse) includes:
...
ID), the generic fallback entry in `inf/aero_virtio_input.inf` can also bind to tablet devices (but will use the generic
device name).

Legacy alias note: `inf/virtio-input.inf.disabled` is a filename-only alias for basename compatibility (rename to
`virtio-input.inf` to enable). From the first section header (`[Version]`) onward it is expected to remain byte-for-byte
identical to `inf/aero_virtio_input.inf` (banner/comments may differ; see `scripts/check-inf-alias.py`) and does **not**
change HWID matching behavior. Do not ship/install both basenames at the same time.
...
The canonical keyboard/mouse INF (`inf/aero_virtio_input.inf`) includes a strict revision-gated generic fallback match
(`PCI\VEN_1AF4&DEV_1052&REV_01`, no `SUBSYS`), so devices without the Aero subsystem IDs can still bind as long as they report
`REV_01`. When binding via this fallback entry, Device Manager will show the generic **Aero VirtIO Input Device** name rather
than distinct keyboard/mouse names.

If you want distinct keyboard/mouse (and tablet) Device Manager names under Windows, emulate the subsystem IDs to the contract
values (`SUBSYS_0010` / `SUBSYS_0011` / `SUBSYS_0012`) and install the corresponding INFs.
...
so it wins over the generic fallback when both packages are present. If the tablet INF is not installed (or the device does not expose
the tablet subsystem ID), the generic fallback entry in `inf/aero_virtio_input.inf` can also bind to tablet devices (but will use the
generic device name).

The legacy filename alias INF (`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable) exists only for compatibility
with workflows/tools that still look for `virtio-input.inf`. It is a filename alias only: from the first section header (`[Version]`)
onward it is expected to match `inf/aero_virtio_input.inf` byte-for-byte (banner/comments may differ; see `scripts/check-inf-alias.py`).
