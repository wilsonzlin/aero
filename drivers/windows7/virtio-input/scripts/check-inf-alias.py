...
onward, except that the alias is allowed to differ in the models sections
(`Aero.NTx86` / `Aero.NTamd64`) to provide an opt-in generic fallback HWID.

Comments and empty lines are ignored for comparison.
...
def inf_functional_lines(path: Path) -> list[str]:
    ...
    - Starts at the first section header (or the first non-comment line if one
      appears before any section header; this avoids masking functional drift).
    ...
    for line in raw_lines[start:]:
        ...
        if stripped.startswith("[") and "]" in stripped:
            ...
            skip_section = sect_name in MODELS_SECTIONS
            if skip_section:
                continue
