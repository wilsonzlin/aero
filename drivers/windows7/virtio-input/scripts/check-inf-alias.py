...
Comparison notes:
  - The policy check is intentionally byte-based. It does not normalize
    whitespace, comments, section casing, or line endings.
...
def inf_bytes_from_first_section(path: Path) -> bytes:
    """Return the file content starting from the first section header line.

    We intentionally ignore the leading banner/comments so the alias INF can have
    a different filename banner, while still enforcing byte-for-byte equality for
    all sections/keys.
    """
    ...
    for i, line in enumerate(lines):
        ...
        # it as part of the compared region to avoid masking drift.
        return b"".join(lines[i:])
...
def _decode_lines_for_diff(data: bytes) -> list[str]:
    ...
    # Split on LF so CRLF endings remain visible as a trailing "\\r" marker.
    out: list[str] = []
...
def main() -> int:
    ...
    try:
        canonical_body = inf_bytes_from_first_section(canonical)
        alias_body = inf_bytes_from_first_section(alias)
    ...
    sys.stderr.write(
        "From the first section header (`[Version]`) onward, the alias INF must be "
        "byte-for-byte identical to the canonical INF.\n"
    )
    sys.stderr.write(
        "Fix: copy the canonical INF content starting at `[Version]` into the alias "
    ...
    if diff:
        ...
        sys.stderr.write(
            "Files differ at the byte level, but a line-based diff was not able to "
            "produce output. Ensure the alias is copied byte-for-byte from the canonical "
            "file starting at `[Version]`.\n"
        )
