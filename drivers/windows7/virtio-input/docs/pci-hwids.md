...
* Windows 7 will show the device as an unknown PCI device until a matching driver
  ...
  **Aero contract v1** hardware IDs (revision-gated `REV_01`):
  - `inf/aero_virtio_input.inf` (keyboard/mouse):
    ...
    - Note: the canonical keyboard/mouse INF is intentionally **SUBSYS-only** (no strict generic fallback).
  ...
  - Optional legacy filename alias `inf/virtio-input.inf.disabled` (disabled by default; rename to `virtio-input.inf` to enable):
    ...
    - Adds opt-in strict generic fallback matching (no `SUBSYS`):
      - `PCI\VEN_1AF4&DEV_1052&REV_01` (shown as **Aero VirtIO Input Device** when binding via that line)
    - Alias drift policy: allowed to diverge from `inf/aero_virtio_input.inf` only in the models sections
      (`[Aero.NTx86]` / `[Aero.NTamd64]`) where it adds the fallback model line. Outside those models sections, from the first
      section header (`[Version]`) onward, it must remain byte-for-byte identical (banner/comments may differ; see `../scripts/check-inf-alias.py`).
  ...
  so it wins over the generic fallback when the opt-in alias is enabled and both driver packages are installed.
...
The canonical keyboard/mouse INF (`inf/aero_virtio_input.inf`) is intentionally **SUBSYS-only** and does not include a strict generic
fallback model line (no `SUBSYS`).

For opt-in strict generic fallback binding, the repo also carries a legacy filename alias INF
(`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable), which adds:
...
so it wins over the generic fallback when the opt-in alias is enabled and both driver packages are installed. If the tablet INF is
not installed (or the device does not expose the tablet subsystem ID), the generic fallback entry (when enabled via the alias INF)
can also bind to tablet devices (but will use the generic device name).

Alias drift policy: the legacy alias INF is allowed to diverge from `inf/aero_virtio_input.inf` only in the models sections
(`[Aero.NTx86]` / `[Aero.NTamd64]`) where it adds the fallback line above. Outside those models sections, from the first section
header (`[Version]`) onward it must remain byte-for-byte identical (banner/comments may differ; see `../scripts/check-inf-alias.py`).
Enabling the alias does **change** HWID matching behavior (it enables strict generic fallback binding).

Do not ship/install the alias alongside `aero_virtio_input.inf` (install only one of the two basenames at a time).
