...
* Windows 7 will show the device as an unknown PCI device until a matching driver
  ...
  **Aero contract v1** hardware IDs (revision-gated `REV_01`):
  ...
  - Optional legacy filename alias `inf/virtio-input.inf.disabled` (disabled by default; rename to `virtio-input.inf` to enable):
    ...
    - Adds an opt-in strict revision-gated generic fallback HWID (no SUBSYS): `PCI\VEN_1AF4&DEV_1052&REV_01`
      (**Aero VirtIO Input Device**).
    - Allowed to diverge from `inf/aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` / `[Aero.NTamd64]`) to
      add the opt-in fallback entry; outside the models sections it must remain byte-for-byte identical (see `../scripts/check-inf-alias.py`).
  ...
  wins over the generic fallback when the fallback alias is enabled and both are installed.
...
(`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable). It is allowed to diverge from
`inf/aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` / `[Aero.NTamd64]`) to add the opt-in fallback entry,
but must otherwise remain byte-for-byte identical from the first section header (`[Version]`) onward (only the leading
banner/comments may differ; see `../scripts/check-inf-alias.py`).
