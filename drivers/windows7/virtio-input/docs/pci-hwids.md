...
* The “Hardware Ids” list in Device Manager includes more-specific forms (with
  ...
  **Aero contract v1** hardware IDs (revision-gated `REV_01`):
  ...
  - Optional legacy filename alias `inf/virtio-input.inf.disabled` (rename to `virtio-input.inf` to enable):
    - Intended to remain byte-for-byte identical to `aero_virtio_input.inf` from `[Version]` onward (a filename alias only).
    - Do not install/ship it alongside `aero_virtio_input.inf` (overlapping matches can cause confusing install behavior).
  This avoids binding to non-contract virtio-input devices (no `REV_01`, wrong DEV, etc). The tablet INF is more specific (`SUBSYS_0012...`), so it wins over the generic fallback when both driver packages are present.
...
The keyboard/mouse INF (`inf/aero_virtio_input.inf`) includes a strict revision-gated generic fallback
`PCI\VEN_1AF4&DEV_1052&REV_01` entry that will appear as **Aero VirtIO Input Device** when subsystem IDs are not exposed.

Note: `inf/virtio-input.inf.disabled` is a legacy filename alias of `inf/aero_virtio_input.inf` for workflows that still
expect `virtio-input.inf` and is expected to stay byte-for-byte identical to the canonical INF from `[Version]` onward
