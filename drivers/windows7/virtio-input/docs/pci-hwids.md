...
* Windows 7 will show the device as an unknown PCI device until a matching driver
  ...
  **Aero contract v1** hardware IDs (revision-gated `REV_01`):
  ...
  - Optional legacy filename alias: `inf/virtio-input.inf.disabled`
...
so it wins over the generic fallback when both driver packages are installed. If the tablet INF is not installed (or the device does
not expose the tablet subsystem ID), the generic fallback entry can also bind to tablet devices (but will use the generic device name).
