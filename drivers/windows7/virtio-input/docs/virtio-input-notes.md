...
When binding via the generic fallback model line (no `SUBSYS`), Device Manager will show the generic **Aero VirtIO Input Device**
name rather than distinct keyboard/mouse names.
...
- The legacy alias INF (`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable) is a filename alias only:
