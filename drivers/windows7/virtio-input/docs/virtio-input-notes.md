...
- Strict generic fallback (when subsystem IDs are not exposed/recognized): `PCI\VEN_1AF4&DEV_1052&REV_01` → `inf/virtio-input.inf.disabled` (rename to `virtio-input.inf` to enable)
- Tablet (absolute pointer / EV_ABS): `PCI\VEN_1AF4&DEV_1052&SUBSYS_00121AF4&REV_01` → `inf/aero_virtio_tablet.inf`

The canonical keyboard/mouse INF (`inf/aero_virtio_input.inf`) is intentionally **SUBSYS-only** (no strict generic fallback HWID)
to keep keyboard/mouse vs tablet binding deterministic.
If you need the strict revision-gated generic fallback binding (`PCI\VEN_1AF4&DEV_1052&REV_01`), enable the legacy filename alias
INF (`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable).

The `...&SUBSYS_...&REV_01` variants use distinct `DeviceDesc` strings so the keyboard and mouse PCI functions show up as
...
Tablet devices bind via `inf/aero_virtio_tablet.inf` when that INF is installed (its `SUBSYS_0012...` HWID is more specific than the generic fallback, so it wins). If the tablet INF is not installed, the generic fallback entry can also bind to tablet devices (but will use the generic device name).
...
It is allowed to diverge from `inf/aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` / `[Aero.NTamd64]`) to add
the opt-in fallback entry, but must otherwise remain byte-for-byte identical from the first section header (`[Version]`) onward
(only the leading banner/comments may differ; see `../scripts/check-inf-alias.py`).
Do not ship/install it alongside `aero_virtio_input.inf` (it is intended as a compatibility filename alias, not a second distinct
driver package).
