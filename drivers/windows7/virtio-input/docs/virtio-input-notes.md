...
If your environment does not expose the Aero subsystem IDs (for example stock QEMU), an optional legacy filename alias
INF (`inf/virtio-input.inf.disabled`, rename to `virtio-input.inf` to enable) includes a strict generic fallback HWID
(no `SUBSYS_...`): `PCI\VEN_1AF4&DEV_1052&REV_01`.
...
Do not ship/install both `aero_virtio_input.inf` and the alias INF at the same time (they overlap on the keyboard/mouse IDs).
Outside the models sections, the alias INF is expected to stay in sync with `aero_virtio_input.inf` (see `drivers/windows7/virtio-input/scripts/check-inf-alias.py`).
