...
for workflows/tools that still reference `virtio-input.inf`. From the first section header (`[Version]`) onward it is expected
to remain **byte-for-byte identical** to `aero_virtio_input.inf` (CI enforces this). Do not ship/install both filenames at the
same time.
