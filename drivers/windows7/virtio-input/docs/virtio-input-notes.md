...
revision-gated even when subsystem IDs are not exposed/recognized or a device ships with an unexpected subsystem ID.
...
`inf/aero_virtio_input.inf` from the first section header (`[Version]`) onward (only the banner/comments may differ; see
`../scripts/check-inf-alias.py`). Do not ship/install it alongside `aero_virtio_input.inf` (it is intended as a compatibility
filename alias, not a second distinct driver package).
