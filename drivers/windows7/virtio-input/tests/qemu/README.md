...
- Tablet/absolute pointer: `aero_virtio_tablet.inf`
  ...
  - This HWID is more specific than the generic fallback, so it wins when it matches (i.e. when both the tablet INF and the opt-in alias INF are installed).
  - If the tablet subsystem ID is missing (or `aero_virtio_tablet.inf` is not installed), the device may bind via the generic fallback
    model line (when enabled via the legacy alias INF) and appear as **Aero VirtIO Input Device**.
- Optional legacy filename alias (disabled by default): `virtio-input.inf.disabled` → rename to `virtio-input.inf` to enable
  ...
  - Adds opt-in strict generic fallback matching (no `SUBSYS`): `PCI\VEN_1AF4&DEV_1052&REV_01` (**Aero VirtIO Input Device**).
  - Drift policy: allowed to diverge from `aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` / `[Aero.NTamd64]`) where it adds
    the fallback model line. Outside those models sections, from the first section header (`[Version]`) onward, it must remain byte-identical
...
If your device is `REV_01` but does not expose the Aero subsystem IDs, the canonical `aero_virtio_input.inf` will not bind (SUBSYS-only).
To bind in that environment, enable the legacy alias INF (`virtio-input.inf.disabled` → rename to `virtio-input.inf`), which adds the strict
generic fallback model line (`PCI\VEN_1AF4&DEV_1052&REV_01`). In that case the device will show up as **Aero VirtIO Input Device**.
...
HWID set **and** adds the strict generic fallback, which can lead to duplicate DriverStore entries and confusing PnP driver selection.
