...
- Tablet: `aero_virtio_tablet.inf` matches `SUBSYS_00121AF4` (Aero contract tablet). This HWID is more specific than the generic fallback,
  so it will win when it matches.
...
Tablet devices without the Aero tablet subsystem ID may also bind via the fallback entry if `aero_virtio_tablet.inf` does not match and
the driver classifies the device as a tablet via `EV_BITS` (`EV_ABS` + `ABS_X`/`ABS_Y`).
