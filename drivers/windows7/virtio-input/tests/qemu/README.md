...
- Keyboard/mouse (canonical): `aero_virtio_input.inf`
  ...
  - Note: canonical is intentionally **SUBSYS-only** (no strict generic fallback entry).
- Tablet/absolute pointer: `aero_virtio_tablet.inf`
  - Contract tablet HWID: `...&SUBSYS_00121AF4&REV_01` (Aero contract tablet).
  - This HWID is more specific than the generic fallback, so it wins when it matches.
  - If the tablet subsystem ID is missing (or `aero_virtio_tablet.inf` is not installed), the device may bind via the strict
    generic fallback (when enabled via the legacy alias INF) and appear as **Aero VirtIO Input Device**.
- Optional legacy filename alias (disabled by default): `virtio-input.inf.disabled` → rename to `virtio-input.inf` to enable
  ...
  - Adds an opt-in strict generic fallback model entry (no `SUBSYS`): `PCI\VEN_1AF4&DEV_1052&REV_01`
  ...
  - Alias drift policy: allowed to diverge from `aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` /
    `[Aero.NTamd64]`) where the alias adds the fallback match. Outside those sections it is expected to remain byte-identical
...
If your device is `REV_01` but does not expose the Aero subsystem IDs, the canonical INF (`aero_virtio_input.inf`) will not bind.
Enable the legacy alias INF to opt into the strict generic fallback entry (`PCI\VEN_1AF4&DEV_1052&REV_01`). In that case it will show
up as **Aero VirtIO Input Device**.
...
(it is more specific than the generic fallback match, so it wins when both are present).
