...
>   - Compatibility filename for workflows/tools that still reference `virtio-input.inf` instead of `aero_virtio_input.inf`.
>   - Filename-only alias: from the first section header (`[Version]`) onward, expected to be byte-for-byte identical to
