...
>   - Note: the canonical keyboard/mouse INF is intentionally **SUBSYS-only** (no strict generic fallback model line).
> - Tablet/absolute pointer: `aero_virtio_tablet.inf` (`...&SUBSYS_00121AF4&REV_01`)
>   - Tablet binding is more specific, so it wins over the generic fallback (if enabled) when the tablet subsystem ID is present
...
>   - Adds the **opt-in** strict generic fallback HWID (no `SUBSYS`): `PCI\VEN_1AF4&DEV_1052&REV_01`
