...
>   - canonical INF is intentionally **SUBSYS-gated** and does **not** include a generic (no `SUBSYS`) fallback entry.
...
>   - Allowed to diverge from `aero_virtio_input.inf` only in the models sections (`[Aero.NTx86]` / `[Aero.NTamd64]`) to add an
>     opt-in strict revision-gated generic fallback match (no `SUBSYS`): `PCI\VEN_1AF4&DEV_1052&REV_01`.
>     Outside those models sections, it is expected to stay in sync (see `drivers/windows7/virtio-input/scripts/check-inf-alias.py`).
