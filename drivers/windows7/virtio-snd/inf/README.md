...
- Installable INF(s):
  ...
  - (Optional) `virtio-snd.inf` legacy filename alias (rename `virtio-snd.inf.disabled` back to `virtio-snd.inf` to enable; installs the same driver and matches the same contract-v1 HWID; kept for compatibility with older tooling/workflows)
