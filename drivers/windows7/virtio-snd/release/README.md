...
- `virtio-snd.inf` (optional; legacy filename alias kept for compatibility; rename from `virtio-snd.inf.disabled` to enable)
...
> Catalogs must be regenerated per-architecture because `Inf2Cat` hashes the staged `virtiosnd.sys` / `virtiosnd_legacy.sys`.
