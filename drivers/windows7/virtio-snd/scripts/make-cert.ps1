...
<#
.SYNOPSIS
  Creates a self-signed code-signing certificate for test-signing the Aero virtio-snd driver.

.DESCRIPTION
  Generates an exportable code-signing cert in the CurrentUser\My store and exports:
    - aero-virtio-snd-test.cer
    - aero-virtio-snd-test.pfx
...
param(
  ...
  [string]$Subject = "CN=Aero virtio-snd Test Certificate",
  [string]$PfxPassword,
...
$cerPath = Join-Path $OutDir "aero-virtio-snd-test.cer"
$pfxPath = Join-Path $OutDir "aero-virtio-snd-test.pfx"
