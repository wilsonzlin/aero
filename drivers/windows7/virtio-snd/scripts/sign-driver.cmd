...
rem
rem Contract (default):
rem   inf\aero_virtio_snd.inf
rem   inf\aero_virtio_snd.sys
rem   inf\aero_virtio_snd.cat
rem
rem Legacy (optional / QEMU compatibility):
rem   inf\aero-virtio-snd-legacy.inf
rem   inf\virtiosnd_legacy.sys
...
if /I "%VARIANT%"=="contract" (
  if not exist "%CONTRACT_INF%" (
    echo ERROR: INF not found: "%CONTRACT_INF%"
    exit /b 1
  )
...
if /I "%VARIANT%"=="contract" (
  ...
  signtool.exe sign /v /fd SHA1 /f "%PFX_FILE%" /p "%SIGN_PFX_PASS%" "%LEGACY_SYS%"
  if errorlevel 1 (
    echo ERROR: Failed to sign SYS: "%LEGACY_SYS%"
    exit /b 1
  )
