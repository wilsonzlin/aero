...
static __forceinline USHORT AerovNetReadBe16(_In_reads_bytes_(2) const UCHAR* Ptr) {
  return (USHORT)(((USHORT)Ptr[0] << 8) | (USHORT)Ptr[1]);
}

// NDIS receive checksum offload indicators are stored in
// `TcpIpChecksumNetBufferListInfo` as a bitfield value. Define the bit positions
// directly to avoid relying on WDK-specific bitfield member names.
#define AEROVNET_NDIS_RX_CSUM_IP_FAILED (1u << 0)
#define AEROVNET_NDIS_RX_CSUM_IP_SUCCEEDED (1u << 1)
#define AEROVNET_NDIS_RX_CSUM_TCP_FAILED (1u << 2)
#define AEROVNET_NDIS_RX_CSUM_TCP_SUCCEEDED (1u << 3)
#define AEROVNET_NDIS_RX_CSUM_UDP_FAILED (1u << 4)
#define AEROVNET_NDIS_RX_CSUM_UDP_SUCCEEDED (1u << 5)
#define AEROVNET_NDIS_RX_CSUM_TCP_VALUE_INVALID (1u << 6)
#define AEROVNET_NDIS_RX_CSUM_UDP_VALUE_INVALID (1u << 7)

// Attempts to translate virtio-net RX checksum metadata into NDIS checksum
// offload indicators. Returns 0 when checksum status is unknown.
static ULONG AerovNetGetRxChecksumValue(_In_ const VIRTIO_NET_HDR* VirtioHdr, _In_reads_bytes_(FrameLen) const UCHAR* Frame, _In_ ULONG FrameLen) {
  ULONG CsumValue;
  USHORT Ethertype;
  ULONG L2Len;
  ULONG I;
  ...
  // If the device didn't validate the checksum, leave the NBL checksum info as
  // "unknown" so the Windows network stack can validate in software.
  if ((VirtioHdr->Flags & VIRTIO_NET_HDR_F_DATA_VALID) == 0) {
    return 0;
  }

  // Minimum Ethernet II header.
  if (FrameLen < 14) {
    return 0;
  }
  ...
  // Contract v1 allows up to two VLAN tags (QinQ). Handle common VLAN Ethertypes.
  // Outer: 0x8100 (802.1Q), 0x88A8 (802.1ad).
  for (I = 0; I < 2; I++) {
  ...
  if (Ethertype == 0x0800u) {
    ...
    Ihl = (ULONG)(Ip[0] & 0x0Fu) * 4u;
    if (Ihl < 20u || FrameLen < L2Len + Ihl) {
      return 0;
    }

    // Indicate IPv4 header checksum validated.
    CsumValue |= AEROVNET_NDIS_RX_CSUM_IP_SUCCEEDED;

    // For fragmented IPv4 packets, avoid indicating transport checksum status
    // (the full transport checksum cannot be validated without reassembly).
    Frag = AerovNetReadBe16(Ip + 6);
    ...
    Proto = Ip[9];
    if (Proto == 6u) { // TCP
      if (FrameLen >= L2Len + Ihl + 20u) {
        CsumValue |= AEROVNET_NDIS_RX_CSUM_TCP_SUCCEEDED;
      }
    } else if (Proto == 17u) { // UDP
      if (FrameLen >= L2Len + Ihl + 8u) {
        CsumValue |= AEROVNET_NDIS_RX_CSUM_UDP_SUCCEEDED;
      }
    }
  ...
  // IPv6 (no header checksum). For simplicity, only handle packets without
  // extension headers (NextHeader directly TCP/UDP).
  if (Ethertype == 0x86DDu) {
    const UCHAR* Ip6;
    UCHAR NextHdr;

    if (FrameLen < L2Len + 40) {
      return 0;
    }
    ...
    NextHdr = Ip6[6];
    if (NextHdr == 6u) { // TCP
      if (FrameLen >= L2Len + 40u + 20u) {
        CsumValue |= AEROVNET_NDIS_RX_CSUM_TCP_SUCCEEDED;
      }
    } else if (NextHdr == 17u) { // UDP
      if (FrameLen >= L2Len + 40u + 8u) {
        CsumValue |= AEROVNET_NDIS_RX_CSUM_UDP_SUCCEEDED;
      }
    }
...
static VOID AerovNetInterruptDpc(_In_ NDIS_HANDLE MiniportInterruptContext, _In_ PVOID MiniportDpcContext,
  ...
  for (;;) {
    ...
    // software checksum validation.
    {
      ...
      ULONG CsumValue =
          AerovNetGetRxChecksumValue(VirtioHdr, Rx->BufferVa + sizeof(VIRTIO_NET_HDR), PayloadLen);
      if (CsumValue != 0) {
