...
QEMU boot integration tests live under the workspace root crate (`aero`) in `tests/` (e.g.
`tests/boot_sector.rs`, `tests/freedos_boot.rs`, `tests/windows7_boot.rs`). The `emulator` crate
also registers these files as `[[test]]` targets, but the canonical invocations use `-p aero`.
