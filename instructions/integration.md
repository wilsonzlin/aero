...
- **PCI MSI/MSI-X message delivery (for devices that opt in)**:
  ...
  - virtio-pci MSI-X delivery in both `aero-machine` and `aero-pc-platform` via real virtio
...
| `crates/emulator/` | Legacy/compat integration stack + reference harnesses (not canonical VM wiring; see [`docs/21-emulator-crate-migration.md`](../docs/21-emulator-crate-migration.md)) |
| `crates/aero-boot-tests/` | QEMU-based reference boot-test harness (registers workspace-root `tests/*.rs` boot suites as `[[test]]` targets; see [`docs/TESTING.md`](../docs/TESTING.md)) |
...
| `tests/fixtures/boot/` | Deterministic tiny boot fixtures generated by `cargo xtask fixtures` (CI verifies no diff) |
| `tests/boot_sector.rs`, `tests/freedos_boot.rs`, `tests/windows7_boot.rs` | QEMU-based reference boot tests (registered under `crates/aero-boot-tests`) |
...
| MP-002 | MSI/MSI-X: unify config-state mirroring in canonical PCI integrations | P1 | High | Message delivery exists (`PlatformInterrupts::trigger_msi`) and is used by AHCI (MSI), NVMe (MSI/MSI-X), and virtio (MSI-X). The remaining integration pain is **keeping device-internal capability state coherent** with the canonical PCI config space (`PciConfigPorts`) (e.g. mirroring MSI/MSI-X enable/mask state into device models when the platform owns PCI config space). Snapshot regression: `crates/aero-machine/tests/machine_snapshot_preserves_msix_enable.rs`. |
