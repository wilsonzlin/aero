...
| `crates/emulator/` | QEMU reference test harness (boot tests are registered here) + legacy/compat stack (not canonical VM wiring; see [`docs/21-emulator-crate-migration.md`](../docs/21-emulator-crate-migration.md)) |
...
| `tests/boot_sector.rs`, `tests/freedos_boot.rs`, `tests/windows7_boot.rs` | QEMU-based reference boot tests (registered under `crates/emulator`) |
