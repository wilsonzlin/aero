...
| AGPU-WIRE-002 | Implemented (in `crates/aero-machine/`) | Boot display → WDDM scanout handoff: once the guest enables scanout0, `Machine::display_present` prefers the WDDM framebuffer over VBE/text (sticky until reset; `SCANOUT0_ENABLE=0` blanks output but does not release ownership) | `crates/aero-machine/src/lib.rs` (`display_present`, `display_present_aerogpu_scanout`) | `bash ./scripts/safe-run.sh cargo test -p aero-machine --test aerogpu_scanout_handoff --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-machine --test aerogpu_scanout_enable_before_fb_is_ok --locked` |
