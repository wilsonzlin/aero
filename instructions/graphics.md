...
| D11-003b | Remaining | Broaden GS translation/execution coverage (more opcodes/operands, resource access, additional output topologies, adjacency, multi-stream/stream-out, GS instancing, etc). | Start at: `crates/aero-d3d11/src/runtime/gs_translate.rs` | Add tests under `crates/aero-d3d11/tests/` and run `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --locked` |
