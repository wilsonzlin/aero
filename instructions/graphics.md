...
| D11-003 | Partial | Geometry shaders: **stage_ex ABI + compute-emulated GS execution** (wgpu has no native GS stage). Draws route through a compute “prepass” that runs the guest GS and expands strip output into list geometry (including `RestartStrip`/cut semantics) for a follow-up render pass (typically via indirect draw). | `crates/aero-d3d11/src/runtime/aerogpu_cmd_executor.rs`, `crates/aero-d3d11/src/runtime/gs_translate.rs`, `crates/aero-d3d11/src/runtime/strip_to_list.rs`, `crates/aero-d3d11/tests/aerogpu_cmd_geometry_shader_restart_strip.rs` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_smoke --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_point_to_triangle --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_restart_strip --locked` |
| D11-003a | Implemented (folded into D11-003) | Robustness/forward-compat: stage_ex GS plumbing acceptance + stage mismatch validation (kept as a regression test; full GS execution is covered by D11-003). | `crates/aero-d3d11/tests/aerogpu_cmd_geometry_shader_ignore.rs` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_ignore --locked` |
