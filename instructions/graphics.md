...
| AGPU-DEV-001 | Implemented (in `crates/emulator/`, not yet `aero_machine`) | Reference AeroGPU PCI function device model used by host-side tests (`crates/emulator`): full BAR0 MMIO + ring semantics + IRQ/vblank/scanout plumbing (pairs with AGPU-DEV-002 for command execution) | `crates/emulator/src/devices/pci/aerogpu.rs` | `bash ./scripts/safe-run.sh cargo test -p emulator --test aerogpu_device --locked` |
...
| D11-003 | Partial | Geometry shaders: **stage_ex ABI + compute-prepass scaffolding** (wgpu has no native GS stage). Draws route through a compute+indirect prepass, but guest GS DXBC execution is still WIP (DXBC is accepted-but-ignored today; prepass emits fixed geometry). | `crates/aero-d3d11/src/runtime/aerogpu_cmd_executor.rs`, `crates/aero-d3d11/tests/aerogpu_cmd_geometry_shader_*` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_smoke --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_point_to_triangle --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test gs_cut_restartstrip --locked` |
| D11-003a | Implemented | Robustness/forward-compat: stage_ex GS plumbing acceptance + stage mismatch validation (does **not** assert full GS execution). | `crates/aero-d3d11/tests/aerogpu_cmd_geometry_shader_ignore.rs` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_ignore --locked` |
