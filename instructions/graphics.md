...
| D11-003 | Implemented (subset) | Geometry shader emulation via **compute prepass** (subset; wgpu has no native GS stage). Current coverage includes point-list expansion and triangle-strip restart semantics (`cut`/RestartStrip). | `crates/aero-d3d11/src/runtime/aerogpu_cmd_executor.rs`, `crates/aero-d3d11/tests/aerogpu_cmd_geometry_shader_point_to_triangle.rs`, `crates/aero-d3d11/tests/gs_cut_restartstrip.rs` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_point_to_triangle --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test gs_cut_restartstrip --locked` |
