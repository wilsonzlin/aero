...
| D11-003b | Remaining | Wire guest GS DXBC execution into the compute-prepass path (VS-as-compute + GS-as-compute), then broaden GS translation/execution coverage (more opcodes/operands, resource access, additional output topologies, adjacency, multi-stream/stream-out, GS instancing, etc). | Start at: `crates/aero-d3d11/src/runtime/{aerogpu_cmd_executor.rs,gs_translate.rs,strip_to_list.rs}` | Add tests under `crates/aero-d3d11/tests/` and run `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --locked` |
