...
| D11-003 | Partial | Geometry shaders: GEOMETRY stage + `stage_ex` plumbing + **compute prepass scaffolding** (wgpu has no native GS stage). Draws route through a compute prepass and then an indirect render pass; today the prepass uses built-in synthetic expansion geometry (e.g. fixed triangles) and does **not** execute guest GS/HS/DS DXBC yet. A GS DXBC→WGSL compute translator exists (supported subset), but is not yet integrated into the executor (see D11-003b). | `crates/aero-d3d11/src/runtime/{aerogpu_cmd_executor.rs,gs_translate.rs,strip_to_list.rs}` • Design notes: `docs/graphics/geometry-shader-emulation.md` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_smoke --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_primitive_id --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_vertex_pulling --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test gs_translate --locked` |
