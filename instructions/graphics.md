...
| D11-003 | Partial | Geometry shaders: GEOMETRY stage + `stage_ex` plumbing + **compute prepass path** (wgpu has no native GS stage). Draws route through a compute prepass and then an indirect render pass; the prepass uses built-in synthetic expansion geometry for bring-up/fallback, and can execute a minimal translated SM4 GS DXBC subset for non-indexed `PointList` draws. HS/DS execution is not implemented yet. | `crates/aero-d3d11/src/runtime/{aerogpu_cmd_executor.rs,gs_translate.rs,strip_to_list.rs}` • Design notes: `docs/graphics/geometry-shader-emulation.md` | `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_smoke --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_primitive_id --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test aerogpu_cmd_geometry_shader_compute_prepass_vertex_pulling --locked`; `bash ./scripts/safe-run.sh cargo test -p aero-d3d11 --test gs_translate --locked` |
