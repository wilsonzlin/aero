...
const OUT_TS_PATH = path.join(REPO_ROOT, 'web/src/input/scancodes.ts');
const OUT_RS_PATH = path.join(REPO_ROOT, 'crates/emulator/src/io/input/scancodes.rs');
...
async function main() {
  ...
  // --- Generate TypeScript --------------------------------------------------
  await fs.mkdir(path.dirname(OUT_TS_PATH), { recursive: true });
  ...
  await fs.writeFile(OUT_TS_PATH, ts, 'utf8');

  // --- Generate Rust --------------------------------------------------------
  await fs.mkdir(path.dirname(OUT_RS_PATH), { recursive: true });
  ...
  await fs.writeFile(OUT_RS_PATH, rs, 'utf8');
}
