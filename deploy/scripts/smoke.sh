...
# - `/l2` should default to cookie auth (`AERO_L2_AUTH_MODE=cookie`), matching `/tcp`.
# - Operators can disable cookie auth explicitly by setting `AERO_L2_AUTH_MODE=none` in deploy/.env.
env \
  -u AERO_L2_AUTH_MODE \
  -u AERO_L2_SESSION_SECRET \
  -u AERO_L2_API_KEY \
  ...
  | python3 -c 'import json, sys; cfg=json.load(sys.stdin); env=(cfg.get("services", {}).get("aero-l2-proxy", {}) or {}).get("environment", {}) or {}; mode=env.get("AERO_L2_AUTH_MODE", ""); expected="cookie"; \
print(f"deploy smoke: expected deploy/docker-compose.yml to default AERO_L2_AUTH_MODE={expected!r}, got: {mode!r}", file=sys.stderr) if mode != expected else None; sys.exit(1 if mode != expected else 0)'
